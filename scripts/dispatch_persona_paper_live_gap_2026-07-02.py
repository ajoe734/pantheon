...
TASKS = [
    ...
    (
        ...
        "補 Fleet readiness projection 並移除重複大 payload，讓 row 清楚顯示 paper/evaluation/review/live/risk 狀態。",
        "Codex2",
        "Claude2",
        "EPIC PPLG / fleet read model",
        "PPLG-001",
        "fleet distinguishes setup_failed paper_running eligible review canary live quarterly risk_off frozen;payload materially smaller;contract tests cover row shape",
    ...
    (
        "PPLG-004",
        "Paper eligibility, promotion score, and cohort ranking engine",
        "實作 paper hard gates、promotion_score、cohort ranking 與 recommendation packet；系統只推薦，不批准實盤。",
        "Claude2",
        "Codex",
        "EPIC PPLG / evaluation ranking",
        "PPLG-001",
        "ineligible personas not queued;eligible rows expose score components gates percentile evidence;recommendation cannot start canary/live without human decision;tests cover threshold and overrides",
    ...
    (
        "PPLG-007",
        "Frontend Create Paper Persona and Fleet UX",
        "更新 Persona Registry/Fleet：主要 CTA 是建立 Paper Persona，row action 依狀態顯示，不再對已跑 paper 的 persona 顯示啟動精靈。",
        "Codex2",
        "Claude",
        "EPIC PPLG / frontend UX",
        "PPLG-002,PPLG-003,PPLG-005",
        "primary create reaches paper_running or setup_failed;row actions match concrete states;eligible shows promotion review;canary/live show approval evidence;e2e covers no canary without approval",
    ...
    (
        ...
        "e2e proves create to paper runtime;recommendation cannot start canary without approval;quarterly cannot execute without approval;risk_off interrupts immediately;closeout maps every spec requirement",
