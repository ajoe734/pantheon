...
def test_mutable_incumbent_bootstrap_accepts_provenance_validated_legacy_task_brief_drift(
    ...
    regenerated_brief.write_text(valid_legacy_brief_content, encoding="utf-8")
...
def test_mutable_incumbent_bootstrap_accepts_second_live_byte_legacy_task_brief_drift(
    ...
    second_brief_content = (
        ...
        "- Owner: Antigravity\n"
        "- Reviewer: Codex2\n"
        "- Phase: Fleet Coherence Gate\n"
        "- Last update: 2026-08-09T08:23:22Z\n"
        "- Next: Fleet bootstrap gate validation.\n\n"
        "## Summary\n"
        "-\n\n"
        "## Dependencies\n"
        "- none\n\n"
        "## Artifacts\n"
        "- docs/04/fleet_bootstrap_coherence_gate_2026-08-01.md\n\n"
        "## Recent Task Activity\n"
        "- Omitted from automatic dispatch context.\n\n"
        "## Relevant Canonical Files\n"
        "- AI_COLLABORATION_GUIDE.md\n"
        "- ai-status.json\n\n"
        "## Working Rules\n"
    ...
    second_brief.parent.mkdir(parents=True, exist_ok=True)
    second_brief.write_text(second_brief_content, encoding="utf-8")
    assert len(second_brief_content.encode("utf-8")) == 2132
    assert (
        hashlib.sha256(second_brief_content.encode("utf-8")).hexdigest()
        == "255c0d0e547fdcd0869a2df0a728883f38159cd31695e0d1a70939695c6d5171"
    )
...
def test_mutable_incumbent_bootstrap_rejects_wrong_authoritative_event(
    ...
    with (
        ...
        with pytest.raises(
            ValueError,
            match="authoritative event ID is invalid or unreviewed",
        ):
            promotion._verify_legacy_task_brief_provenance(
                mutable_root,
                relative_path=rel_path,
...
def test_mutable_incumbent_bootstrap_rejects_wrong_legacy_command_runtime_sha(
    tmp_path: Path,
) -> None:
    remote, mutable_root, regenerated_brief, _root_stat = (
        _legacy_mutable_task_brief_drift_fixture(tmp_path)
    ...
    bad_binding = promotion.CandidateTrackedLegacyTaskBriefProvenanceBinding(
        ...
        legacy_command_runtime_sha="0000000000000000000000000000000000000000",
        prevention_boundary_sha="f5570754e6b9534893fc65744e82abe7f0ff0a74",
    )
    ...
    with (
        ...
        with pytest.raises(
            ValueError,
            match="legacy command runtime SHA is not an ancestor",
        ):
            promotion._verify_legacy_task_brief_provenance(
                mutable_root,
                relative_path=rel_path,
...
def test_mutable_incumbent_bootstrap_rejects_wrong_prevention_boundary_sha(
    tmp_path: Path,
) -> None:
    remote, mutable_root, regenerated_brief, _root_stat = (
        _legacy_mutable_task_brief_drift_fixture(tmp_path)
    ...
    bad_binding = promotion.CandidateTrackedLegacyTaskBriefProvenanceBinding(
        ...
        authoritative_event_id="supervisor-task-failure-streak-a9d6b8a54889ffae650c47e67d004eff0dd93f691a92791345e2bcd38cbdccf6",
        legacy_command_runtime_sha="5877b64425c8d6aede147d6cbbc6fbb9e228c259",
        prevention_boundary_sha="0000000000000000000000000000000000000000",
    )
    ...
    with (
        ...
        with pytest.raises(
            ValueError,
            match="prevention boundary SHA is not an ancestor",
        ):
            promotion._verify_legacy_task_brief_provenance(
                mutable_root,
                relative_path=rel_path,
