...
NEXT_BY_TASK = {
    ...
    "MGMT-GAP-004": "Start durable write truth: enumerate in-scope write CTAs, replace toast/local-only success with command receipt/audit or explicit disabled state, and prove dry-run/no-side-effect behavior.",
    "MGMT-GAP-005": "Start studios/capabilities hardening: make Formula Studio, Skill Sandbox, Tools/MCP/Skills runtime-backed with job/command evidence and limits, or demote/disable them.",
    "MGMT-GAP-006": "Wait for MGMT-GAP-004/005/008/009/010, then build the hosted strict-live harness covering routes, endpoints, mock/write-success, undefined/NaN, auth, and load signals.",
    "MGMT-GAP-007": "Wait for MGMT-GAP-006, then archive final PR/deploy/BFF/OpenAPI/hosted-harness evidence and residual risks.",
    "MGMT-GAP-008": "Start detail honesty hardening: fix live-id detail undefined/blank/NaN states, alias canonicalization, empty capability registries, and evidence-source degraded copy.",
    "MGMT-GAP-009": "Start session/RBAC contract hardening: align /bff/me, tenant, roles, and management reads for the documented dev gate token path.",
    "MGMT-GAP-010": "Start load-gate work: enforce management bundle budgets, route-ready markers, shell request-count evidence, and no network-idle-only readiness.",
}
...
TASKS = [
    ...
    (
        ...
        "修正 2026-07-01 全量再盤點發現的 detail DTO/render 誠實度問題: status.undefined、risk.undefined、blank h1/owner/update、NaN%、alias duplicate render、empty capability registry seed ids。",
        "Claude",
        "Codex",
        "MGMT Console Production Gap / Batch 2.5 detail render honesty",
        "MGMT-GAP-002,MGMT-GAP-003",
        "live-id detail pages 不再顯示 undefined/blank/NaN; old detail aliases redirect or canonicalize; empty Tools/MCP/Skills registries show explicit live-empty state; hosted probe records live ids and screenshots/logs",
    ...
    (
        ...
        "MGMT-GAP-003",
        "/bff/me 與 management reads 對同一 token/tenant 的 auth 結果一致; privileged pages 不可在 session 403 時顯示 live data; BFF + hosted browser tests cover success/403/role-missing paths",
    ...
    (
        ...
        "落實 management load-gap follow-up: code split management routes、defer shell fanout、aggregate counts、remove duplicate jobs reads、把 bundle/load regression 納入 release gate。",
        "Gemini2",
        "Codex",
        "MGMT Console Production Gap / Batch 5 load release gate",
        "MGMT-GAP-001,MGMT-GAP-002",
        "management initial bundle/runtime chunk budget documented and enforced; shell no longer fans out duplicate heavyweight reads before route readiness; hosted probe avoids network-idle false readiness and records load evidence",
