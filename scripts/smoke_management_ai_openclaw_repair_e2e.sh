...
status_tmp="$(mktemp)"
auth_header="$(mktemp)"
chmod 0600 "${request_tmp}" "${response_tmp}" "${status_tmp}" "${auth_header}"
...
deactivate_control_mode() {
  ...
  if [ "${activated}" = "true" ]; then
    ...
    deactivate_code="$(curl_json POST /bff/assistant/control-mode/deactivate "${request_tmp}" 30)"
...
cleanup() {
  ...
  rm -f "${request_tmp}" "${response_tmp}" "${status_tmp}" "${auth_header}"
  exit "${final_status}"
}
trap cleanup EXIT
