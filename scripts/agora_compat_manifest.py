...
EXTENSION_BUNDLE_PATH = "services/control-plane/specs/agora/bundle_index.v1_5.json"
OPENAPI_PATH = "services/control-plane/openapi/agora_v1_5.openapi.yaml"
...
def generated_types_snapshot_reasons(frontend_root: Path) -> list[str]:
    ...
    if snapshot.get("contract_version") != "1.5":
        return ["frontend-generated-types-not-agora-v1.5"]
...
def validate_manifest_shape(manifest: dict[str, Any]) -> list[str]:
    ...
    if manifest.get("contract_family") != "agora.v1.5":
        issues.append("contract_family must be agora.v1.5")
