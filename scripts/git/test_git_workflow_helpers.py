...
class PullRequestTrailerRangeLiveRegressionTests(unittest.TestCase):
    ...
    def test_pr_and_push_use_the_same_source_classification(self) -> None:
        workflow = (ROOT / ".github" / "workflows" / "branch-ci.yml").read_text()
        push_block = workflow.split("  pull_request:", 1)[0]
        self.assertNotIn('      - "task/**"', push_block)
        self.assertIn('      - "hotfix/**"', push_block)
        self.assertNotIn("github.event.pull_request.labels.*.name", workflow)
        self.assertIn('--delivery-class auto', workflow)
        self.assertNotIn('--delivery-class "$DELIVERY_CLASS"', workflow)
