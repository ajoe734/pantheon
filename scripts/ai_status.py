...
def write_current_work(state: dict[str, Any], logs: list[dict[str, Any]]) -> None:
    ...
    if current_logs:
        for entry in current_logs:
            ...
            message = entry.get("message") or entry.get("type") or "(no message)"
            lines.append(
                f"- {format_display_timestamp(timestamp)} {entry.get('agent') or 'Unknown'}:{task_id} {localize_embedded_timestamps(str(message))}"
            )
    else:
