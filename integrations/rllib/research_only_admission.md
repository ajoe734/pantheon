# RLlib Research-Only Admission Evidence
...
This evidence records the current RLlib admission posture against the generic
`ProductionDataProof.v1` shape delivered by `RES-ACT-001-V2`.

It is not a production registry write, deployment-stage change, broker route,
runtime binding, or capital-binding claim. RLlib remains a research-only,
gate-closed adapter until a separate registry service and reviewer admit a
complete upstream-backed packet.

## Evidence Inputs

The proof is based on two already reviewed RLlib evidence lanes:

| Evidence | Path |
|---|---|
| Runtime smoke evidence summary | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_activation_evidence_summary.json` |
| Runtime smoke dataset evidence | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_dataset_evidence.json` |
| Runtime smoke artifact bundle | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_artifact_bundle.json` |
| Runtime smoke candidate packet | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_candidate_packet.json` |
| Runtime smoke registry entry | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_registry_entry.json` |
| Runtime smoke real-backend attempt | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/rllib_real_backend_attempt.json` |
| Runtime smoke manifest | `support/evidence/P2-RL-UPSTREAM-RUNTIME-SMOKE-001/manifest.json` |
| RLlib registry admission packet | `support/evidence/OSS-RLLIB-V2-001/admission_packet.json` |
...
| `activation_tier` | `R3` |
| `adapter_kind` | `rllib` |
| `adapter_id` | `rllib-research-only-admission-20260502` |
...
| `provider.dataset_id` | `rllib-activation-smoke-20260501` |
...
The runtime smoke records the upstream RLlib attempt as an explicit dependency
...
| `cause_message` | `No module named 'ray'` |
...
## Research-Only Admission Boundary

The `OSS-RLLIB-V2-001` admission packet is still incomplete for candidate
admission because upstream RLlib PPO backend confirmation is missing:
...
The packet may be reviewed as evidence of shape and safety only. It does not
authorize registry mutation or paper/canary/live deployment.

## No-Order-Route Controls

| Control | Recorded value |
|---|---|
| Static scanner root | `services/research/rllib` |
| Dynamic training probe | `rllib_stub_training_step` in `tests/integrations/test_research_no_order_route.py` |
| Broker outbox | empty |
...
RLlib may produce only research / registry-review artifacts:
...
- capital binding
- direct governance or registry writes from the RLlib adapter
