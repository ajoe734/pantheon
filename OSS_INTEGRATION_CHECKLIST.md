...
|---|---|---|---|
| `OpenClaw` | upstream repo/runtime | `governed` | Source pin bumped to `openclaw/openclaw` tag `v2026.6.8` / commit `844f405ac1be805d5c598922a37254f12ab6d765` (OPENCLAW-GOVERNED-BUMP-2026-6-8, 2026-06-19 (prior 6.6 bump 2026-06-16)); runtime artifact is a Pantheon-derived image (`integrations/openclaw/gateway/Dockerfile` = `FROM ghcr.io/openclaw/openclaw:2026.6.8` + Claude Code CLI baked for the claude-cli agent runtime); upstream pin itself is unchanged. Reason: headless device-code OAuth for subscription accounts (`openclaw models auth login --provider openai --device-code`), zero API key required; the 6.8 bump also enables the OpenAI-compatible `POST /v1/responses` HTTP endpoint with SSE streaming through the agent (for low-latency token-streamed Management AI, see governance.md §11); plus Claude CLI subscription reuse for multi-model persona routing (`anthropic/*` via `claude -p`, see `integrations/openclaw/governance.md §9`). The runnable Pantheon-side adapter lives under `integrations/openclaw/adapter/`, `docker-compose.yml` exposes an `openclaw-gateway` runtime dependency path (profile `openclaw`), and the fail-closed repo-authoritative runtime-adoption scaffold is landed. `bash scripts/openclaw-smoke-test.sh` and `bash scripts/openclaw-gateway-adapter-smoke.sh` rerun against `2026.6.8`; evidence in `integrations/openclaw/evidence_pack.md §7`. This is not broker-session, paper/canary/live, execution-kernel, or capital-binding activation. |
