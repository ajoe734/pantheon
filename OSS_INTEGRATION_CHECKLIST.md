...
| `QuantLib` | Python package/framework | `smoke-tested` | `QuantLib-Python==1.18` pinned in `services/research/quantlib/requirements.txt`; governed adapter implemented in `services/research/quantlib/adapter/` (`GovernedQuantLibInputAdapter`, `StubQuantLibBackend`, `QuantLibBackend`, `run_quantlib_workflow`); stub-path smoke test passes via `python3 services/research/quantlib/smoke_test.py`, default workspace unit coverage is `17 passed, 1 skipped` via `python3 -m pytest services/research/quantlib/test_adapter.py -q`, and the pinned live rerun also passes with `PYTHONPATH=/tmp/oss-impl-002-site python3 -m pytest services/research/quantlib/test_adapter.py -q` => `18 passed` plus `PYTHONPATH=/tmp/oss-impl-002-site python3 services/research/quantlib/smoke_test.py --backend real` on 2026-04-17; governed artifact envelope confirmed (`artifact_family=pricing_report`, `artifact_state=draft`, `direct_live_influence=false`, `lean_consumption=research_only_not_direct_action`); first approved scope remains derivatives pricing and fixed-income risk analytics; next: complete Gate 2 evidence pack (`integrations/quantlib/{governance,smoke_test}.md`) to claim `governed` |
