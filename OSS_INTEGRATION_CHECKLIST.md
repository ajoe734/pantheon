...
|---|---|---|---|
| `OpenClaw` | upstream repo/runtime | `governed` | Source pin is official stable tag `v2026.7.1` / commit `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`; runtime artifact is the Pantheon-derived `ghcr.io/openclaw/openclaw:2026.7.1` image with pinned Claude Code and Gemini CLIs. Root deploys idempotently register the shared two-Codex/two-Claude/one-Gemini model pool while OAuth remains in the persistent OpenClaw auth store. The bump preserves `/v1/responses` streaming and adds native Codex app-server routing, named OpenAI auth profiles, usage-limit profile rotation, canonical Gemini CLI routing, and stronger provider/session recovery. The runnable Pantheon-side adapter remains under `integrations/openclaw/adapter/`; OC-001 deny-first governance and the broker/paper/canary/live/capital boundaries are unchanged. Validation entrypoints are `scripts/openclaw-smoke-test.sh`, the derived-image build, strict dev deployment, and live adapter/Management AI smoke. |
