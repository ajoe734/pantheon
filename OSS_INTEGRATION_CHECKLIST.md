...
|---|---|---|---|
| `OpenClaw` | upstream repo/runtime | `governed` | Source pin bumped to `openclaw/openclaw` tag `v2026.6.6` / commit `8c802aa683510c7f7503597b54c3021733245e59` (OPENCLAW-GOVERNED-BUMP-2026-6-6, 2026-06-16); runtime artifact is a Pantheon-derived image (`integrations/openclaw/gateway/Dockerfile` = `FROM ghcr.io/openclaw/openclaw:2026.6.6` + Claude Code CLI baked for the claude-cli agent runtime); upstream pin itself is unchanged. Reason: headless device-code OAuth for subscription accounts (`openclaw models auth login --provider openai --device-code`), zero API key required, plus Claude CLI subscription reuse for multi-model persona routing (`anthropic/*` via `claude -p`, see `integrations/openclaw/governance.md §9`). The runnable Pantheon-side adapter lives under `integrations/openclaw/adapter/`, `docker-compose.yml` exposes an `openclaw-gateway` runtime dependency path (profile `openclaw`), and the fail-closed repo-authoritative runtime-adoption scaffold is landed. `bash scripts/openclaw-smoke-test.sh` and `bash scripts/openclaw-gateway-adapter-smoke.sh` rerun against `2026.6.6`; evidence in `integrations/openclaw/evidence_pack.md §7`. This is not broker-session, paper/canary/live, execution-kernel, or capital-binding activation. |
