...
## Task
- Title: TEJ historical backfill and paid-source fallback
- Status: in_progress
- Owner: Codex
- Reviewer: Claude
- Phase: DATASTRAT market-data completion
- Last update: 2026-06-11T01:25:24Z
- Next: Started TEJ paid backfill catalog, connector readiness, planner, and lineage implementation.

## Summary
接 TEJ 作為歷史補洞與付費備援，支援 TQuant 歷史、AMTOP1、ABSR20 詢價後資料回填，並明確避免一開始訂全量分點月費。

TEJ should be used for historical backfill and research-grade supplement,
especially broker/fundamental gaps. It must not replace MOPS official
disclosure truth.

## Dispatch Notes
- Auto-reassigned away from unavailable lane Codex2; owner Codex2 -> Codex.
- Reviewer is Claude.
- Current branch is `task/DATASTRAT-MARKETDATA-TW-TEJ-006`.

## Dependencies
- DATASTRAT-MARKETDATA-FOUNDATION-001: adapter dispatch, storage, health, and scheduler bridge.
- DATASTRAT-MARKETDATA-TW-MOPS-005: official disclosure and fundamental baseline.
- DATASTRAT-MARKETDATA-TW-FINMIND-004: low-cost FinMind layer.
...
- With key, table inventory and one small dataset smoke can run.
...
- Existing client helpers: `fetch_tej_trial_table_catalog`, `fetch_tej_table_metadata`, `fetch_tej_dataset`.
