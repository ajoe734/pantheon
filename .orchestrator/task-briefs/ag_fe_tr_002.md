...
- Owner: Claude
- Reviewer: Codex
- Next: Header fix complete — If-Match/Idempotency-Key wired through candidatePool.ts + CandidateReviewDrawer; X-Request-Id/If-Match/Idempotency-Key wired through tradingRoom.ts + TradeDecisionCard + TradingRoomPage. BFF client tests added (candidatePool.test.ts, tradingRoom.test.ts). All 116 targeted tests pass. Ready for re-review.
