...
- Owner: Claude
- Reviewer: Claude2
- Next: Implementation complete. PR #1720 open with auto-merge. Three gaps closed: (1) openclaw_client.py now auto-wires OpenClawCronGatewayTransport when OPENCLAW_PAPER_ADAPTER_ENABLED=true. (2) PersonaCronRegistrar registers all 4 WORKFLOW_CATALOG workflows as recurring cron jobs (kind:cron, deleteAfterRun:false) per persona. (3) POST /bff/personas now calls _try_register_persona_cron + _try_bootstrap_persona_ooda_packet and surfaces ooda_packet_id + cron_registration_mode in response meta. 14/14 new tests + 14/14 existing cron tests + 45/45 ooda tests pass. Please review commit 6420b861.
