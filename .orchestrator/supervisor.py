    ...
    for event in load_event_queue(config):
        ...
        if active_worker:
            ...
            if record.get("status") != desired_status or record.get("run_id") != active_worker.get("run_id"):
                ...
                sync_dispatched_task_status(config, event, record.get("run_id"))
                changed = True
            continue
        ...
        sync_dispatched_task_status(config, event, worker_run_id)
        changed = True
    return changed
...
def sync_dispatched_task_status(
    ...
    # ai_status.validate_active_status_command_lease rejects any auto worker that
    # mutates canonical status without a lease. This call impersonates the target
    # agent (AI_NAME above), so without the dispatching worker's run_id it lands in
    # the no-ORCH_RUN_ID branch and dies with "status command lease required for
    # auto worker: <agent>" — the dispatched task then never leaves its pre-dispatch
    # status and gets re-dispatched. The caller has just written this run_id onto the
    # queue record, and the worker it names is the one this sync is reporting for, so
    # the lease it grants is the correct one rather than a bypass.
