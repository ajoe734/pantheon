...
def _persist_task_reassignment_locked(
    ...
    *,
    ...
    if (
        ...
        or (expected_status is not None and old_status != expected_status)
    ):
...
def run_once(
    ...
    *,
    ...
    try:
        # Reconcile before the first worker poll.  The regular cycle performs
        # worker I/O in reserved phases, so keep the assignment truth join in
        # the same CAS boundary rather than moving it back under the short
        # core transaction.
        pre_reconcile_changed = bool(
            _safe_phase(
                "reconcile_worker_task_assignments_pre_dispatch_reserved",
                _run_reserved_runtime_phase,
                config,
                "reconcile_worker_task_assignments_pre_dispatch",
                lambda state: reconcile_worker_task_assignments(config, state),
                quiet=quiet,
            )
        )
        ...
        changed = changed or pre_reconcile_changed or pre_poll_changed
        ...
        # Reconcile a newly dispatched run after its first post-dispatch poll,
        # so the runtime summary cannot call an owner/reviewer drift healthy.
        post_reconcile_changed = bool(
            _safe_phase(
                "reconcile_worker_task_assignments_post_dispatch_reserved",
                _run_reserved_runtime_phase,
                config,
                "reconcile_worker_task_assignments_post_dispatch",
                lambda state: reconcile_worker_task_assignments(config, state),
                quiet=quiet,
            )
        )
        changed = post_reconcile_changed or changed
        prune_changed = bool(
            _safe_phase(
