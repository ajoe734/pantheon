    ...
    if advanced:
        ...
        if state is not None:
            clear_task_failure_streak(
                state,
                worker=worker,
                progress_generation=int(now.timestamp()),
...
def clear_task_failure_streak(
    ...
    *,
    ...
    if isinstance(record, dict) and non_terminal_only:
        ...
        allowed_kinds = {"generic_exit", "missing_process", ""}
...
def same_owner_reviewer_retry_allowed(
    ...
    owner = str(task.get("owner") or "").strip()
    if not owner or target_agent != owner:
        return False
    ...
    # 1. Non-terminal failure kind allowlist (fail closed on empty/unclassified kind)
    record = ((state.get("provider_guardrails", {}) or {}).get("task_failure_streaks", {}) or {}).get(
        _failure_streak_key(task_id, provider_id)
    )
    if not isinstance(record, dict):
    ...
    last_kind = str(record.get("last_failure_kind") or "").strip().lower()
    allowed_kinds = {"generic_exit", "missing_process"}
    if not last_kind or last_kind not in allowed_kinds:
        return False
...
def task_progress_generation(config: dict[str, Any], state: dict[str, Any], task: dict[str, Any]) -> int:
    """Compute task progress generation based on governed reviewer events / verified commit progress."""
    task_id = str(task.get("id") or "").strip()
    if not task_id:
        return 0
    
    gen = 0
    # 1. Governed reviewer reopen / handoff timestamps
    for field in ("github_review_bridge", "review_binding"):
        bridge = task.get(field)
        if isinstance(bridge, dict):
            rec_at = str(bridge.get("recorded_at") or "").strip()
            if rec_at:
                dt = _parse_iso_utc(rec_at)
                if dt:
                    gen = max(gen, int(dt.timestamp()))
    
    # 2. Handoffs in state
    handoffs = state.get("handoffs")
    if isinstance(handoffs, list):
        for h in handoffs:
            if isinstance(h, dict) and str(h.get("task_id") or "").strip() == task_id:
                for ts_field in ("created_at", "resolved_at"):
                    ts = str(h.get(ts_field) or "").strip()
                    if ts:
                        dt = _parse_iso_utc(ts)
                        if dt:
                            gen = max(gen, int(dt.timestamp()))

    # 3. Verified commit progress timestamp
    commit_ts = str(task.get("last_commit_progress_at") or task.get("last_work_progress_at") or "").strip()
    if commit_ts:
        dt = _parse_iso_utc(commit_ts)
        if dt:
            gen = max(gen, int(dt.timestamp()))

    # 4. Fallback to delivery / review_file commit timestamp or task last_update if progress evidence present
    if gen == 0:
        delivery = task.get("delivery")
        if isinstance(delivery, dict) and delivery.get("commit"):
            lu = str(task.get("last_update") or "").strip()
            if lu:
                dt = _parse_iso_utc(lu)
                if dt:
                    gen = int(dt.timestamp())
    ...
    for agent_id in agent_ids:
        ...
        for _, _, _, _, task, reason, review_redispatch in candidates[:per_occurrence_limit]:
            ...
            if queue_delivery_event(config, event):
                ...
                provider_id = normalize_agent_id(target_agent)
                streak_record = ((state.get("provider_guardrails", {}) or {}).get("task_failure_streaks", {}) or {}).get(
                    _failure_streak_key(str(task.get(task_id_field) or ""), provider_id)
                )
                if isinstance(streak_record, dict) and int(streak_record.get("count", 0)) > 0:
                    last_prog_gen = int(streak_record.get("last_progress_generation", 0))
                    current_gen = task_progress_generation(config, state, task)
                    if current_gen > last_prog_gen:
                        write_activity_log(
                            config,
                            {
                                ...
                                "owner": str(task.get("owner") or "").strip(),
                                "reviewer": str(task.get("reviewer") or "").strip(),
                                "prior_failure_count": int(streak_record.get("count", 0)),
                                "last_failure_kind": str(streak_record.get("last_failure_kind") or ""),
                                "last_failure_at": streak_record.get("last_failure_at"),
                                "last_progress_generation": last_prog_gen,
                                "current_progress_generation": current_gen,
                                "decision": "redispatch_allowed",
                                "event_key": event["key"],
                                "message": f"Allowed same-owner reviewer retry for task {task.get(task_id_field)} on provider {provider_id} (current gen {current_gen} > last gen {last_prog_gen}).",
                            },
                        )
