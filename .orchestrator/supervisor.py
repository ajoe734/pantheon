...
def l12_recovery_lane_order(
    config: dict[str, Any], task: Mapping[str, Any] | None
) -> list[str]:
    """Return canonical provider lanes declared by one L12 recovery task."""

    if not task_is_l12_recovery_work(task):
        return []
    raw_lanes = (task or {}).get("preferred_lane_order")
...
def plan_task_assignment_pair(
    ...
    *,
    ...
    preferred_lanes = l12_recovery_lane_order(config, task)
...
def reconcile_unavailable_assignments(
    ...
    for task in tasks:
        ...
        preferred_recovery_lanes = (
            l12_recovery_lane_order(config, task) if l12_recovery else []
        )
