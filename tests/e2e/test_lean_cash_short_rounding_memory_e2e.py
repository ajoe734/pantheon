...
def test_cash_value_market_short_rounding_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    ...
    configured = source_client.post(
        ...
        json={
            ...
            "fetch": {
                ...
                "next_watermark": "2026-06-12T20:58:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-058-us-prices",
            "trace_id": "trace-e2e-loop-058-data-fetch",
        },
    )
    ...
    assert row["metadata"]["symbol"] == "TWLO"
    assert row["metadata"]["close"] == 100.0

    signal = _cash_short_rounding_signal(
        row,
    ...
    assert snapshot["paper_state"]["positions"] == [
        {"symbol": "TWLO", "quantity": -10.0, "price": 100.0}
    ]
    ...
    assert fill_event["metadata"]["signal_id"] == "quant-twlo-cash-short-rounding-058"
    assert fill_event["metadata"]["alpha_source"] == "pure_quant_cash_short_rounding"
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        stored_fill,
        sponsor_persona_id="persona-cash-short-rounding-sponsor",
        contributing_persona_ids=["persona-cash-short-rounding-ops"],
        summary=(
            "TWLO fetched close=100.0 produced a SELL/SHORT CASH_VALUE market signal for 1049.0 cash; "
            "LEAN rounded sizing to a -10 share paper fill, recovered adapter feedback, and wrote "
            "the short cash market rounding context into memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-cash-short-rounding-ops",
                "summary": "Short cash market feedback preserved requested cash and rounded negative share fill.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["cash_short_rounding", "paper_fill", "sizing_audit"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_fill["event_id"], stored_pnl["event_id"]],
    )
    writeback_payload["tags"].extend(["cash_short_rounding", "paper_fill", "sizing_audit"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="TWLO cash market short rounding requested cash negative fill",
        tags=["cash_short_rounding", "paper_fill"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert lineage["alpha_context"]["signal_id"] == "quant-twlo-cash-short-rounding-058"
    assert lineage["alpha_context"]["alpha_source"] == "pure_quant_cash_short_rounding"
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-cash-short-rounding-ops",
        query="short requested cash rounded negative shares",
        tags=["sizing_audit"],
        limit=3,
    )
    assert persona_hits
    ...
    assert persona_lineage["strategy_id"] == "strategy-cash-short-rounding"
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-058-us-prices",
        "source_type": "market",
        "provider": "E2E Loop 058 Static US Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_cash_short_rounding_price_daily",
            "feature_targets": ["features/quant_cash_short_rounding_inputs"],
            "schema_hash": "us_cash_short_rounding_price_daily.e2e_loop_058.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-058-twlo",
        "title": "TWLO daily close for E2E loop 058",
        "content_ref": "market://us_cash_short_rounding_price_daily/TWLO/2026-06-12",
        "metadata": {
            "dataset": "us_cash_short_rounding_price_daily",
            "date": "2026-06-12",
            "symbol": "TWLO",
...
def _cash_short_rounding_signal(
    row: dict[str, Any],
    *,
    ...
    return {
        "signal_id": "quant-twlo-cash-short-rounding-058",
        "version": "1.0",
        "strategy_id": "strategy-cash-short-rounding",
        "timestamp": _iso_now(),
        "symbol": "TWLO.US",
        "action": "SELL",
        "direction": "SHORT",
        ...
        "source_worker": "mock-cash-short-rounding-normalizer",
        "metadata": {
            "alpha_source": "pure_quant_cash_short_rounding",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-058",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-058-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-058",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-058",
                "runtime_id": "paper-runtime-058",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-cash-short-rounding",
                "artifact_version": "6.1.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-cash-short-rounding",
                "persona_capital_binding_id": "pcb-paper-cash-short-rounding",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-058-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-cash-short-rounding",
            "target": {
                "registry_id": "artifact-paper-cash-short-rounding",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-cash-short-rounding",
                "artifact_version": "6.1.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-058-runtime",
        }
