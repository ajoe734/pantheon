...
def test_exit_short_cover_profit_feedback_performance_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    loop_id = "064"
    strategy_id = "strategy-exit-short-cover"
    ...
    configured = source_client.post(
        ...
        json={
            "connector": market_connector(
                connector_id="conn-e2e-loop-064-us-prices",
                provider="E2E Loop 064 Static US Prices",
                dataset="us_exit_short_cover_price_daily",
                feature_target="features/quant_exit_short_cover_inputs",
                schema_hash="us_exit_short_cover_price_daily.e2e_loop_064.v1",
            ),
            "fetch": {
                "mode": "static_records",
                "records": [
                    market_record(
                        source_id="src-e2e-loop-064-snow-short",
                        dataset="us_exit_short_cover_price_daily",
                        symbol="SNOW",
                    ...
                    market_record(
                        source_id="src-e2e-loop-064-snow-cover",
                        dataset="us_exit_short_cover_price_daily",
                        symbol="SNOW",
                ...
                "next_watermark": "2026-06-10T21:04:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-064-us-prices",
            "trace_id": "trace-e2e-loop-064-data-fetch",
        },
    )
    ...
    telemetry = CanonicalTelemetryRecorder(
        loop_id=loop_id,
        artifact_id="artifact-paper-exit-short-cover",
        artifact_version="6.7.0",
        plan_id="plan-paper-exit-short-cover",
        persona_capital_binding_id="pcb-paper-exit-short-cover",
        default_strategy_id="paper-runtime-exit-short-cover",
    )
    ...
    assert short_snapshot["paper_state"]["positions"] == [
        {"symbol": "SNOW", "quantity": -6.0, "price": 50.0}
    ]
    ...
    assert [event["metadata"]["signal_id"] for event in fill_events] == [
        "quant-snow-short-064",
        "quant-snow-exit-short-cover-064",
    ]
    ...
    assert [event["metrics"]["fill_price"] for event in fill_events] == [50.0, 46.0]

    short_fill, cover_fill = fill_events
    assert short_fill["metadata"]["alpha_source"] == "pure_quant_short_entry"
    assert short_fill["metadata"]["requested_quantity"] == 6.0
    assert short_fill["metadata"]["market_price"] == 50.0
    assert cover_fill["metadata"]["alpha_source"] == "pure_quant_exit_short_cover"
    ...
    assert cover_fill["metadata"]["market_price"] == 46.0
    ...
    assert pnl_event["metrics"]["pnl"] == 24.0
    ...
    recovered_records = recovered_adapter.query_lineage_records("runtime_binding", "binding-e2e-loop-064")
    ...
    assert recovered_cover_context["fill_quantity"] == 6.0
    assert recovered_cover_context["fill_price"] == 46.0
    assert recovered_cover_context["quantity_type"] == "SHARES"
    assert recovered_cover_context["order_type"] == "MARKET"
    assert recovered_cover_context["market_price"] == 46.0
    ...
    assert recovered_pnl_context["pnl"] == 24.0
    assert recovered_pnl_context["fill_event_count"] == 2
    assert recovered_pnl_context["open_position_count"] == 0

    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        stored_pnl,
        sponsor_persona_id="persona-exit-short-cover-sponsor",
        contributing_persona_ids=["persona-exit-short-cover-ops"],
        summary=(
            "SNOW fetched prices opened 6 short shares at 50.0, then EXIT/SHORT covered them "
            "at 46.0 for 24.0 paper PnL, recovered adapter feedback, and wrote cover evidence "
            "into memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-exit-short-cover-ops",
                "summary": "EXIT/SHORT cover feedback preserved buy-to-cover fill quantity, price, and PnL evidence.",
                "proposal_ids": [signal["signal_id"] for signal in signals],
                "tags": ["exit_short_cover", "paper_fill", "paper_performance"],
            }
        ],
        proposal_ids=[
            signals[0]["signal_id"],
    ...
    writeback_payload["tags"].extend(["exit_short_cover", "paper_fill", "paper_performance"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="SNOW EXIT SHORT cover profit PnL",
        tags=["exit_short_cover", "paper_performance"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert pnl_lineage["order_context"]["pnl"] == 24.0
    assert cover_lineage["alpha_context"]["signal_id"] == "quant-snow-exit-short-cover-064"
    assert cover_lineage["order_context"]["fill_quantity"] == 6.0
    assert cover_lineage["order_context"]["fill_price"] == 46.0

    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-exit-short-cover-ops",
        query="buy to cover short profit",
    ...
    assert persona_pnl_lineage["order_context"]["pnl"] == 24.0


def _exit_short_signals(
    ...
    *,
    ...
    common = {
        ...
        "source_worker": "mock-exit-short-cover-normalizer",
    ...
    return [
        signal_from_market_row(
            ...
            signal_id="quant-snow-short-064",
            action="SELL",
            direction="SHORT",
            quantity=6.0,
            alpha_source="pure_quant_short_entry",
            **common,
        ),
        signal_from_market_row(
            rows_by_date["2026-06-10"],
            signal_id="quant-snow-exit-short-cover-064",
            action="EXIT",
            direction="SHORT",
            quantity=0.0,
            alpha_source="pure_quant_exit_short_cover",
            **common,
        ),
    ]
