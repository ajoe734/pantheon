...
def test_lean_cash_value_limit_order_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-032-us-cash-limit-prices",
            "trace_id": "trace-e2e-loop-032-data-fetch",
        },
    )
    ...
    assert position["symbol"] == "ZS"
    assert position["quantity"] == 10.0
    assert position["price"] == 99.0
    ...
    assert fill_event["metrics"]["fill_price"] == 99.0
    assert fill_event["metadata"]["signal_id"] == "cash-limit-zs-032"
    assert fill_event["metadata"]["alpha_source"] == "quant_cash_limit_entry"
    assert fill_event["metadata"]["order_type"] == "LIMIT"
    assert fill_event["metadata"]["limit_price"] == 99.0
    assert fill_event["metadata"]["quantity_type"] == "CASH_VALUE"
    assert fill_event["metadata"]["requested_quantity"] == 990.0
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        recovered_fills[0],
        sponsor_persona_id="persona-cash-limit-sponsor",
        contributing_persona_ids=["persona-limit-execution"],
        summary=(
            "ZS market data produced a CASH_VALUE LIMIT signal; LEAN sized the order from the limit price, "
            "placed a simulated LimitOrder instead of a market order, recovered the fill through the adapter, "
            "and wrote cash-limit execution context into Learn memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-limit-execution",
                "summary": "Cash-value limit feedback preserved limit_price, requested cash, and limit-order action.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["cash_value_limit", "limit_order", "paper_fill"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_fill["event_id"]],
    )
    writeback_payload["tags"].extend(["cash_value_limit", "limit_order", "paper_fill"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="ZS cash value limit order limit price",
        tags=["cash_value_limit", "limit_order"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert alpha_context["signal_id"] == "cash-limit-zs-032"
    assert alpha_context["alpha_source"] == "quant_cash_limit_entry"
    assert alpha_context["market_price"] == 100.0
    assert alpha_context["market_data_ref"] == normalized_ref["uri"]
    assert order_context["order_type"] == "LIMIT"
    assert order_context["limit_price"] == 99.0
    assert order_context["quantity_type"] == "CASH_VALUE"
    assert order_context["requested_quantity"] == 990.0
    assert order_context["fill_quantity"] == 10.0
    assert order_context["fill_price"] == 99.0
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-limit-execution",
        query="cash value limit price action",
        tags=["paper_fill"],
        limit=3,
    )
    assert persona_hits
    ...
    assert persona_order_context["limit_price"] == 99.0
    assert persona_order_context["fill_price"] == 99.0
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-032-us-cash-limit-prices",
        "source_type": "market",
        "provider": "E2E Loop 032 Static Cash Limit Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_cash_limit_price_daily",
            "feature_targets": ["features/quant_cash_limit_inputs"],
            "schema_hash": "us_cash_limit_price_daily.e2e_loop_032.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-032-zs",
        "title": "ZS daily close for E2E loop 032",
        "content_ref": "market://us_cash_limit_price_daily/ZS/2026-06-12",
        "metadata": {
            "dataset": "us_cash_limit_price_daily",
            "date": "2026-06-12",
            "symbol": "ZS",
...
def _cash_limit_signal(
    ...
    *,
    ...
    return {
        "signal_id": "cash-limit-zs-032",
        "version": "1.0",
        "strategy_id": "strategy-quant-cash-limit",
        "timestamp": _iso_now(),
        "symbol": "ZS.US",
        "action": "BUY",
        "direction": "LONG",
        ...
        "source_worker": "mock-cash-limit-normalizer",
        "metadata": {
            "alpha_source": "quant_cash_limit_entry",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-032",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-032-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-032",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-032",
                "runtime_id": "paper-runtime-032",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-cash-limit",
                "artifact_version": "32.0.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-cash-limit",
                "persona_capital_binding_id": "pcb-paper-cash-limit",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-032-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-cash-limit",
            "target": {
                "registry_id": "artifact-paper-cash-limit",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-cash-limit",
                "artifact_version": "32.0.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-032-runtime",
        }
