...
def test_lean_exit_without_position_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    ...
    configured = source_client.post(
        ...
        json={
            ...
            "fetch": {
                ...
                "next_watermark": "2026-06-12T23:55:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-021-us-exit-prices",
            "trace_id": "trace-e2e-loop-021-data-fetch",
        },
    )
    ...
    assert row["metadata"]["symbol"] == "ADBE"
    assert row["metadata"]["close"] == 600.0
    ...
    assert noop_event["metadata"]["signal_id"] == "quant-exit-adbe-empty-021"
    assert noop_event["metadata"]["alpha_source"] == "quant_drawdown_exit"
    ...
    assert noop_event["metadata"]["price"] == 600.0
    assert noop_event["metadata"]["market_price"] == 600.0
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        recovered_noops[0],
        sponsor_persona_id="persona-exit-sponsor",
        contributing_persona_ids=["persona-quant-exit"],
        summary=(
            "ADBE market data produced a quantitative EXIT signal, but LEAN paper runtime found no "
            "long position to close; it emitted recoverable no-order feedback, recovered it through "
            "the adapter, and wrote the position context into Learn memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-quant-exit",
                "summary": "EXIT no-position feedback preserved computed quantity and zero position context.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["exit_no_position", "paper_noop", "paper_performance"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_noop["event_id"]],
    )
    writeback_payload["tags"].extend(["exit_no_position", "paper_noop", "paper_performance"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="ADBE EXIT no position paper noop",
        tags=["exit_no_position", "paper_noop"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert alpha_context["signal_id"] == "quant-exit-adbe-empty-021"
    assert alpha_context["alpha_source"] == "quant_drawdown_exit"
    assert alpha_context["market_price"] == 600.0
    assert alpha_context["market_data_ref"] == normalized_ref["uri"]
    assert order_context["noop_reason"] == "exit_long_without_position"
    ...
    assert order_context["exit_direction"] == "LONG"
    assert order_context["price"] == 600.0
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-quant-exit",
        query="computed quantity zero position",
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-021-us-exit-prices",
        "source_type": "market",
        "provider": "E2E Loop 021 Static US Exit Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_exit_no_position_price_daily",
            "feature_targets": ["features/quant_exit_empty_inputs"],
            "schema_hash": "us_exit_no_position_price_daily.e2e_loop_021.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-021-adbe",
        "title": "ADBE daily close for E2E loop 021",
        "content_ref": "market://us_exit_no_position_price_daily/ADBE/2026-06-12",
        "metadata": {
            "dataset": "us_exit_no_position_price_daily",
            "date": "2026-06-12",
            "symbol": "ADBE",
...
def _exit_signal(
    ...
    *,
    ...
    return {
        ...
        "version": "1.0",
        "strategy_id": "strategy-quant-exit-no-position",
        "timestamp": _iso_now(),
        "symbol": "ADBE.US",
        "action": "EXIT",
        "direction": "LONG",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-021",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-021-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-021",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-021",
                "runtime_id": "paper-runtime-021",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-exit-empty",
                "artifact_version": "21.0.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-exit-empty",
                "persona_capital_binding_id": "pcb-paper-exit-empty",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-021-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-exit-empty",
            "target": {
                "registry_id": "artifact-paper-exit-empty",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-exit-empty",
                "artifact_version": "21.0.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-021-runtime",
        }
