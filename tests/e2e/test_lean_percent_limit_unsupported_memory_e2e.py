...
def test_lean_percent_portfolio_limit_unsupported_feedback_memory_readback_e2e(
    tmp_path,
    monkeypatch,
) -> None:
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-033-us-percent-limit-prices",
            "trace_id": "trace-e2e-loop-033-data-fetch",
        },
    )
    ...
    assert row["metadata"]["close"] == 26.0

    signal = _percent_limit_signal(
        row,
    ...
    assert noop_event["metadata"]["signal_id"] == "percent-limit-pltr-033"
    assert noop_event["metadata"]["alpha_source"] == "quant_percent_limit_guard"
    ...
    assert noop_event["metadata"]["limit_price"] == 25.0
    ...
    assert noop_event["metadata"]["price"] == 26.0
    assert noop_event["metadata"]["market_price"] == 26.0
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        recovered_noops[0],
        sponsor_persona_id="persona-percent-limit-sponsor",
        contributing_persona_ids=["persona-percent-limit-guard"],
        summary=(
            "PLTR market data produced a PERCENT_PORTFOLIO LIMIT signal; LEAN rejected the unsupported "
            "SetHoldings limit-order combination before broker submission, recovered feedback through the "
            "adapter, and wrote the unsupported-order context into Learn memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-percent-limit-guard",
                "summary": "Percent-portfolio limit feedback preserved limit_price and broker non-submission.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["percent_limit_unsupported", "execution_error", "paper_noop"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_noop["event_id"]],
    )
    writeback_payload["tags"].extend(["percent_limit_unsupported", "execution_error", "paper_noop"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="PLTR percent portfolio limit unsupported no order",
        tags=["percent_limit_unsupported", "execution_error"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert alpha_context["signal_id"] == "percent-limit-pltr-033"
    assert alpha_context["alpha_source"] == "quant_percent_limit_guard"
    assert alpha_context["market_price"] == 26.0
    assert alpha_context["market_data_ref"] == normalized_ref["uri"]
    assert order_context["noop_reason"] == "limit_percent_portfolio_unsupported"
    assert order_context["filter_reason"] == "limit_percent_portfolio_unsupported"
    ...
    assert "PERCENT_PORTFOLIO quantity_type" in order_context["execution_error_message"]
    ...
    assert order_context["limit_price"] == 25.0
    assert order_context["quantity_type"] == "PERCENT_PORTFOLIO"
    assert order_context["requested_quantity"] == 0.25
    assert order_context["computed_quantity"] == 0.0
    assert order_context["price"] == 26.0
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-percent-limit-guard",
        query="percent limit unsupported broker non submission",
        tags=["paper_noop"],
        limit=3,
    )
    assert persona_hits
    ...
    assert persona_order_context["filter_reason"] == "limit_percent_portfolio_unsupported"
    assert persona_order_context["order_type"] == "LIMIT"
    assert persona_order_context["limit_price"] == 25.0
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-033-us-percent-limit-prices",
        "source_type": "market",
        "provider": "E2E Loop 033 Static Percent Limit Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_percent_limit_price_daily",
            "feature_targets": ["features/quant_percent_limit_inputs"],
            "schema_hash": "us_percent_limit_price_daily.e2e_loop_033.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-033-pltr",
        "title": "PLTR daily close for E2E loop 033",
        "content_ref": "market://us_percent_limit_price_daily/PLTR/2026-06-12",
        "metadata": {
            "dataset": "us_percent_limit_price_daily",
            "date": "2026-06-12",
            "symbol": "PLTR",
...
def _percent_limit_signal(
    ...
    *,
    ...
    return {
        "signal_id": "percent-limit-pltr-033",
        "version": "1.0",
        "strategy_id": "strategy-quant-percent-limit",
        "timestamp": _iso_now(),
        "symbol": "PLTR.US",
        "action": "BUY",
        "direction": "LONG",
        ...
        "source_worker": "mock-percent-limit-normalizer",
        "metadata": {
            "alpha_source": "quant_percent_limit_guard",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-033",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-033-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-033",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-033",
                "runtime_id": "paper-runtime-033",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-percent-limit",
                "artifact_version": "33.0.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-percent-limit",
                "persona_capital_binding_id": "pcb-paper-percent-limit",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-033-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-percent-limit",
            "target": {
                "registry_id": "artifact-paper-percent-limit",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-percent-limit",
                "artifact_version": "33.0.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-033-runtime",
        }
