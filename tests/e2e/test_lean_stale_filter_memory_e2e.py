...
def test_lean_stale_signal_filter_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    ...
    configured = source_client.post(
        ...
        json={
            ...
            "fetch": {
                ...
                "next_watermark": "2026-06-12T23:59:30Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-025-us-stale-prices",
            "trace_id": "trace-e2e-loop-025-data-fetch",
        },
    )
    ...
    assert row["metadata"]["symbol"] == "ZM"
    assert row["metadata"]["close"] == 68.0
    ...
    assert noop_event["metadata"]["signal_id"] == "stale-filter-zm-025"
    assert noop_event["metadata"]["alpha_source"] == "quant_stale_breakout"
    ...
    assert noop_event["metadata"]["price"] == 68.0
    assert noop_event["metadata"]["market_price"] == 68.0
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        recovered_noops[0],
        sponsor_persona_id="persona-stale-filter-sponsor",
        contributing_persona_ids=["persona-quant-staleness"],
        summary=(
            "ZM market data produced an old breakout signal; the signal consumer filtered it as stale, "
            "emitted no-order feedback instead of silently discarding it, recovered the feedback after "
            "adapter restart, and wrote stale-filter context into Learn memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-quant-staleness",
                "summary": "Stale signal feedback preserved filter reason, requested quantity, and broker non-submission.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["stale_filter", "paper_noop", "paper_performance"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_noop["event_id"]],
    )
    writeback_payload["tags"].extend(["stale_filter", "paper_noop", "paper_performance"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="ZM stale signal filtered no order",
        tags=["stale_filter", "paper_noop"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert alpha_context["signal_id"] == "stale-filter-zm-025"
    assert alpha_context["alpha_source"] == "quant_stale_breakout"
    assert alpha_context["market_price"] == 68.0
    assert alpha_context["market_data_ref"] == normalized_ref["uri"]
    assert order_context["noop_reason"] == "stale_signal"
    assert order_context["filter_reason"] == "stale_signal"
    ...
    assert order_context["price"] == 68.0
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-quant-staleness",
        query="filter reason stale signal",
    ...
    assert persona_order_context["filter_reason"] == "stale_signal"
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-025-us-stale-prices",
        "source_type": "market",
        "provider": "E2E Loop 025 Static US Stale Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_stale_filter_price_daily",
            "feature_targets": ["features/quant_stale_filter_inputs"],
            "schema_hash": "us_stale_filter_price_daily.e2e_loop_025.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-025-zm",
        "title": "ZM daily close for E2E loop 025",
        "content_ref": "market://us_stale_filter_price_daily/ZM/2026-06-12",
        "metadata": {
            "dataset": "us_stale_filter_price_daily",
            "date": "2026-06-12",
            "symbol": "ZM",
...
def _stale_signal(
    ...
    *,
    ...
    return {
        "signal_id": "stale-filter-zm-025",
        "version": "1.0",
        "strategy_id": "strategy-quant-stale-filter",
        "timestamp": (datetime.now(timezone.utc) - timedelta(hours=30)).replace(microsecond=0).isoformat(),
        "symbol": "ZM.US",
        "action": "BUY",
        "direction": "LONG",
        "quantity": 0.25,
        "quantity_type": "PERCENT_PORTFOLIO",
        "source_worker": "mock-stale-filter-normalizer",
        "metadata": {
            "alpha_source": "quant_stale_breakout",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-025",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-025-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-025",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-025",
                "runtime_id": "paper-runtime-025",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-stale-filter",
                "artifact_version": "25.0.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-stale-filter",
                "persona_capital_binding_id": "pcb-paper-stale-filter",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-025-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-stale-filter",
            "target": {
                "registry_id": "artifact-paper-stale-filter",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-stale-filter",
                "artifact_version": "25.0.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-025-runtime",
        }
