...
def test_fractional_shares_market_rounding_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    ...
    configured = source_client.post(
        ...
        json={
            ...
            "fetch": {
                ...
                "next_watermark": "2026-06-12T20:59:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-059-us-prices",
            "trace_id": "trace-e2e-loop-059-data-fetch",
        },
    )
    ...
    signal = _shares_rounding_signal(
        row,
    ...
    assert snapshot["paper_state"]["positions"] == [
        {"symbol": "TEAM", "quantity": 11.0, "price": 20.0}
    ]
    ...
    assert fill_event["metadata"]["signal_id"] == "quant-team-shares-rounding-059"
    assert fill_event["metadata"]["alpha_source"] == "pure_quant_shares_rounding"
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        stored_fill,
        sponsor_persona_id="persona-shares-rounding-sponsor",
        contributing_persona_ids=["persona-shares-rounding-ops"],
        summary=(
            "TEAM fetched close=20.0 produced a BUY/LONG SHARES market signal for 10.6 shares; "
            "LEAN rounded sizing to an 11 share paper fill, recovered adapter feedback, and wrote "
            "the fractional share rounding context into memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-shares-rounding-ops",
                "summary": "Shares market feedback preserved requested fractional quantity and rounded fill.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["shares_rounding", "paper_fill", "sizing_audit"],
            }
        ],
        proposal_ids=[signal["signal_id"], stored_fill["event_id"], stored_pnl["event_id"]],
    )
    writeback_payload["tags"].extend(["shares_rounding", "paper_fill", "sizing_audit"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="TEAM fractional shares rounding requested fill",
        tags=["shares_rounding", "paper_fill"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert lineage["alpha_context"]["signal_id"] == "quant-team-shares-rounding-059"
    assert lineage["alpha_context"]["alpha_source"] == "pure_quant_shares_rounding"
    ...
    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-shares-rounding-ops",
        query="requested fractional rounded fill",
        tags=["sizing_audit"],
        limit=3,
    )
    assert persona_hits
    ...
    assert persona_lineage["strategy_id"] == "strategy-shares-rounding"
...
def _market_connector() -> dict[str, Any]:
    return {
        "connector_id": "conn-e2e-loop-059-us-prices",
        "source_type": "market",
        "provider": "E2E Loop 059 Static US Prices",
        "license_scope": "internal",
        "metadata": {
            "dataset": "us_shares_rounding_price_daily",
            "feature_targets": ["features/quant_shares_rounding_inputs"],
            "schema_hash": "us_shares_rounding_price_daily.e2e_loop_059.v1",
        },
    }
...
def _market_record() -> dict[str, Any]:
    return {
        "source_id": "src-e2e-loop-059-team",
        "title": "TEAM daily close for E2E loop 059",
        "content_ref": "market://us_shares_rounding_price_daily/TEAM/2026-06-12",
        "metadata": {
            "dataset": "us_shares_rounding_price_daily",
            "date": "2026-06-12",
            "symbol": "TEAM",
...
def _shares_rounding_signal(
    ...
    *,
    ...
    return {
        "signal_id": "quant-team-shares-rounding-059",
        "version": "1.0",
        "strategy_id": "strategy-shares-rounding",
        "timestamp": _iso_now(),
        "symbol": "TEAM.US",
        "action": "BUY",
        "direction": "LONG",
        ...
        "source_worker": "mock-shares-rounding-normalizer",
        "metadata": {
            "alpha_source": "pure_quant_shares_rounding",
    ...
    return RuntimeIdentity.from_env(
        {
            ...
            "PANTHEON_RUNTIME_ID": "paper-runtime-059",
            "PANTHEON_RUNTIME_MANAGER_URL": "http://runtime-manager:8081",
            "PANTHEON_RUNTIME_MANAGER_TOKEN": "runtime-control-internal",
            "PANTHEON_TRACE_ID": "trace-e2e-loop-059-runtime",
            "PANTHEON_REQUEST_ID": "request-e2e-loop-059",
        }
    )
...
class _RuntimeManagerClient:
    def list_all(self) -> list[dict[str, Any]]:
        return [
            {
                "binding_id": "binding-e2e-loop-059",
                "runtime_id": "paper-runtime-059",
                "capital_pool_id": "pool-paper",
                "artifact_id": "artifact-paper-shares-rounding",
                "artifact_version": "6.2.0",
                "deployment_mode": "paper",
                "deployment_stage": "paper",
                "plan_id": "plan-paper-shares-rounding",
                "persona_capital_binding_id": "pcb-paper-shares-rounding",
                "status": "active",
            }
        ]
...
class _CanonicalTelemetryRecorder:
    ...
    def emit(self, event_type: str, metrics: dict[str, Any], metadata: dict[str, Any] | None = None) -> bool:
        ...
        event = {
            "event_id": f"e2e-loop-059-{event_type}-{index:03d}",
            ...
            "persona_capital_binding_id": "pcb-paper-shares-rounding",
            "target": {
                "registry_id": "artifact-paper-shares-rounding",
                "strategy_id": metadata.get("strategy_id") or "paper-runtime-shares-rounding",
                "artifact_version": "6.2.0",
            ...
            "metadata": metadata,
            "trace_id": "trace-e2e-loop-059-runtime",
        }
