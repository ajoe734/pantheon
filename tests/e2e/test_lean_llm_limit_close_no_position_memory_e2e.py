...
LLM_REFS = {
    "model_id": "gpt-alpha-e2e-097",
    "prompt_bundle_id": "prompt-bundle-e2e-097",
    "llm_prompt_id": "prompt-e2e-097",
    "llm_response_id": "response-e2e-097",
    "llm_decision_id": "decision-e2e-097",
    "research_note_ref": "memory://research/e2e-097/asnd-limit-close-empty",
    "llm_note_ref": "memory://llm/e2e-097/asnd-limit-close-empty",
    "research_data_ref": ["research://asnd/e2e-097/risk-reduction"],
}


def test_llm_sell_long_limit_close_without_position_feedback_memory_readback_e2e(
    tmp_path,
    monkeypatch,
) -> None:
    loop_id = "097"
    strategy_id = "strategy-llm-limit-close-no-position"
    ...
    configured = source_client.post(
        ...
        json={
            "connector": market_connector(
                connector_id="conn-e2e-loop-097-us-prices",
                provider="E2E Loop 097 Static US Prices",
                dataset="us_llm_limit_close_empty_price_daily",
                feature_target="features/llm_limit_close_empty_inputs",
                schema_hash="us_llm_limit_close_empty_price_daily.e2e_loop_097.v1",
            ),
            "fetch": {
                "mode": "static_records",
                "records": [
                    market_record(
                        source_id="src-e2e-loop-097-asnd",
                        dataset="us_llm_limit_close_empty_price_daily",
                        symbol="ASND",
                ...
                "next_watermark": "2026-06-10T21:55:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-097-us-prices",
            "trace_id": "trace-e2e-loop-097-data-fetch",
        },
    )
    ...
    assert row["metadata"]["symbol"] == "ASND"
    assert row["metadata"]["close"] == 70.0
    ...
    telemetry = CanonicalTelemetryRecorder(
        loop_id=loop_id,
        artifact_id="artifact-paper-llm-limit-close-empty",
        artifact_version="9.9.0",
        plan_id="plan-paper-llm-limit-close-empty",
        persona_capital_binding_id="pcb-paper-llm-limit-close-empty",
        default_strategy_id="paper-runtime-llm-limit-close-empty",
    )
    ...
    assert noop_event["metrics"]["requested_quantity"] == 4.0
    ...
    assert noop_event["metadata"]["signal_id"] == "llm-asnd-limit-close-empty-097"
    assert noop_event["metadata"]["alpha_source"] == "llm_limit_close_no_position"
    assert noop_event["metadata"]["model_id"] == "gpt-alpha-e2e-097"
    ...
    assert noop_event["metadata"]["limit_price"] == 71.0
    assert noop_event["metadata"]["position_quantity"] == 0.0
    assert noop_event["metadata"]["market_price"] == 70.0
    ...
    recovered_records = recovered_adapter.query_lineage_records("runtime_binding", "binding-e2e-loop-097")
    records_by_id = {record["record_id"]: record for record in recovered_records}
    recovered_noop = records_by_id[stored_noop["event_id"]]
    assert recovered_noop["alpha_context"]["model_id"] == "gpt-alpha-e2e-097"
    assert recovered_noop["alpha_context"]["llm_response_id"] == "response-e2e-097"
    assert recovered_noop["order_context"]["noop_reason"] == "sell_long_limit_without_position"
    assert recovered_noop["order_context"]["limit_price"] == 71.0
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        stored_noop,
        sponsor_persona_id="persona-llm-limit-close-empty-sponsor",
        contributing_persona_ids=["persona-llm-limit-close-empty-ops"],
        summary=(
            "ASND LLM limit close fetched market data and requested a SELL/LONG limit close at 71.0, "
            "but LEAN found no long position, emitted no-order feedback, recovered adapter lineage, "
            "and wrote the zero-fill performance result into memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-llm-limit-close-empty-ops",
                "summary": "LLM limit close no-position feedback preserved prompt lineage, no broker submit, and zero fill-rate.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["llm_limit_close_no_position", "paper_noop", "paper_performance"],
            }
        ],
    ...
    writeback_payload["tags"].extend(["llm_limit_close_no_position", "paper_noop", "paper_performance"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="ASND LLM limit close no position zero fill",
        tags=["llm_limit_close_no_position", "paper_performance"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert noop_lineage["alpha_context"]["signal_id"] == "llm-asnd-limit-close-empty-097"
    assert noop_lineage["alpha_context"]["llm_decision_id"] == "decision-e2e-097"
    assert noop_lineage["order_context"]["noop_reason"] == "sell_long_limit_without_position"
    assert noop_lineage["order_context"]["computed_quantity"] == 0.0
    assert pnl_lineage["order_context"]["fill_rate"] == 0.0

    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-llm-limit-close-empty-ops",
        query="limit close no broker submit zero fill",
        tags=["paper_noop"],
        limit=3,
    )
    assert persona_hits
...
def _limit_close_without_position_signal(
    ...
    *,
    ...
    return signal_from_market_row(
        row,
        signal_id="llm-asnd-limit-close-empty-097",
        strategy_id=strategy_id,
        symbol="ASND.US",
        action="SELL",
        direction="LONG",
        ...
        source_worker="mock-llm-limit-close-empty-normalizer",
        alpha_source="llm_limit_close_no_position",
