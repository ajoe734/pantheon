...
LLM_REFS = {
    "model_id": "gpt-alpha-e2e-083",
    "prompt_bundle_id": "prompt-bundle-e2e-083",
    "llm_prompt_id": "prompt-e2e-083",
    "llm_response_id": "response-e2e-083",
    "llm_decision_id": "decision-e2e-083",
    "research_note_ref": "memory://research/e2e-083/team-exit-note",
    "llm_note_ref": "memory://llm/e2e-083/team-risk-exit",
    "research_data_ref": ["research://team/e2e-083/usage-slowdown"],
}


def test_llm_exit_long_without_position_feedback_memory_readback_e2e(tmp_path, monkeypatch) -> None:
    loop_id = "083"
    strategy_id = "strategy-llm-exit-long-no-position"
    ...
    configured = source_client.post(
        ...
        json={
            "connector": market_connector(
                connector_id="conn-e2e-loop-083-us-prices",
                provider="E2E Loop 083 Static US Prices",
                dataset="us_llm_exit_long_empty_price_daily",
                feature_target="features/llm_exit_long_empty_inputs",
                schema_hash="us_llm_exit_long_empty_price_daily.e2e_loop_083.v1",
            ),
            "fetch": {
                "mode": "static_records",
                "records": [
                    market_record(
                        source_id="src-e2e-loop-083-team",
                        dataset="us_llm_exit_long_empty_price_daily",
                        symbol="TEAM",
                ...
                "next_watermark": "2026-06-10T21:15:00Z",
            },
        },
    )
    ...
    ingest = source_client.post(
        ...
        json={
            "connector_id": "conn-e2e-loop-083-us-prices",
            "trace_id": "trace-e2e-loop-083-data-fetch",
        },
    )
    ...
    assert row["metadata"]["close"] == 143.0

    signal = signal_from_market_row(
        row,
        signal_id="llm-team-exit-long-empty-083",
        ...
        source_worker="mock-llm-exit-long-normalizer",
        alpha_source="llm_research_exit_long_no_position",
    ...
    telemetry = CanonicalTelemetryRecorder(
        loop_id=loop_id,
        artifact_id="artifact-paper-llm-exit-long-empty",
        artifact_version="8.5.0",
        plan_id="plan-paper-llm-exit-long-empty",
        persona_capital_binding_id="pcb-paper-llm-exit-long-empty",
        default_strategy_id="paper-runtime-llm-exit-long-empty",
    )
    ...
    assert noop_event["metrics"]["action"] == "exit_long_without_position_noop"
    ...
    assert noop_event["metadata"]["signal_id"] == "llm-team-exit-long-empty-083"
    assert noop_event["metadata"]["alpha_source"] == "llm_research_exit_long_no_position"
    assert noop_event["metadata"]["model_id"] == "gpt-alpha-e2e-083"
    assert noop_event["metadata"]["llm_decision_id"] == "decision-e2e-083"
    assert noop_event["metadata"]["noop_reason"] == "exit_long_without_position"
    ...
    assert noop_event["metadata"]["market_price"] == 143.0
    ...
    recovered_records = recovered_adapter.query_lineage_records("runtime_binding", "binding-e2e-loop-083")
    records_by_id = {record["record_id"]: record for record in recovered_records}
    recovered_noop = records_by_id[stored_noop["event_id"]]
    assert recovered_noop["alpha_context"]["model_id"] == "gpt-alpha-e2e-083"
    assert recovered_noop["alpha_context"]["llm_response_id"] == "response-e2e-083"
    assert recovered_noop["order_context"]["noop_reason"] == "exit_long_without_position"
    assert recovered_noop["order_context"]["decision_status"] == "no_order"
    ...
    writeback_payload = recovered_adapter.build_learn_feedback_writeback_payload(
        stored_noop,
        sponsor_persona_id="persona-llm-exit-long-empty-sponsor",
        contributing_persona_ids=["persona-llm-exit-long-empty-ops"],
        summary=(
            "TEAM LLM exit signal consumed fetched market data, but LEAN found no long position; "
            "it emitted recoverable no-order feedback, preserved LLM lineage through adapter recovery, "
            "and wrote zero-position evidence into memory."
        ),
        contributor_feedback=[
            {
                "persona_id": "persona-llm-exit-long-empty-ops",
                "summary": "LLM EXIT/LONG no-position feedback preserved decision, no-order status, and zero position context.",
                "proposal_ids": [signal["signal_id"]],
                "tags": ["llm_exit_long_no_position", "paper_noop", "adapter_recovery"],
            }
        ],
    ...
    writeback_payload["tags"].extend(["llm_exit_long_no_position", "paper_noop", "adapter_recovery"])
    ...
    institutional_hits = InstitutionalMemoryStore(path=institutional_path).retrieve(
        query="TEAM LLM exit long no position no order",
        tags=["llm_exit_long_no_position", "paper_noop"],
        scope="system_wide",
        limit=3,
    )
    ...
    assert noop_lineage["alpha_context"]["llm_decision_id"] == "decision-e2e-083"
    assert noop_lineage["order_context"]["noop_reason"] == "exit_long_without_position"
    assert noop_lineage["order_context"]["position_quantity"] == 0.0
    assert pnl_lineage["order_context"]["fill_event_count"] == 0

    persona_hits = PersonaMemoryStore(path=persona_path).retrieve(
        persona_id="persona-llm-exit-long-empty-ops",
        query="llm exit long zero position",
