# AG-FE-DB-002 Sidecar Acceptance Follow-up 19

| Field | Value |
|---|---|
| Task ID | `AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-19` |
...
| Reviewer | `Codex2` |
| Date | `2026-06-21` |
| Baseline | follow-up 18 merged to `dev` at `66aa9d24` (PR #2013) |
| Current dev | `d69ba6f9` (PR #2033) |
...
`AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-18` was reviewed, finalized, and
archived `done`.

The parent task remains active `blocked`. Current status now records parent
owner `Codex2`, reviewer `Codex`, and `waiting_for` `Codex`. That differs from
older DB002 packets that described parent owner/reviewer as `Codex`/`Claude` or
parent waiting on `Claude`; this packet uses current `ai-status` as the state
truth.

This sidecar does not unblock, reopen, implement, or close the parent. It does
not change runtime, registry, schema, OpenAPI, BFF, governance, broker,
RuntimeBinding, or canonical L1/L2 truth surfaces.

## Reviewed Evidence Chain

All prior DB002 support packets through follow-up 18 are durable. Follow-up 18
...
Follow-up 18 archived status records PR #2013 merged into `dev`, all three CI
checks passing, no canonical truth mutation, and the parent still blocked at
that time. This follow-up starts from the merged follow-up 18 checkpoint instead
of reopening earlier packets.

## Current Dev Delta Since Follow-up 18 Closeout

Follow-up 18 merged to `dev` at `66aa9d24`. Current `origin/dev` during this
packet is `d69ba6f9`.

`git log --first-parent --oneline 66aa9d24..origin/dev` shows later merges in
these areas:

| Area | Merged work since follow-up 18 | DB002 consequence |
|---|---|---|
| BFF identity / servant sessions | PR #2014 support closeout, PR #2025 and PR #2029 for `AG-BE-ID-003` servant session facade and closeout. | Touches `/bff/agora/servant` implementation and tests, not dashboard editor, widget renderer, dashboard helper, or layout route behavior. |
| Strategy workshop and strategy completeness | PR #2018 `AG-BE-SW-003`, PR #2020/#2024 `AG-BE-SW-002` packets, PR #2021/#2023/#2027 `AG-BE-SW-003` sidecar/review material, PR #2028/#2031/#2032/#2033 `AG-BE-SW-002` follow-up material. | Adds strategy completeness and support packets for separate SW surfaces. No DB002 runtime completion or dashboard editor file appears. |
| Agora v1.2 contract extension | PR #2017 `AG-XR-OPENAPI-003` plus PR #2022 sidecar review. | Contract path changed, but the change adds optional `session_type` to `ServantSessionCreateRequest`; it does not alter dashboard recipe layout PATCH, layout operation names, or widget schemas. |
| Frontend identity support | PR #2019 `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-29`. | Support packet and review material only; no DB002 dashboard/editor/widget path touched. |
| Ops / release / design records | PR #2026 open design gaps archive and PR #2030 current-run race-id release gate. | No dashboard editor or DB002 BFF helper consequence. |

`git diff --name-only 66aa9d24 origin/dev -- execute-plans/src/agora/dashboard
execute-plans/src/agora/widgets execute-plans/src/lib/bff-v1/agora
services/control-plane/openapi services/control-plane/specs/agora` returned:
...
Inspection of those diffs shows only the `AG-XR-OPENAPI-003` servant-session
extension:

- `ServantSessionCreateRequest.session_type` added with enum
  `interactive | trainer | research_task`, default `interactive`.
- `agora.servant.v1` capability manifest records the session-type contract.
- `bundle_index.v1_2.json` hashes changed for the OpenAPI and capability
  manifest files.
...
| `AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-19` | Active `in_progress`; owner `Codex`; reviewer `Codex2`; artifact is this packet. | Owner should commit and hand this packet to `Codex2` for review. Parent status remains unchanged. |
| `AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-18` | Archived `done`; PR #2013 merged; closeout confirmed. | Follow-up 19 starts from finalized follow-up 18 evidence. |
...
| `AG-XR-OPENAPI-003` | Merged through PR #2017. | Adds servant-session `session_type`; does not change DB002 dashboard layout semantics. |
...
follow-up 18 already answers both points. Follow-up 19 adds current-dev delta
confirmation and calls out the only contract-path change since follow-up 18:
`AG-XR-OPENAPI-003` touched v1.2 OpenAPI/manifest files, but only for servant
...
| v1.2 contract files changed after follow-up 18 | The new change is servant-session `session_type` from `AG-XR-OPENAPI-003`. It does not alter dashboard layout PATCH semantics, operation names, or generated `patchDashboardRecipeLayout` route inventory. |
| Dev advanced after follow-up 18 | Later merges are servant identity, strategy workshop/completeness, sidecar/review material, and ops/design records. No newer merge adds `DashboardGridEditor`, changes the dashboard BFF helper, or changes DB002 dashboard layout behavior. |

Recommended parent path:

1. `Codex`, as the current parent reviewer and `waiting_for`, explicitly
   acknowledges reviewed sidecar evidence through follow-up 19 or records a new
   concrete parent blocker.
2. If acknowledged, `Codex` reopens the parent for owner implementation:

   ```bash
   AI_NAME=Codex ./scripts/ai-status.sh reopen AG-FE-DB-002 "Reviewed DB002 sidecar waiver and current-dev evidence accepted through followup-19; return parent to Codex2 for DashboardGridEditor implementation."
   ```
...
| v1.2 layout contract | `services/control-plane/openapi/agora_v1_2.openapi.yaml` | Confirms `PATCH /bff/agora/dashboard-recipes/{recipe_id}/layout`, `If-Match`, `Idempotency-Key`, `expected_version`, and the six layout operations. The post-followup-18 session-type change does not alter this route. |
...
graph TD
    ...
    OPENAPI002 --> OPENAPI003["AG-XR-OPENAPI-003 merged<br/>servant session_type only"]
    ...
    P19["FOLLOWUP-19 this packet<br/>post-followup-18 delta + session_type note"] -. review handoff .-> DB002
...
- `AG-XR-OPENAPI-003` changes the v1.2 contract bundle, but only for servant
  session create request shape. It is not a new dashboard layout blocker.
...
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-19
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-18
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-002
AI_NAME=Codex python3 scripts/ai_status.py show AG-XR-OPENAPI-002
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-001
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-003
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-DB-004
AI_NAME=Codex python3 scripts/ai_status.py show AG-BE-DB-001
AI_NAME=Codex python3 scripts/ai_status.py show AG-FE-TR-002
AI_NAME=Codex python3 scripts/ai_status.py show AG-E2E-TR-001
git log --first-parent --oneline 66aa9d24..origin/dev
git diff --name-only 66aa9d24 origin/dev
git diff --name-only 66aa9d24 origin/dev -- execute-plans/src/agora/dashboard execute-plans/src/agora/widgets execute-plans/src/lib/bff-v1/agora services/control-plane/openapi services/control-plane/specs/agora
git diff 66aa9d24 origin/dev -- services/control-plane/openapi/agora_v1_2.openapi.yaml
git diff 66aa9d24 origin/dev -- services/control-plane/specs/agora/v3/capability_manifest_v1_2.json
git diff 66aa9d24 origin/dev -- services/control-plane/specs/agora/bundle_index.v1_2.json
...
rg -n "dashboard-recipes/\\{recipe_id\\}/layout|patchDashboardRecipeLayout|move_widget|resize_widget|add_registered_widget|replace_chart_spec|update_widget_query|remove_widget" services/control-plane/openapi/agora_v1_2.openapi.yaml execute-plans/src/lib/bff-v1/agora/types.ts execute-plans/src/lib/bff-v1/agora/dashboard.ts
```

Observed results:

- Branch is `task/AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-19`.
- Current branch HEAD equals `origin/dev` at `d69ba6f9` after fast-forwarding
  over PR #2033, which added only AG-BE-SW-002 follow-up 3 review/brief/support
  updates.
- Follow-up 18 is archived `done`; PR #2013 merged at `66aa9d24`.
- Since `66aa9d24`, dev advanced through identity/servant, strategy
  workshop/completeness, sidecar/review, OpenAPI-003, ops, and design-record
  merges.
- DB002 execute-plans dashboard, widget, and Agora BFF helper paths have no
  changed files in this delta.
- OpenAPI/specs paths changed only for servant-session `session_type` and
  bundle hash updates; dashboard layout PATCH semantics remain unchanged.
...
1. Confirm follow-up 19 accurately reflects follow-up 18 as archived `done`
   (PR #2013).
2. Confirm the post-followup-18 dev delta is characterized correctly,
   especially that `AG-XR-OPENAPI-003` touched v1.2 contract files only for
   servant-session `session_type`, not DB002 dashboard layout semantics.
