# AG-XR-003 Sidecar Acceptance Follow-up 8

- Parent task: `AG-XR-003` - Dev deployment compatibility manifest
- Helper task: `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-8`
...
Follow-up 7 recorded that Pantheon PR #1852 had merged, execute-plans PR #63
was still open/unstable, and local manifest sanity still failed on the
generated-types hash mismatch. This follow-up refreshes the same
acceptance/dependency packet against current `origin/dev`.

The conclusion is unchanged: parent `AG-XR-003` remains correctly blocked,
waiting for `Claude2` disposition. Pantheon-side implementation is durable on
`dev`, but the execute-plans mirror PR is still unmerged and the local
manifest verification path is still not green.

## Current Status Snapshot

| Surface | Current state | Sidecar stance |
|---|---|---|
| Parent `AG-XR-003` | `blocked`, waiting for `Claude2` | Keep blocked until execute-plans PR #63 and manifest sanity receive explicit disposition. |
| This sidecar | `in_progress`, owner `Codex2`, reviewer `Codex` | Packet is ready for reviewer handoff after task commit/PR. |
...
| PR #63 integration gate | Failed at `Aggregate release gate` in run `27877483718` | F13 Agora passes, but the release-level aggregate gate remains red. |
...
| Agora frontend drift | `npm --prefix execute-plans run contract:drift` passes locally | Useful Agora evidence, but not equivalent to a green deployment gate or merged mirror PR. |
...
| Parent acceptance surface | Current evidence | Follow-up 8 stance |
|---|---|---|
...
The failed execute-plans gate remains a release-level blocker, not an
Agora-only blocker:

| Gate | Current evidence | Interpretation |
|---|---|---|
| Gate 0 Preconditions | PASS | Release identifiers and target URLs are present. |
| Gate 1 Static / Build / Unit | FAIL, 2 open checks | `npm run lint` and `npm run test:contract` remain red in the aggregate summary. |
| Gate 2 Contract Drift | FAIL, 6 open checks | Aggregate summary still reports canonical contract drift checks red, even though local Agora-only `contract:drift` passes in this worktree. |
| Gate 3 BFF Route Probes | FAIL, 4 open checks | Live dry-run write, RBAC, and two-man checks remain red or warning. |
| Gate 4 Browser Frontend E2E | PASS | Browser/BFF probe passed. |
| Gate 5 Playwright User Flows | FAIL, 3 open checks | F01 and F05 remain red; F13 Agora passes. |
| Gate 6 A11y / Perf | FAIL, 2 open checks | Performance and SSE rerender checks remain red. |
| Gate 7 Release Decision | FAIL, 1 open check | Aggregate decision reports 15 failing or missing checks. |
...
gh pr view 63 --repo ajoe734/execute-plans --json number,title,state,mergeStateStatus,statusCheckRollup,headRefOid,headRefName,baseRefName,url,isDraft,mergedAt,reviewDecision,updatedAt
gh pr view 1852 --repo ajoe734/pantheon --json number,title,state,mergeStateStatus,statusCheckRollup,headRefOid,headRefName,baseRefName,url,isDraft,mergedAt,reviewDecision,mergeCommit,updatedAt
