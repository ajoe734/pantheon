# AG-FE-ID-001 Followup-39 Sidecar BFF and Frontend Handoff

| Field | Value |
|---|---|
| Sidecar task | `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-39` |
...
`AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-38` closed and Pantheon `dev`
advanced to the followup-38 review-artifact merge `f8b4fbaa`.

Material changes since followup-38:

1. Pantheon `origin/dev` remains at `f8b4fbaab2ef`; there are no commits after
   followup-38 closeout in the checked dev window.
2. No identity/servant parent-shell source or contract path changed:
   ...
   diff since followup-38 closeout.
3. No Agora BFF route path changed since followup-38 closeout.
...
5. execute-plans `dev` remains `ee835e2e`; comparing PR `#66` head against it
   ...
   `servant.test.ts`.
6. execute-plans PR `#63` remains open and `UNSTABLE` with its older failed
   integration gate.
...
|---|---|---|
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-39` | active `in_progress`; owner `Codex`; reviewer `Claude` | This packet is the only declared artifact and should go to Claude for review. |
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-38` | archived `done` at `2026-06-22T04:42:28Z` | Previous support packet and review artifact are approved and merged; they are the baseline for this refresh. |
...
|---|---|
| `.orchestrator/task-briefs/ag_fe_id_001_sidecar_bff_handoff_followup_39.md` | This sidecar's generated support-only assignment. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-39` | Confirms active owner/reviewer/status and support-only artifact path. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-38` | Confirms predecessor archived `done`, packet PR `#2200`, review artifact PR `#2201`, packet commit, review commit, and parent blocker note. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-38.md` | Previous AG-FE-ID-001 support baseline. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-38-REVIEW.md` | Confirms Claude approved the prior support packet and preserved the execute-plans PR `#66` gate blocker. |
...
| `git fetch origin --prune`; `git rev-parse origin/dev` | Confirms current Pantheon dev base `f8b4fbaab2effeb85118b2309103a1f0a236a013`. |
| `git log --oneline f8b4fbaab2effeb85118b2309103a1f0a236a013..origin/dev --decorate` | No output; there are no Pantheon dev commits after followup-38 closeout. |
| `git diff --name-status f8b4fbaab2effeb85118b2309103a1f0a236a013..origin/dev -- <identity/servant pathset>` | No output; identity/servant/main/docs contract surfaces unchanged. |
| `git diff --name-status f8b4fbaab2effeb85118b2309103a1f0a236a013..origin/dev -- services/control-plane/bff/agora` | No output; no Agora BFF route source changed since followup-38 closeout. |
...
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_servant_sessions.py services/control-plane/bff/tests/test_agora_router.py services/control-plane/bff/tests/test_agora_identity_scope.py integrations/openclaw/test_persona_agent_sync.py -q` | `39 passed in 25.81s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_candidate_pool.py -q` | `3 passed in 10.47s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/agora/trading_room/test_trading_room.py -q` | `24 passed in 6.77s`. |
...
## 4. Delta Since Followup-38 Closeout

Baseline: followup-38 merge target `f8b4fbaab2ef`. Current Pantheon dev base:
`f8b4fbaab2ef`.

| Change | What changed | Parent implication |
|---|---|---|
| Pantheon dev advanced | No advancement after followup-38 closeout. | Followup-38 packet/review/closeout remains the latest Pantheon support baseline. |
| Identity/servant checked paths | Diff over `agora/router.py`, `agora/servant`, `agora/identity`, `main.py`, and `docs/contracts/agora` produced no output. | Followup-38 identity/servant route ledger remains valid. |
| Agora BFF checked paths | Diff over `services/control-plane/bff/agora` produced no output. | No new route surface has landed since followup-38 closeout. |
...
|---|---|---|
| `GET /bff/agora/me` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover identity scope and servant policy context. No diff since followup-38. | Parent may use for identity readiness through strict BFF transport. Do not infer servant/session success from identity alone. |
| `GET /bff/agora/capabilities` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover filtered capabilities. No diff since followup-38. | Parent may use for readiness/capability display. Keep route status distinct from research/workshop/candidate/trading capabilities. |
| `POST /bff/agora/servant/ensure` | Implemented in `services/control-plane/bff/agora/servant/router.py`; requires `Idempotency-Key` and `X-Request-Id`; returns a user-private `ServantProfile` envelope and maps OpenClaw sync failures to 503 dependency unavailable. No diff since followup-38. | `servant.ts` should send required headers, parse the envelope, and map 401/403/422/503 explicitly. |
| `POST /bff/agora/servant/sessions` | Implemented by `AG-BE-ID-003`; focused tests cover default `interactive`, explicit `trainer` and `research_task`, unknown type rejection, audit fields, no-authority context, and 201 create. No diff since followup-38. | Frontend may target this only with strict servant-session clients and UI tests. Do not use legacy `/bff/agora/sessions` for these modes. |
...
The release-gate ownership from followup-38 remains the current handoff model:
...
1. Confirm this packet is support-only and only claims facts rechecked in the
   followup-38 closeout to followup-39 refresh window.
2. Confirm Pantheon dev has no new commits after followup-38 closeout at
   `f8b4fbaa`.
3. Confirm no identity/servant/Agora BFF path changed since followup-38.
...
```text
FOLLOWUP-39 support packet updates AG-FE-ID-001 handoff after followup-38
closed at Pantheon dev f8b4fbaa. There are no new Pantheon dev commits since
that closeout; identity/servant/Agora BFF route paths are unchanged.
