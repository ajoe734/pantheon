# AG-FE-ID-001 Followup-37 Sidecar BFF and Frontend Handoff

| Field | Value |
|---|---|
| Sidecar task | `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37` |
...
| Current Pantheon dev base | `fd335b5c43623d5821180faf25e4337052cb786f` |
| Previous packet closeout | Followup-36 archived `done` at `2026-06-22T03:41:21Z`; PR `#2195` merged into Pantheon dev at `447d215e6a9df213fe01ac6b4fcb1e7fd8f36d42`; packet commit `3f0ce8a9421213b35011da8e5fa9979b9621d645` |
...
`AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-36` closed and Pantheon `dev`
advanced to `fd335b5c`.

Material changes since followup-36:

1. Pantheon `origin/dev` advanced from followup-36 merge `447d215e6a9d` to
   `fd335b5c4362`.
2. The only commit in that window is PR `#2196`, which added
...
3. No identity/servant parent-shell source or contract path changed:
   ...
   diff since followup-36.
4. No Agora BFF route path changed since followup-36.
...
|---|---|---|
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37` | active `in_progress`; owner `Codex`; reviewer `Claude` | This packet is the only declared artifact and should go to Claude for review. |
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-36` | archived `done` at `2026-06-22T03:41:21Z` | Previous support packet is approved and merged; it is the baseline for this refresh. |
...
| `AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3` | archived `done`; PR `#2196` merged at `fd335b5c` | New dev delta is support-only for another parent and does not affect AG-FE-ID-001 identity/servant readiness. |
...
|---|---|
| `.orchestrator/task-briefs/ag_fe_id_001_sidecar_bff_handoff_followup_37.md` | This sidecar's generated support-only assignment. The generated brief still says `todo`; active `ai-status` is authoritative and reports `in_progress`. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37` | Confirms active owner/reviewer/status and support-only artifact path. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-36` | Confirms predecessor archived `done`, PR `#2195`, packet commit, review file, and parent blocker note. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-36.md` | Previous AG-FE-ID-001 support baseline. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-36-REVIEW.md` | Confirms Claude approved the prior support packet and preserved the execute-plans PR `#66` gate blocker. |
...
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-XR-CP-001` | Confirms candidate-pool v1.4 contract archived `done`. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-BE-CP-001` | Confirms candidate-pool BFF implementation archived `done`. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-BE-TR-001` | Confirms trading-room aggregate / event queues archived `done`. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3` | Confirms the only new dev commit since followup-36 is another support-only packet. |
| `git fetch origin --prune`; `git rev-parse origin/dev` | Confirms current Pantheon dev base `fd335b5c43623d5821180faf25e4337052cb786f`. |
| `git log --oneline 447d215e6a9d..origin/dev --decorate` | Shows only PR `#2196` / `AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3`. |
| `git show --name-status --oneline 1901039d` | Shows the dev delta added only `support/sidecars/AG-FE-RS-001/AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3.md`. |
| `git diff --name-status 447d215e6a9d..origin/dev -- <identity/servant pathset>` | No output; identity/servant/main/docs contract surfaces unchanged. |
| `git diff --name-status 447d215e6a9d..origin/dev -- services/control-plane/bff/agora` | No output; no Agora BFF route source changed since followup-36. |
...
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_servant_sessions.py services/control-plane/bff/tests/test_agora_router.py services/control-plane/bff/tests/test_agora_identity_scope.py integrations/openclaw/test_persona_agent_sync.py -q` | `39 passed in 47.46s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_candidate_pool.py -q` | `3 passed in 14.60s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/agora/trading_room/test_trading_room.py -q` | `24 passed in 9.47s`. |
...
## 4. Delta Since Followup-36 Closeout

Baseline: followup-36 merge `447d215e6a9d`. Current Pantheon dev base:
`fd335b5c4362`.

| Change | What changed | Parent implication |
|---|---|---|
| Pantheon dev advanced | `origin/dev` moved from `447d215e6a9d` to `fd335b5c4362`. | Use `fd335b5c4362`, not followup-36's closeout merge, for current support facts. |
| AG-FE-RS-001 support packet | PR `#2196` added `support/sidecars/AG-FE-RS-001/AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3.md`. | Support-only for another parent; no AG-FE-ID-001 identity/servant implication. |
| Identity/servant checked paths | Diff over `agora/router.py`, `agora/servant`, `agora/identity`, `main.py`, and `docs/contracts/agora` produced no output. | Followup-36 identity/servant route ledger remains valid. |
| Agora BFF checked paths | Diff over `services/control-plane/bff/agora` produced no output. | No new route surface has landed since followup-36. |
...
|---|---|---|
| `GET /bff/agora/me` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover identity scope and servant policy context. No diff since followup-36. | Parent may use for identity readiness through strict BFF transport. Do not infer servant/session success from identity alone. |
| `GET /bff/agora/capabilities` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover filtered capabilities. No diff since followup-36. | Parent may use for readiness/capability display. Keep route status distinct from research/workshop/candidate/trading capabilities. |
| `POST /bff/agora/servant/ensure` | Implemented in `services/control-plane/bff/agora/servant/router.py`; requires `Idempotency-Key` and `X-Request-Id`; returns a user-private `ServantProfile` envelope and maps OpenClaw sync failures to 503 dependency unavailable. No diff since followup-36. | `servant.ts` should send required headers, parse the envelope, and map 401/403/422/503 explicitly. |
| `POST /bff/agora/servant/sessions` | Implemented by `AG-BE-ID-003`; focused tests cover default `interactive`, explicit `trainer` and `research_task`, unknown type rejection, audit fields, no-authority context, and 201 create. No diff since followup-36. | Frontend may target this only with strict servant-session clients and UI tests. Do not use legacy `/bff/agora/sessions` for these modes. |
...
The release-gate ownership from followup-36 remains the current handoff model:
...
| Command | Result |
|---|---|
| `git status -sb`; `git branch --show-current`; `git remote -v` | Started on expected branch `task/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37`; remote is `origin` `https://github.com/ajoe734/pantheon.git`; only generated task brief was initially untracked. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37` | Active `in_progress`; owner `Codex`; reviewer `Claude`; artifact path is this support packet. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001` | Parent remains `blocked`, waiting for `Gemini`; PR `#66` aggregate release gate remains the blocker. |
| `git fetch origin --prune`; `git rev-parse origin/dev` | Pantheon `origin/dev` is `fd335b5c43623d5821180faf25e4337052cb786f`. |
| `git log --oneline 447d215e6a9d..origin/dev --decorate` | Dev delta only includes PR `#2196` / `AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3`. |
| `git show --name-status --oneline 1901039d` | Dev delta added only `support/sidecars/AG-FE-RS-001/AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-3.md`. |
| `git diff --name-status 447d215e6a9d..origin/dev -- services/control-plane/bff/agora/router.py services/control-plane/bff/agora/servant services/control-plane/bff/agora/identity services/control-plane/bff/main.py docs/contracts/agora` | No output; identity/servant/main/docs-contract surfaces unchanged. |
| `git diff --name-status 447d215e6a9d..origin/dev -- services/control-plane/bff/agora` | No output; no Agora BFF route source changed since followup-36. |
...
especially the followup-37 delta statement:

- no AG-FE-ID-001 identity/servant path changed since followup-36
- the only Pantheon dev delta is another support-only AG-FE-RS packet
...
`AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-37` support slice on `2026-06-22`.*
