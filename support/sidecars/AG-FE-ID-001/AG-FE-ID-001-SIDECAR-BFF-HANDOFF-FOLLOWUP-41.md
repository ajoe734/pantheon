# AG-FE-ID-001 Followup-41 Sidecar BFF and Frontend Handoff

| Field | Value |
|---|---|
| Sidecar task | `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-41` |
...
| Current Pantheon dev base | `80edfc2bfec1a35040d6340271754fad111fa86f` |
| Previous packet closeout | Followup-40 archived `done` at `2026-06-22T06:06:33Z`; packet PR `#2208` merged at `ea209f34975063fa7c73350babc212c747d292ed`; review artifact PR `#2209` merged at `c885402f4175be1af563584ab54eb47d74c3986e`; closeout PR `#2210` merged at `ad09c3b2e3e03bae6bbf6058f6f1f87e33d999ae`; packet commit `2b3f1f32`; review commit `2034868e`; closeout commit `985e0944` |
...
`AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-40` closed and Pantheon `dev`
advanced from its closeout merge `ad09c3b2` to `80edfc2b`.

Material changes since followup-40 closeout:

1. Pantheon `origin/dev` advanced by AG-FE-RS-001 sidecar packet merges and one
   Management live-evidence / CI-helper merge.
2. The post-closeout dev-window diff includes:
...
3. No identity/servant parent-shell source or contract path changed:
   ...
   diff since followup-40 closeout.
4. No `support/sidecars/AG-FE-ID-001/` file changed after the followup-40
   closeout merge.
...
|---|---|---|
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-41` | active `in_progress`; owner `Codex`; reviewer `Claude` | This packet is the only declared artifact and should go to Claude for review. |
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-40` | archived `done` at `2026-06-22T06:06:33Z` | Previous support packet, review artifact, and owner closeout are merged and are the baseline for this refresh. |
...
|---|---|
| `.orchestrator/task-briefs/ag_fe_id_001_sidecar_bff_handoff_followup_41.md` | This sidecar's generated support-only assignment. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-41` | Confirms active owner/reviewer/status and support-only artifact path. |
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-40` | Confirms predecessor archived `done`, packet/review/closeout PRs, commits, and parent blocker note. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-40.md` | Previous AG-FE-ID-001 support baseline and owner closeout record. |
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-40-REVIEW.md` | Confirms Claude approved the prior support packet and preserved the execute-plans PR `#66` gate blocker. |
...
| `AI_NAME=Codex ./scripts/ai-status.sh show AG-FE-000` and `AG-BE-ID-003` | Confirms both declared parent dependencies remain archived `done`. |
| `git fetch origin --prune`; `git rev-parse origin/dev` | Confirms current Pantheon dev base `80edfc2bfec1a35040d6340271754fad111fa86f`. |
| `git log --oneline ad09c3b2..origin/dev --decorate` | Confirms post-followup-40-closeout dev advancement through AG-FE-RS packet merges and `MGMT-LIVE-EVIDENCE` / CI-helper changes. |
| `git diff --name-status ad09c3b2..origin/dev` | Confirms exact file list in the post-closeout dev window. |
| `git diff --name-status ad09c3b2..origin/dev -- <identity/servant pathset>` | No output; identity/servant/main/docs contract surfaces unchanged. |
| `git diff --name-status ad09c3b2..origin/dev -- support/sidecars/AG-FE-ID-001` | No output; no AG-FE-ID-001 support files changed after followup-40 closeout. |
...
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_servant_sessions.py services/control-plane/bff/tests/test_agora_router.py services/control-plane/bff/tests/test_agora_identity_scope.py integrations/openclaw/test_persona_agent_sync.py -q` | `39 passed in 26.09s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/tests/test_agora_candidate_pool.py -q` | `3 passed in 11.19s`. |
| `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest services/control-plane/bff/agora/trading_room/test_trading_room.py -q` | `24 passed in 7.53s`. |
...
## 4. Delta Since Followup-40 Closeout

Baseline: followup-40 closeout merge target `ad09c3b2`. Current Pantheon dev
base: `80edfc2b`.

| Change | What changed | Parent implication |
|---|---|---|
| AG-FE-RS sidecar packets | PR `#2211` / `#2212` added and closed `AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-6.md`; PR `#2213` added `AG-FE-RS-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-7.md`. | Adjacent research-support packets only. They do not change parent `AG-FE-ID-001` runtime readiness. |
| Management live-evidence / CI helper | PR `#2171` added secret-inventory check scripts and updated branch CI / trailer range helper tests. | Repository tooling/live-evidence surface changed, but no Agora identity, servant, frontend handoff, or contract path changed. |
| Identity/servant checked paths | Diff over `agora/router.py`, `agora/servant`, `agora/identity`, `main.py`, and `docs/contracts/agora` produced no output. | Followup-40 identity/servant route ledger remains valid. |
| AG-FE-ID-001 support folder | Diff over `support/sidecars/AG-FE-ID-001` produced no output. | No newer AG-FE-ID-001 packet supersedes followup-40 before this followup-41 packet. |
...
|---|---|---|
| `GET /bff/agora/me` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover identity scope and servant policy context. No diff since followup-40 closeout. | Parent may use for identity readiness through strict BFF transport. Do not infer servant/session success from identity alone. |
| `GET /bff/agora/capabilities` | Implemented in `services/control-plane/bff/agora/router.py`; focused tests cover filtered capabilities. No diff since followup-40 closeout. | Parent may use for readiness/capability display. Keep route status distinct from research/workshop/candidate/trading capabilities. |
| `POST /bff/agora/servant/ensure` | Implemented in `services/control-plane/bff/agora/servant/router.py`; requires `Idempotency-Key` and `X-Request-Id`; returns a user-private `ServantProfile` envelope and maps OpenClaw sync failures to 503 dependency unavailable. No diff since followup-40 closeout. | `servant.ts` should send required headers, parse the envelope, and map 401/403/422/503 explicitly. |
| `POST /bff/agora/servant/sessions` | Implemented by `AG-BE-ID-003`; focused tests cover default `interactive`, explicit `trainer` and `research_task`, unknown type rejection, audit fields, no-authority context, and 201 create. No diff since followup-40 closeout. | Frontend may target this only with strict servant-session clients and UI tests. Do not use legacy `/bff/agora/sessions` for these modes. |
...
The release-gate ownership from followup-40 remains the current handoff model:
...
1. Confirm this packet is support-only and only claims facts rechecked in the
   followup-40 closeout to followup-41 refresh window.
2. Confirm Pantheon dev advancement after followup-40 is limited to AG-FE-RS
   support packets plus non-Agora Management live-evidence / CI-helper changes.
3. Confirm no identity/servant/Agora BFF path changed since followup-40.
...
```text
FOLLOWUP-41 support packet updates AG-FE-ID-001 handoff after followup-40
closed at Pantheon dev ad09c3b2. Current Pantheon dev is 80edfc2b; the
post-closeout changes are AG-FE-RS support packet files plus non-Agora
MGMT-LIVE-EVIDENCE / CI-helper updates. Identity, servant, and Agora BFF route
paths are unchanged. execute-plans PR #66 remains OPEN/UNSTABLE on
integration-gate, PR #63 remains OPEN/UNSTABLE, and the Agora compatibility
manifest remains fail-closed. Backend-focused validation passed: 39
identity/servant/session tests, 3 candidate-pool tests, and 24 trading-room
tests. Please review scope discipline and factual accuracy before parent
absorption.
```

*Prepared by Codex for the `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-41` support slice.*
