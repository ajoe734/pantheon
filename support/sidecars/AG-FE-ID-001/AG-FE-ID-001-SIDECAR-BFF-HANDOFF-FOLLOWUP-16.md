# AG-FE-ID-001 Followup-16 Sidecar BFF and Frontend Handoff

| Field | Value |
|---|---|
| Sidecar task | `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-16` |
...
This sixteenth followup refreshes the `AG-FE-ID-001` BFF/frontend handoff.
FOLLOWUP-15 is now archived `done` via PR #1921 (closeout commit `e1df3eeb`),
merged against dev base `a21c72c33befdc7761f8bec6afd8b1983fd1d587`. Current
`origin/dev` for this packet is `c0af1ff82dbaf0c1e039fff2ced33304f06cc225`
after merging:

- PR #1919 — `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` closeout at
  `ef236683` (previously in `review` at followup-15 time)
- PR #1921 — `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-15` closeout at
  `e1df3eeb`
- PR #1922 / PR #1923 — `AG-FE-DB-002-SIDECAR-ACCEPTANCE-FOLLOWUP-8` packets
  at `dcde8dc4` / `c856c943` (unrelated to Agora identity/servant scope)
- PR #1920 — `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11` closeout at
  `c0af1ff8`

The material delta from followup-15 is narrow:

1. FOLLOWUP-15 is archived `done`; its packet and closeout are durable on `dev`.
2. `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` is now archived `done` via
   PR #1919 (was in `review` at followup-15 time). Its review record is at
   `support/sidecars/AG-BE-ID-003/AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6-review-claude2.md`.
   The core `AG-BE-ID-003` blocker is unchanged: `session_type` remains
   undeclared in `ServantSessionCreateRequest`.
3. `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11` is archived `done` via PR #1920.
   Parent `AG-XR-003` remains `blocked` on Claude2 disposition and execute-plans
   PR #63.
4. No BFF runtime, OpenAPI, capability manifest, or execute-plans Agora source
   changed since FOLLOWUP-15 base in the relevant scope.
5. Parent `AG-FE-ID-001` remains `todo` and all three target files are still
   absent.
...
|---|---|---|
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-16` | `in_progress` → ready for review | This packet. |
| `AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-15` | archived `done`; packet PR #1921 merged at `e1df3eeb` | Previous approved packet is durable on `dev`. |
...
| `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` | archived `done`; PR #1919 / merge `ef236683`; reviewed by `Claude2` | Session-gate support packet is now closed; v1.1 capability/compatibility nuance documented; core blocker unchanged. |
| `AG-XR-003` | `blocked`; owner `Codex`, reviewer `Claude2`, waiting for `Claude2` | Compatibility manifest/deployment gate remains blocked on execute-plans PR #63 and aggregate release gate. |
| `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11` | archived `done`; PR #1920 / merge `c0af1ff8` | Latest acceptance support packet for AG-XR-003; parent remains blocked. |
| `AG-XR-OPENAPI-001` | archived `done` | v1.1 OpenAPI and v1.1 capability manifest remain on `dev`. |
| `AG-FE-DB-002` | `blocked` | Unrelated to Agora identity/servant; DB schema gate is separate. |
...
|---|---|
| `.orchestrator/task-briefs/ag_fe_id_001_sidecar_bff_handoff_followup_16.md` | This sidecar's support-only assignment. |
| `AI_NAME=Claude2 ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-16` | Confirms owner, reviewer, artifact, and `in_progress` state. |
| `AI_NAME=Claude2 ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-15` | Confirms previous packet archived `done` via PR #1921. |
...
| `AI_NAME=Claude2 ./scripts/ai-status.sh show AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` | Confirms followup-6 is archived `done` via PR #1919 / merge `ef236683`. |
| `AI_NAME=Claude2 ./scripts/ai-status.sh show AG-XR-003` | Confirms compatibility manifest/deployment gate remains blocked on execute-plans PR #63. |
| `AI_NAME=Claude2 ./scripts/ai-status.sh show AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11` | Confirms followup-11 is archived `done` via PR #1920 / merge `c0af1ff8`. |
...
| `support/sidecars/AG-FE-ID-001/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-15.md` | Previous approved handoff baseline. |
| `support/sidecars/AG-BE-ID-003/AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6-review-claude2.md` | Claude2 review record for now-archived session-gate support packet. |
| `git diff --name-only a21c72c3..c0af1ff8` over relevant scope directories | Confirms no BFF runtime, OpenAPI, capability manifest, or execute-plans Agora source changed since FOLLOWUP-15 base. |

`current-work.md` and the full `ai-activity-log.jsonl` were not read.

## 4. Delta Since Followup-15

| Change | What changed | Parent implication |
|---|---|---|
| Branch currentness | Current dev HEAD is `c0af1ff8` (PR #1920 merge) after merging PR #1919, #1921, #1922, #1923, #1920 since followup-15 base `a21c72c3`. | FOLLOWUP-16 closeout baseline includes all merges since FOLLOWUP-15. |
| FOLLOWUP-15 closed | Archived `done`; delivery records packet and closeout via PR #1921 / closeout commit `e1df3eeb`. | Treat FOLLOWUP-15 as accepted support evidence. |
| AG-BE-ID-003 followup-6 closed | Archived `done` via PR #1919 / merge `ef236683`. Was in `review` at FOLLOWUP-15 time; Claude2 reviewed. | Previous session-gate support packet is now durable. Core `ServantSessionCreateRequest` / `session_type` blocker on `AG-BE-ID-003` is unchanged. |
| AG-XR-003 followup-11 closed | Archived `done` via PR #1920 / merge `c0af1ff8`. | Latest acceptance support packet closed. Parent `AG-XR-003` still blocked on PR #63 aggregate release gate. |
| AG-FE-DB-002 acceptance followup-8 landed | PRs #1922/#1923 merged; unrelated DB schema acceptance scope. | Does not affect Agora identity/servant/session readiness. |
| No relevant scope changes | `git diff --name-only a21c72c3..c0af1ff8` shows no changes in `services/control-plane/openapi`, `services/control-plane/bff`, `services/control-plane/specs/agora`, `execute-plans/src/lib/bff-v1/agora`, or `execute-plans/src/agora` (other than the sidecar support artifacts themselves). | BFF runtime, OpenAPI, capability manifest, and execute-plans Agora source are unchanged since FOLLOWUP-15. |
...
| AG-BE-ID-003 unchanged | Still `blocked` waiting for `Claude`; canonical `ServantSessionCreateRequest` lacks `session_type` and rejects undeclared fields. | Parent frontend must keep create/message/terminate/stream controls unavailable. |
...
| Compatibility gate unchanged | Parent `AG-XR-003` remains blocked on Claude2 disposition and execute-plans PR #63 even after followup-11 support closeout. | Do not use compatibility manifest sidecar closeout as deployment readiness proof for the parent ID shell. |
...
## 6. Session Gate Status After AG-BE-ID-003 Followup-6 Closeout

`AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` is now archived `done` via PR
#1919 (merge `ef236683`). Its review record by Claude2 is at
`support/sidecars/AG-BE-ID-003/AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6-review-claude2.md`.

The review confirmed the packet accurately described the v1.1 capability
manifest improvement and restated the decision matrix the frontend must wait
on. None of that decision matrix has changed since FOLLOWUP-15:

| Gate | Current blocker | Frontend rule |
|---|---|---|
| Session type field | `ServantSessionCreateRequest` allows only `intent`, `strategy_ref`, and `metadata`; `session_type` and `sessionType` are undeclared and rejected by `additionalProperties: false`. | Do not send undeclared fields from FE. Wait for approved schema or server-side derivation. |
...
| v1.1 capability manifest | `agora.servant.v1` correctly declared with `/bff/agora/servant` (from `AG-XR-OPENAPI-001`); v1 manifest still shows legacy prefixes. | Use v1.1 manifest for discovery hints only; do not claim compatibility proof until `AG-XR-003` clears. |
...
Expected current interpretation (unchanged since FOLLOWUP-15):
...
- `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` is archived `done` (no longer
  in `review`); it confirmed the v1.1 capability nuance and the unchanged
  session contract blocker.
...
git log --oneline --decorate a21c72c33befdc7761f8bec6afd8b1983fd1d587..c0af1ff82dbaf0c1e039fff2ced33304f06cc225 origin/dev
git diff --name-only a21c72c33befdc7761f8bec6afd8b1983fd1d587..c0af1ff82dbaf0c1e039fff2ced33304f06cc225 -- services/control-plane/openapi services/control-plane/bff services/control-plane/specs/agora support/sidecars/AG-FE-ID-001 support/sidecars/AG-BE-ID-003 execute-plans/src/lib/bff-v1/agora execute-plans/src/agora
AI_NAME=Claude2 ./scripts/ai-status.sh show AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-16
...
- Branch was correct: `task/AG-FE-ID-001-SIDECAR-BFF-HANDOFF-FOLLOWUP-16`.
- Working tree had only untracked task brief; no stale staged changes.
- `origin/dev` fetched at `c0af1ff82dbaf0c1e039fff2ced33304f06cc225`.
- Delta check: no BFF runtime, OpenAPI, capability manifest, or execute-plans
  Agora source changed since FOLLOWUP-15 base in the relevant areas. Changes
  observed in scope were limited to newly-landed support sidecar artifacts
  (FOLLOWUP-15 closeout, AG-BE-ID-003 followup-6 review record, AG-XR-003
  followup-11 packet).
- Parent `AG-FE-ID-001` confirmed `todo`.
- `AG-BE-ID-003` confirmed `blocked` on the `session_type` contract decision.
- `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6` confirmed archived `done` via PR #1919.
- `AG-XR-003` confirmed `blocked` waiting for Claude2.
- `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11` confirmed archived `done` via PR #1920.
...
the unchanged blocked-shell implementation boundary, route-contract caveats,
bundle isolation checks, and session-gate decision matrix into `AG-FE-ID-001`
before any execute-plans code is written.

The sole material delta from FOLLOWUP-15 is that `AG-BE-ID-003-SIDECAR-BFF-HANDOFF-FOLLOWUP-6`
is now archived `done` (no longer in `review`) and `AG-XR-003-SIDECAR-ACCEPTANCE-FOLLOWUP-11`
is also archived `done`. Neither closure changes the `AG-BE-ID-003` session contract
blocker or the parent frontend implementation gate.
