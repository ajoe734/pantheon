"""Durable local state for the supervised alpha-replication controller.

Writes use fsync + atomic rename, and every snapshot carries a checksum.
"""
...
SCHEMA_VERSION = "alpha_replication_controller_state.v1"
...
class ControllerState:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "ControllerState":
        if payload.get("schema_version") != SCHEMA_VERSION:
            raise ControllerStateError("unsupported controller state schema")
        return cls(
...
class ControllerStateStore:
    ...
    def save(self, state: ControllerState) -> None:
        ...
        try:
            ...
            raise ControllerStateError(f"failed to save controller state: {exc}") from exc
        finally:
            if temp_path.exists():
                temp_path.unlink()
