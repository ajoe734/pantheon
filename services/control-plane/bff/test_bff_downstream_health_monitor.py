...
class TestTelemetryEmit:
    ...
    def test_emit_telemetry_posts_to_ingest(self):
        ...
        assert body["infrastructure_probe"]["target_name"] == "incidents"
...
# ---------------------------------------------------------------------------
# Integration tests exercising real telemetry and incidents apps
# ---------------------------------------------------------------------------


class TestRealServiceIngestAndIncidentsIntegration:
    """Blocker 1 & 2 verification: exercise real Flask/FastAPI apps without mocking _post_json."""

    def test_real_telemetry_ingest_accepts_infrastructure_health_probe(self):
        """Verify TelemetryIngestService accepts emitted infrastructure_health event even when binding_store is configured."""
        import services.telemetry.main as tel_main
        from services.telemetry.main import app as tel_flask_app

        class StubBindingStore:
            def get_binding(self, binding_id: str):
                return None  # All bindings unknown

        old_svc = getattr(tel_main, "_svc", None)
        try:
            # Instantiate service with an active binding_store
            svc = tel_main._build_service(lineage_write_store=None)
            svc._binding_store = StubBindingStore()
            tel_main._svc = svc

            client = tel_flask_app.test_client()
            monitor = DownstreamHealthMonitor(telemetry_url="", incidents_url="")
            probe_res = DownstreamProbeResult(
                target_name="telemetry",
                ok=False,
                status_code=503,
                latency_ms=120.0,
            ...
            # Build event using monitor's internal logic
            event_id = str(uuid.uuid4())
            event = {
                "tenant_id": "tenant-alpha",
                "event_id": event_id,
                "event_type": "infrastructure_health",
                "created_at": probe_res.checked_at,
                "execution_mode": "paper",
                "deployment_stage": "paper",
                "binding_id": "00000000-0000-4000-8000-0000000000b1",
                ...
                "target": {
                    "strategy_id": "bff-health-telemetry",
                    "registry_id": "00000000-0000-4000-8000-0000000000b1",
                    "artifact_version": "0.1.0",
                },
                "metrics": {
                    "probe_ok": 0,
    ...
    def test_real_incidents_app_open_and_status_resolution(self):
        ...
        sentinel_id = f"bff-downstream-incidents-degraded-{uuid.uuid4().hex[:6]}"

        create_body = {
            "incident_id": sentinel_id,
            "title": "BFF downstream degradation: incidents unreachable",
            "status": "open",
            "severity": "high",
            "binding_id": "00000000-0000-4000-8000-0000000000b1",
            "deployment_stage": "paper",
            "deployment_plan_id": "bff-health-probe",
            "capital_pool_id": "bff-control-plane",
            "persona_capital_binding_id": "bff-health-probe",
            "artifact_id": "bff-operator",
            "artifact_version": "0.1.0",
        ...
        res = client.post("/api/incidents", json=create_body)
        assert res.status_code == 201, res.text
        assert res.json()["incident_id"] == sentinel_id
        assert res.json()["status"] == "open"

        # 2. Duplicate create -> 409
        res_dup = client.post("/api/incidents", json=create_body)
        assert res_dup.status_code == 409

        # 3. Transition to resolved via POST /api/incidents/{id}/status -> 200
        status_body = {
            "status": "resolved",
            "resolved_at": _utc_now_rfc3339(),
        }
        res_status = client.post(f"/api/incidents/{sentinel_id}/status", json=status_body)
        assert res_status.status_code == 200, res_status.text
        assert res_status.json()["status"] == "resolved"
        assert res_status.json()["resolved_at"] is not None
