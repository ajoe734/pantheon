...
| RS-04 | Dormant OSS Pre-Activation View | research orchestrator, policy-learning, research-worker gateway, OpenClaw adapter read models | `GET /api/v1/operator/research/oss-preactivation` | Read-only capability and rejection evidence for activation-gated OSS backends; no production activation or write authority |

**Objects referenced**: `ResearchPackage`, `StrategySpec` (via OC-003), `ReplicationGateResult`

**Objects additionally referenced by RS-04**: dormant capability metadata and rejected run/job records from service-owned read APIs. RS-04 is pre-activation-only metadata and must not be used as evidence of Qlib/TRL/RL/W&B/OpenClaw production activation.
