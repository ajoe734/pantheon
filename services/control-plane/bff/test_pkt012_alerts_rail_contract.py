...
def _make_client(store: Any) -> TestClient:
    app = FastAPI()
    app.include_router(create_incident_router(read_surface=store))
...
def test_pkt012_alerts_rail_returns_backend_owned_alert_feed() -> None:
    ...
    assert payload["meta"]["surfaces"]["review_queue"]["status"] == "ok"
    assert payload["meta"]["surfaces"]["approval_queue"]["status"] == "ok"
    assert payload["meta"]["surfaces"]["kill_switch"]["status"] == "ok"
    assert payload["meta"]["surfaces"]["runtime_roster"]["status"] == "ok"
    assert payload["meta"]["surfaces"]["telemetry_summary"]["status"] == "ok"
    ...
    assert payload["alerts"][0] == {
        ...
        "target_ref": {
            ...
            "href": "/management/runtime",
            "target_id": "runtime-042",
        },
    }
    assert payload["alerts"][1]["target_ref"]["surface_id"] == "OC-03"
    assert payload["alerts"][1]["target_ref"]["href"] == "/management/health"
    assert payload["alerts"][2]["target_ref"]["surface_id"] == "PKT-002"
    assert payload["alerts"][2]["target_ref"]["href"] == "/management/incidents/inc-001"
    assert payload["alerts"][3]["target_ref"]["surface_id"] == "GV-02"
    assert payload["alerts"][3]["target_ref"]["href"] == "/management/governance/approvals"
    assert payload["alerts"][4]["target_ref"]["surface_id"] == "PKT-001"
    assert payload["alerts"][4]["target_ref"]["href"] == "/management/governance/review-queue"
...
def test_pkt012_alerts_rail_returns_unavailable_when_all_sources_are_missing() -> None:
    ...
    assert payload["meta"]["surfaces"]["incident"]["status"] == "unavailable"
...
def test_alt001_bff_alerts_endpoint_returns_operator_alert_projection_and_detail() -> None:
    ...
    assert alert == {
        ...
        "target_ref": {
            ...
            "href": "/management/incidents/inc-alt-001",
