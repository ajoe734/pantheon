...
class InMemoryAgoraJournalStore:
    ...
    def _project(record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "createdBy": str(record.get("createdBy") or "op-2"),
            "tenant_id": str(record.get("tenant_id") or "pantheon-dev"),
            "tenantId": str(record.get("tenantId") or record.get("tenant_id") or "pantheon-dev"),
...
def _build_client(store: InMemoryAgoraJournalStore, monkeypatch: Any = None) -> TestClient:
    if monkeypatch:
...
def test_agora_journal_patch_rejects_non_merge_patch_content_type(monkeypatch: Any) -> None:
    client = _build_client(_seeded_store(), monkeypatch)
...
def test_agora_journal_patch_rejects_body_idempotency_key(monkeypatch: Any) -> None:
    client = _build_client(_seeded_store(), monkeypatch)
    response = client.patch(
        "/bff/agora/journal/journal-001",
...
def test_agora_journal_patch_returns_data_and_audit_diff(monkeypatch: Any) -> None:
    store = _seeded_store()
    client = _build_client(store, monkeypatch)
    response = client.patch(
        "/bff/agora/journal/journal-001",
...
def test_agora_journal_patch_idempotency_conflict_rejected(monkeypatch: Any) -> None:
    client = _build_client(_seeded_store(), monkeypatch)
