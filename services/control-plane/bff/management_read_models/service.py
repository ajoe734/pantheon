...
class ManagementService:
    ...
    # -----------------------------------------------------------------------
    # 15. Management Natural-Language (NL) Ask & Stream Operations
    # -----------------------------------------------------------------------
    _HIGH_RISK_PATTERNS = [
        (r"\bdrop\s+(table|database|schema)\b", "destructive_database_mutation", "Direct database drop operations are not permitted via natural language."),
        (r"\bdelete\s+from\b", "destructive_database_mutation", "Bulk delete queries are not permitted via natural language."),
        (r"\bdelete\s+all\s+personas\b", "persona_destruction", "Bulk deletion of personas is prohibited."),
        (r"\b(place|execute)\s+(real|live)\s+(trade|order)\b", "capital_mutation", "Live order execution must go through governed capital deployment workflows."),
    ...
    def classify_high_risk(self, question: str) -> Optional[Dict[str, Any]]:
        ...
        for pattern, category, alt in self._HIGH_RISK_PATTERNS:
