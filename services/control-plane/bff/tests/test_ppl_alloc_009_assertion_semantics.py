...
def test_float_int_roundtrip_matches():
    ...
    assert _pm12_semantic_values_match(0.0, 0)
    assert _pm12_semantic_values_match(1, 1.0)
    assert _pm12_semantic_values_match(Decimal("1.0"), 1)


def test_nested_and_list_roundtrip_matches():
    assert _pm12_semantic_values_match([1.0, 2.0], [1, 2])
...
def test_genuine_divergence_still_fails_closed():
    ...
    assert not _pm12_semantic_values_match(1.0, 2.0)
    # Booleans must never collapse into numbers.
    assert not _pm12_semantic_values_match(True, 1)
    assert not _pm12_semantic_values_match(1, True)
    ...
    assert not _pm12_semantic_values_match(None, 0)
...
def test_noncanonicalizable_input_fails_closed():
    ...
    assert not _pm12_semantic_values_match({1: "int-key"}, {1: "int-key"})
...
def test_semantic_hash_still_used_for_rebalance_line():
    ...
    assert _pm12_semantic_values_match(line_float, line_int)
