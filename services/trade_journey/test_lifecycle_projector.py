...
def test_relational_projector_quarantines_source_drift_and_advances_cursor():
    rows = lifecycle_rows()
    ...
    store = _RecordingRelationalStore()
    projector = RelationalLifecycleProjector(
        store, deployment_sha="relational-quarantine-test", clock=lambda: NOW
    )
    ...
    assert result.quarantined == 1
    mutation = store.mutations[-1]
    assert mutation.quarantines[-1].ingested_seq == 1
    assert "created_at mismatch" in mutation.quarantines[-1].reason_detail
    assert any(
        receipt.event_id.startswith(rows[1]["event_id"])
