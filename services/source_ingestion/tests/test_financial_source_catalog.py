...
def test_catalog_config_templates_are_secret_ref_only_and_cover_active_universe_sources() -> None:
    ...
    assert "template-us-yahoo-daily-ohlcv" not in templates
    ...
    assert not any(
        template["fetch"].get("adapter", "").startswith("YahooUsEquityDailyAdapter")
...
def test_catalog_embeds_active_universe_scheduling_policy() -> None:
    ...
    assert "us-yahoo-daily-ohlcv" not in policy["summary"]["archive_baseline_connector_ids"]
