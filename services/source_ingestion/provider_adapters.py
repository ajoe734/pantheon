...
def _tdcc(
    ...
    if payload in (None, ""):
        raise SourceEvidenceError("TDCC shareholding distribution adapter requires payload or rows")
    return adapter.records_from_payload(
        payload,
...
def _taifex(
    ...
    payload = request.get("payload") or request.get("rows") or request.get("data")
    if payload in (None, ""):
        raise SourceEvidenceError("TAIFEX derivatives chip adapter requires payload or rows")
    return adapter.records_from_payload(
        payload,
        dataset=dataset,
        source_dataset=request.get("source_dataset"),
...
def _social(
    ...
    if payload in (None, ""):
        raise SourceEvidenceError("Admitted social media adapter requires payload, items, or posts")
    return adapter.records_from_payload(
        payload,
...
def _alpha_db(
    ...
    if payload in (None, ""):
        raise SourceEvidenceError("External Alpha DB adapter requires payload, signals, or records")
    return adapter.records_from_payload(
        payload,
