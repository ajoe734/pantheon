...
1. **Retention and Archival Schedules**: Neither U10A nor U10B may invent retention periods or archival purge authorities for job stores (`worker_jobs.json`, `research_runs.json`, `training_session.authority_records`, `JsonlIngestScheduleStore`, `policy_learning_jobs.json` / `policy_learning.jobs`). Defining retention periods (e.g. 30-day cold storage vs permanent record) and archival mutator authority requires explicit operator policy decisions.
...
| `services/source_ingestion/main.py`; `routers/ingest_operations.py:168–183`; `runtime.py:267, 318`; `scheduler.py:281, 394–423` | Ingestion service generates native `ingest-{uuid}` or `run-{connector_id}` IDs. Jobs and receipts are owned by `JsonlIngestScheduleStore` (`runtime.store`), which maintains runs, receipts, connector watermarks, and crawl frontier items with exclusive file locking; `SourceManagementStore` is separate source-instance/control authority. Has NO cancel endpoint anywhere in the service. |
| `services/policy-learning/main.py:189–196, 551–564`; `store.py:197, 617, 1028–1060` | Policy learning generates native `plj-{date}-{seq}` IDs. Employs dual backend: file-backed `policy_learning_jobs.json` or authoritative `PostgresPolicyLearningJobStore` table `policy_learning.jobs` when `POLICY_LEARNING_STORE_BACKEND=postgres`. Candidate authority upgrades to PostgreSQL by default with DSN, whereas job authority strictly requires the backend environment variable. `reject_job` only mutates status row; has no worker stop; direct promotion returns 409 (requires Governance). |
...
| **4. Source-ingest runs/jobs** | `services/source_ingestion/` (`JsonlIngestScheduleStore` in `scheduler.py:281, 394–423` via `runtime.py:267, 318`; `SourceManagementStore` is separate source-instance/control authority) | `ingest-{uuid4().hex[:12]}` or `run-{connector_id}` (connectors/base.py:634) | `job-ingest-{ingest_run_id}` | **Enforcement**: Controller token verified on connector mutation via `load_controller_token()`.<br>**Gaps**: `/api/source-ingest/jobs` has no tenant filter and no auth dependency. | **Observed**: Processes ingest batches, records runs and receipts via `runtime.store`, frontier replay.<br>**Missing**: **NO cancel endpoint exists**; no graceful extraction worker halt. | **Conditionally Admitted for Read Projection Only**; Excluded from Management Job Actions. | Read: U10A.<br>Cancel operation: **SI-CANCEL-001** (Source ingestion owner task, outside U10B). |
| **5. Policy-learning jobs** | `services/policy-learning/` (`PolicyLearningStore` with dual backend: `policy_learning_jobs.json` or `PostgresPolicyLearningJobStore` table `policy_learning.jobs` when `POLICY_LEARNING_STORE_BACKEND=postgres`, store.py:197, 617, 1028–1060) | `plj-{YYYYMMDD}-{index:03d}` (main.py:189-196) | `job-policy-{plj_id}` | **Enforcement**: Tenant scope helper for claims/DLQ.<br>**Gaps**: `list_jobs` has no tenant filter; unauthenticated endpoints; reject is row-only. | **Observed**: `reject_job` (main.py:551-564) updates row status `rejected` & appends event; promote returns 409.<br>**Missing**: Reject does not halt running worker; promote requires Governance. | **Conditionally Admitted for Read Projection Only**; Excluded from Direct JobAction Dispatch. | Read: U10A.<br>Worker stop: **PL-CANCEL-001** (Policy learning owner task, outside U10B). |
...
- **Owner Tests**: Existing test suites: `services/research-worker-gateway/tests/test_research_worker_gateway_http_service.py` (HTTP service lifecycle, status replay, cancel idempotency), `services/research-worker-gateway/tests/test_research_worker_gateway_postgres_event_store.py` (durable event append). Proposed new tests: worker process termination and cancellation fence suite to be delivered in `GW-STOP-FENCE-001`.
...
- **Owner Tests**: Existing test suites: `services/research/tests/test_research_orchestrator_http_service.py` (orchestrator lifecycle, idempotent handoff, stage execution), `services/research/tests/test_research_postgres_event_store.py` (event store append and readback). Proposed new tests: run cancellation and cancellation fence receipts in `services/control-plane/bff/tests/test_research_jobs_action_receipts.py` (U10B).
...
- **Owner Tests**: Existing test suites: `services/training-session/tests/test_http_service.py` (training session lifecycle, preview job evaluation proof, inbound authority), `services/training-session/tests/test_store_durability.py` (concurrent job updates and advisory lock durability). Proposed new tests: preview job cancellation suite to be delivered in `TS-CANCEL-001`.

#### 2.4 Source-ingest runs/jobs (`services/source_ingestion`)
- **Native Store & Persistence**: `JsonlIngestScheduleStore` (`services/source_ingestion/scheduler.py:281, 394–423`, bound via `runtime.py:267, 318` as `runtime.store`). Append/replay journal on disk with exclusive file locking (`exclusive_file_lock` with `.lock` file). Stores ingest runs (`IngestRun`), receipts (`IngestReceipt`), connector watermarks (`SourceWatermark`), and crawl frontier items (`CrawlFrontierItem`). `SourceManagementStore` in `source_management.py` is separate source-instance/control authority.
- **Concurrency & Durability Limitations**: Exclusive file locking (`exclusive_file_lock` and `threading.RLock`) serializes read/decide/append mutations to schedule JSONL store. Runs and receipts are durable upon append. Sequential execution per connector; checkpoint watermarks updated after verified batch extractions. No graceful extraction worker/thread cancellation endpoint; process abort leaves partial frontier batches for replay.
- **Native Endpoints**: `GET /api/source-ingest/jobs` and `GET /api/source-ingest/jobs/{ingest_run_id}` in `routers/ingest_operations.py:168–183` read `runtime.store.list_runs()`, `runtime.store.list_receipts()`, and `get_run`/`get_receipt`.
...
- **Owner Tests**: Existing test suites: `services/source_ingestion/tests/test_ingest_run.py` (ingest run lifecycle, completion events, transition validation, watermark restart), `services/source_ingestion/tests/test_scheduled_connector.py` (connector scheduling and watermark progression). Proposed new tests: extraction interrupt and cancellation fence suite to be delivered in `SI-CANCEL-001`.

#### 2.5 Policy-learning jobs (`services/policy-learning`)
- **Native Store & Persistence**: `PolicyLearningStore` configured via `build_policy_learning_store(data_dir)` (`store.py:1028–1060`). Employs dual backend:
  - File-backed JSON: `policy_learning_jobs.json` (`store.py:617`) in `data_dir` when `POLICY_LEARNING_STORE_BACKEND` is not `postgres`.
  - Authoritative PostgreSQL: `PostgresPolicyLearningJobStore` (`store.py:197–235`) storing jobs in table `policy_learning.jobs` (or `POLICY_LEARNING_STORE_TABLE`) with schema bootstrap when `POLICY_LEARNING_STORE_BACKEND=postgres`.
  - Candidate authority vs Job authority: In `build_policy_learning_store`, `resolve_candidate_authority` automatically upgrades the candidate store (`shadow_imitation_candidates`) to PostgreSQL if a DSN is configured, whereas the job store strictly requires `POLICY_LEARNING_STORE_BACKEND=postgres` (`store.py:1043–1049`).
- **Concurrency & Durability Limitations**:
  - In Postgres mode: ACID durability with schema bootstrap and primary key integrity on `job_id` in `policy_learning.jobs`.
  - In JSON mode: Local file `policy_learning_jobs.json` without cross-process locks.
  - Operational limitation: Reject handler updates JSON row status to `rejected` (`main.py:551–564`) and appends event, but background scheduler thread (`scheduler_worker.py`) does not terminate active training processes.
...
- **Owner Tests**: Existing test suites: `services/policy-learning/tests/test_policy_learning_http_service.py` (policy learning HTTP service lifecycle, stub replay), `services/policy-learning/tests/test_policy_learning_postgres_store.py` (JSON default vs PostgreSQL store backend configuration). Proposed new tests: worker process termination suite to be delivered in `PL-CANCEL-001`.
...
- **Owner Tests**: Existing test suites: `services/openclaw-gateway-adapter/test_tool_workflow_bridge.py` (tool policy, bridge audit log, workflow trigger, no live/paper broker execution), `services/openclaw-gateway-adapter/test_main.py` (adapter HTTP service routes). Proposed new tests: none; all write operations are permanently excluded per repository architecture.
...
| --- | --- | --- | --- | --- | --- | --- |
| **research_worker_gateway** | **Cancel** | `POST /api/research-worker-gateway/jobs/{id}/cancel` updates row status in `worker_jobs.json` and appends event to `worker_events`. Does NOT kill subprocess or set late fence. | Add subprocess kill (`SIGTERM`/`SIGKILL`) in `_execute_worker` and cancellation fence in `cancel_job`. | Gateway owner (`GW-STOP-FENCE-001`, outside U10B). | Existing: `services/research-worker-gateway/tests/test_research_worker_gateway_http_service.py`; Proposed cancel suite in `GW-STOP-FENCE-001` | No (in-scope for GW task) |
| **research_worker_gateway** | **Retry** | No retry API. New dispatch via `POST .../jobs` with idempotency key loses attempt lineage. | Add `POST /api/research-worker-gateway/jobs/{id}/retry` with `attempt_number=N+1` and `parent_job_id`. | Gateway owner (follow-up task). | Existing: `services/research-worker-gateway/tests/test_research_worker_gateway_http_service.py`; Proposed retry suite in follow-up task | No (standard domain retry) |
| **research_worker_gateway** | **Archive** | No archive API or retention policy. `worker_jobs.json` grows unboundedly. | Add `POST /api/research-worker-gateway/jobs/{id}/archive` with `is_archived=True`. Exclude from default list. | Gateway owner (follow-up task). | Existing: `services/research-worker-gateway/tests/test_research_worker_gateway_postgres_event_store.py`; Proposed archive suite in follow-up task | **Operator Decision Required** (retention period & archival authority) |
...
| **research_orchestrator** | **Cancel** | `services/research/main.py` has NO run cancel endpoint. | Implement `POST /api/research-orchestrator/runs/{run_id}/cancel`, persist `canceled` in `research_runs.json`, and set cancellation fence timestamp. | In scope for **U10B** (`services/research/main.py`, `write_owner.py`). | Existing: `services/research/tests/test_research_orchestrator_http_service.py`; Proposed U10B test: `services/control-plane/bff/tests/test_research_jobs_action_receipts.py` | No (in-scope for U10B) |
| **research_orchestrator** | **Retry** | Dispatches new run under task via `POST .../tasks/{task_id}/runs`, but lacks explicit `attempt_number` and `parent_run_id` tracking. | Implement explicit run retry linking `parent_run_id` and incrementing `attempt_number` under `rtask`. | In scope for **U10B**. | Existing: `services/research/tests/test_research_orchestrator_http_service.py`; Proposed U10B test: `services/control-plane/bff/tests/test_research_jobs_action_receipts.py` | No (in-scope for U10B) |
| **research_orchestrator** | **Archive** | No archive mutation endpoint, retention period, or authority exists. | Add `POST /api/research-orchestrator/runs/{run_id}/archive` setting `is_archived=True` in `research_runs.json`. | Research Orchestrator owner (follow-up task). | Existing: `services/research/tests/test_research_postgres_event_store.py`; Proposed archive suite in follow-up task | **Operator Decision Required** (artifact retention period & archival authority) |
| **research_orchestrator** | **Promote** | Proposal handoff exists (`POST .../tasks/{id}/proposals`), but lacks artifact hash verification, operator approval, target stage binding, and registry readback. | Enforce: (1) candidate artifact hash check, (2) signed approval token, (3) target stage binding, (4) registry readback verification. | Research Orchestrator owner + Governance. | Existing: `services/research/tests/test_research_orchestrator_http_service.py`; Proposed governance suite in follow-up task | **Operator Decision Required** (elevation to paper canary / live) |
| **training_session** | **Cancel** | `services/training-session/main.py` has NO cancel endpoint. `preview_eval_worker.py` runs without abort handle. | Implement `POST /api/training/preview-jobs/{job_id}/cancel`, interrupt worker thread, mutate record with advisory lock. | Training Session owner (`TS-CANCEL-001`, outside U10B). | Existing: `services/training-session/tests/test_http_service.py`; Proposed cancel suite in `TS-CANCEL-001` | No (in-scope for TS task) |
| **training_session** | **Retry** | Can re-queue preview job via `POST .../sessions/{id}/preview-jobs`, but lacks attempt linkage. | Add `attempt_number` to `preview_job` record in `authority_records` linking previous `job_id`. | Training Session owner (follow-up task). | Existing: `services/training-session/tests/test_store_durability.py`; Proposed retry suite in follow-up task | No (standard domain retry) |
| **training_session** | **Archive** | Preview jobs tied to session lifecycle; no independent preview job archive endpoint. | Add session/preview archival endpoint with `is_archived=True`. | Training Session owner (follow-up task). | Existing: `services/training-session/tests/test_store_durability.py`; Proposed archive suite in follow-up task | **Operator Decision Required** (evaluation dataset & replay log retention) |
| **training_session** | **Promote** | Direct promotion returns 409 Conflict. | Requires signed Governance review proposal, candidate weights verification, model stage binding, and Model Registry readback. | Training Session owner + Governance. | Existing: `services/training-session/tests/test_http_service.py`; Proposed governance suite in follow-up task | **Operator Decision Required** (elevation to paper canary / live) |
| **source_ingestion** | **Cancel** | `services/source_ingestion` has NO cancel endpoint. Ingest workers cannot be interrupted. | Implement `POST /api/source-ingest/jobs/{id}/cancel`, graceful connector extraction interrupt, late-receipt fence. | Source Ingestion owner (`SI-CANCEL-001`, outside U10B). | Existing: `services/source_ingestion/tests/test_ingest_run.py`; Proposed cancel suite in `SI-CANCEL-001` | No (in-scope for SI task) |
| **source_ingestion** | **Retry** | Replay via `POST /api/source-ingest/frontier/{frontier_id}/replay`. | Add `POST /api/source-ingest/jobs/{id}/retry` resuming from last verified watermark with attempt count. | Source Ingestion owner (follow-up task). | Existing: `services/source_ingestion/tests/test_ingest_run.py`; Proposed retry suite in follow-up task | No (standard domain retry) |
| **source_ingestion** | **Archive** | Connectors can be deactivated; raw ingest payload retention is unmanaged. | Add connector run archive endpoint; exclude from active frontier list. | Source Ingestion owner (follow-up task). | Existing: `services/source_ingestion/tests/test_scheduled_connector.py`; Proposed archive suite in follow-up task | **Operator Decision Required** (raw market data & document retention schedule) |
| **source_ingestion** | **Promote** | Seed extraction promotion to Strategy Seed Store. | Schema validation of candidate seed, write to `strategy_seeds`, and readback verification. | Source Ingestion owner (follow-up task). | Existing: `services/source_ingestion/tests/test_scheduled_connector.py`; Proposed seed promotion suite in follow-up task | **Operator Decision Required** (if seed alters live execution feeds) |
| **policy_learning** | **Cancel** | `POST /api/policy-learning/jobs/{id}/reject` mutates row status to `rejected`, but does NOT halt worker processes. | Add actual worker process kill/stop and cancellation fence in `reject_job`. | Policy Learning owner (`PL-CANCEL-001`, outside U10B). | Existing: `services/policy-learning/tests/test_policy_learning_http_service.py`; Proposed worker stop suite in `PL-CANCEL-001` | No (in-scope for PL task) |
| **policy_learning** | **Retry** | DLQ replay or new job submission. | Add `POST /api/policy-learning/jobs/{id}/retry` incrementing attempt count and referencing parent job ID. | Policy Learning owner (follow-up task). | Existing: `services/policy-learning/tests/test_policy_learning_postgres_store.py`; Proposed retry suite in follow-up task | No (standard domain retry) |
| **policy_learning** | **Archive** | Policy learning jobs stored without archiving mechanism. | Add policy job archive endpoint with `is_archived=True`. | Policy Learning owner (follow-up task). | Existing: `services/policy-learning/tests/test_policy_learning_postgres_store.py`; Proposed archive suite in follow-up task | **Operator Decision Required** (model checkpoint & replay buffer retention) |
| **policy_learning** | **Promote** | Direct promotion returns 409 Conflict (requires Governance). | Governance promotion gate: signed authorization, metric threshold check, stage binding, and readback. | Policy Learning owner + Governance (`GOV-PROMOTE-001`). | Existing: `services/policy-learning/tests/test_policy_learning_http_service.py`; Proposed governance suite in follow-up task | **Operator Decision Required** (live trading capital allocation) |
| **openclaw_gateway_adapter** | **All Actions** | Upstream OpenClaw owns execution. Product BFF is strictly read-only diagnostics (`GET .../workflows/jobs/{id}`). | All write actions (cancel, retry, archive, promote) are unsupported and return HTTP 400 Bad Request. | Permanently excluded per repository architecture. | Existing: `services/openclaw-gateway-adapter/test_tool_workflow_bridge.py`, `services/openclaw-gateway-adapter/test_main.py` | No (permanently read-only) |
...
| **source_ingestion** | `services/source_ingestion/` (`JsonlIngestScheduleStore` via `runtime.py:267, 318` as `runtime.store` for runs/receipts, `routers/ingest_operations.py:168–183`; `SourceManagementStore` is separate source-instance/control authority) | `GET /api/source-ingest/jobs`<br>`GET .../jobs/{id}`<br>`POST .../jobs` | `GET /bff/jobs` (`job-ingest-*`)<br>`GET /bff/jobs/{id}` | Read projection in U10A. | Cancel: **NO cancel endpoint exists**. Actions fail closed with `ActionUnavailableError` (HTTP 400/503).<br>Archive: Unmanaged.<br>Promote: Requires seed verification. | Read: U10A.<br>Cancel closure: **`SI-CANCEL-001`** (Source ingestion owner). | **Yes** (raw data retention schedule & live feed promotion) |
| **policy_learning** | `services/policy-learning/` (`PolicyLearningStore` with dual backend: `policy_learning_jobs.json` or `PostgresPolicyLearningJobStore` table `policy_learning.jobs` when `POLICY_LEARNING_STORE_BACKEND=postgres`, `main.py:189-196, 551-564`) | `GET /api/policy-learning/jobs`<br>`POST .../jobs/{id}/reject` | `GET /bff/jobs` (`job-policy-*`)<br>`GET /bff/jobs/{id}` | Read projection in U10A. | Reject/Cancel: Row-only in gateway; does not stop worker. Actions fail closed with `ActionUnavailableError`. Direct promote rejected (409). | Read: U10A.<br>Worker stop: **`PL-CANCEL-001`** (Policy learning owner). | **Yes** (checkpoint retention & capital allocation approval) |
...
1. **Gateway Concurrency & Durability Failure (`research-worker-gateway`)**:
   ...
   - Tested in: Existing suites `services/research-worker-gateway/tests/test_research_worker_gateway_http_service.py` and `test_research_worker_gateway_postgres_event_store.py`.
2. **Orchestrator Concurrency & Late Completion Race (`services/research`, U10B)**:
   ...
   - Tested in: Existing suite `services/research/tests/test_research_orchestrator_http_service.py`; Proposed U10B receipt suite `services/control-plane/bff/tests/test_research_jobs_action_receipts.py`.
3. **Training Session Advisory Lock & Durability Failure (`training-session`)**:
   ...
   - Tested in: Existing suites `services/training-session/tests/test_store_durability.py` (`test_shared_store_keeps_all_concurrent_job_updates`) and `services/training-session/tests/test_http_service.py`.
4. **Source Ingestion Checkpoint Resume Failure (`source_ingestion`)**:
   - Extraction thread interruption before batch watermark commit ensures uncommitted frontier items remain in queue; connector restart resumes strictly from last verified checkpoint in `JsonlIngestScheduleStore`.
   - Tested in: Existing suites `services/source_ingestion/tests/test_ingest_run.py` (`test_scheduled_ingest_resumes_from_persisted_watermark_after_restart`) and `services/source_ingestion/tests/test_scheduled_connector.py`.
5. **Policy Learning Reject Race (`policy_learning`)**:
   ...
   - Tested in: Existing suites `services/policy-learning/tests/test_policy_learning_http_service.py` and `services/policy-learning/tests/test_policy_learning_postgres_store.py`.
