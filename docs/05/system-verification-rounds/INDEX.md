...
| 007 | Service-layer suites (loops #1-#15 implementations) + v5 confirm-token diagnosis | ~600 service tests all GREEN (optimizer/evolution/governance/consultation/telemetry/runtime-manager/foundation/research-gw/broker); rot was isolated to BFF contract layer | Verified service layer healthy; v5 confirm-token precisely diagnosed (precedence confirm-token->two-man; tests need bound-token mint) as harness backlog | shipped -> dev |
| 008 | BFF suite census + authz failures deep-verify | suite down to 5 heterogeneous failures post-R003-007; 2 deprecated-route (stale), 2 authz 200-vs-403, 1 field drift | VERIFIED the 2 authz failures are NOT security bugs (viewer is a read role by design; all operator read-surfaces use read-role) - rejected a harmful authz tightening; 5 precisely diagnosed for owner | shipped -> dev |
| 010 | LIVE authed loop-state (dev BFF stub auth: any Bearer accepted) | OODA loop DEPLOYED but NOT CLOSED: 15 paper bindings active 11 days, but 0 loop-runs / 0 trades / empty synthesized telemetry / 0 evolution-incident-approval; LEAN continuous loop not producing runs | Headline finding documented + escalated (runtime-side, not BFF code); plan current_stage=none vs binding active = consistency follow-up | shipped -> dev |
| 011 | Live referential integrity (read-only, stub auth) | binding->pool refs clean (0 dangling); 15/15 plans current_stage=none (systemic, confirms R010); 8/23 pools unbound; /api/v1/artifacts returns data=null not [] | Verified left-half internally consistent; systemic current_stage gap + artifacts null-list as follow-ups | shipped -> dev |
