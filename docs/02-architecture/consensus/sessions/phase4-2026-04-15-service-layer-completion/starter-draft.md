...
- SVC-BASELINE locked contract (2026-04-28):
  ...
  - Explicit deferrals:
    ...
    - `consultation`, `source_ingestion`, and `search` are explicitly outside the default single-VM compose baseline for this wave:

      | Component | Repo state | Single-VM baseline disposition | Downstream implication |
      |---|---|---|---|
      | `services/consultation/` | FastAPI app, `/health`, append/replay store, and Dockerfile exist, but root compose does not run it and BFF consultation reads currently use a local `ConsultationStore` data-dir adapter rather than a network service client. | Code exists; deployable service activation deferred. Do not add it to the default compose profile until SVC-SURFACES or a follow-up task chooses the BFF integration boundary: HTTP client preferred, explicit shared-store read boundary only if accepted separately. | SVC-SURFACES must not make consultation a hidden default dependency. Consultation-backed surfaces may report degraded/unavailable in the normal no-snapshot path until the deferred service boundary is implemented. |
      | `services/source_ingestion/` | Connector, ingest-run manager, scheduler, persisted watermarks, and DLQ/audit routing exist as library code; no HTTP entrypoint, service Dockerfile, health endpoint, port, or compose contract exists. | Deployable service deferred. | SVC-COMPOSE must not include a `source-ingestion` service in the default profile; later work must create the HTTP/job-trigger wrapper, Dockerfile, storage contract, and smoke criteria first. |
      | `services/search/` | Governed search gateway, filters, keyword retriever, index adapter, and JSONL index snapshot store exist as library code; no HTTP entrypoint, service Dockerfile, health endpoint, port, or compose contract exists. | Deployable service deferred. | SVC-SURFACES should treat research/search data as unavailable/degraded unless backed by existing BFF-local data for explicitly test-only paths; later work must add a real search API wrapper before compose inclusion. |
- Proposed wave order:
