...
| Reviewer | Codex |
| Wave | 1 |
| Fleet lane | `deployment-dispatcher` |
| Repository | `pantheon` |
| Merge target | `dev` |
| Current maturity | api-only receipt consumer without apply side effect |
| Target maturity | product-level |
| Human/Ops final sign-off | not a separate ordinary task |

## Product outcome

讓 deployment outbox worker 真正呼叫 runtime_manager_dispatch_adapter/canonical runtime authority，而非 receipt-only；成功前讀回 RuntimeBinding/post-state，支援 retry/DLQ/replay/restart/compensation/kill-wins。

Next action: Replace receipt-only deployment with canonical target dispatch, binding readback, default service ownership, and recovery/rollback proof.
...
- `DeploymentPlan and environment policy`
- `kill/safe-mode state`

## Actual-state authorities

- `durable deployment outbox`
- `runtime-manager command/receipt`
- `RuntimeBinding and target post-state`
- `controller record`
...
- `docker-compose.yml`
- `docs/deployment/evidence/loop-product-level/LOOP-PROD-DEP-001`
...
- default dispatcher calls the canonical runtime authority and persists its terminal receipt
- success requires authoritative RuntimeBinding/post-state readback; submitted is never shown running
- duplicate, downstream-success-before-receipt, timeout, DLQ/replay, restart, compensation, rollback, and kill-wins pass
- unsafe environment, unapproved artifact, missing MFA/two-person, and live-capital attempts fail closed
