...
| Reviewer | Codex |
| Wave | 2 |
| Fleet lane | `trade-journey-actions-be` |
| Repository | `pantheon` |
| Merge target | `dev` |
| Current maturity | default action dispatcher unavailable |
| Target maturity | product-level |
| Human/Ops final sign-off | not a separate ordinary task |

## Product outcome

取代 default ACTION_DISPATCH_UNAVAILABLE；pause/cancel/escalate/retry/ack 依 action type 呼叫 canonical authorities，回傳 terminal receipt/refetch，保留 RBAC/MFA/idempotency/stale/live gate/partial failure。

Next action: Implement the governed action dispatcher, authority mapping, terminal receipt/refetch, and full safety/recovery matrix.
...
- `Trade Journey action intent`
- `journey revision and governance policy`

## Actual-state authorities

- `canonical authority command`
- `terminal receipt/post-state`
- `journey refetch and audit`
...
- `services/evolution`
- `docs/deployment/evidence/loop-product-level/LOOP-PROD-TJ-001`
...
- each supported action maps to its canonical authority and returns terminal post-state readback
- ACTION_DISPATCH_UNAVAILABLE is not the production default when the authority is healthy
- RBAC, MFA, idempotency, stale revision, live-action feature gate, wrong tenant, and partial downstream failure pass
- duplicate and downstream-success-before-receipt recovery produce no duplicate effect
