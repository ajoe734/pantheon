...
| Reviewer | Codex2 |
| Wave | 1 |
| Fleet lane | `paper-signal-runtime` |
| Repository | `pantheon` |
| Merge target | `dev` |
| Current maturity | manual signal injection around a partial paper runtime |
| Target maturity | product-level |
| Human/Ops final sign-off | not a separate ordinary task |

## Product outcome

建立非 SmokeStrategy/手工 script 的 first-class DecisionSignalProducer，發現 eligible paper bindings，攜帶 tenant/persona/binding/runtime/pool identities，確保 exactly-one worker、queue isolation、stop/restart，live broker/capital fail closed。

Next action: Ship the first-class paper signal producer, binding discovery, exactly-one ownership, isolation, and full paper event/readback proof.
...
- `eligible paper RuntimeBinding`
- `paper signal policy and safe-mode state`
...
- `binding-scoped queues and controller record`
...
- `docker-compose.yml`
- `docs/deployment/evidence/loop-product-level/LOOP-PROD-CAP-001`
...
- default bounded producer discovers only eligible paper bindings and emits schema-valid identity-complete signals
- one binding has exactly one supervised worker and queues cannot leak across bindings
- real paper signal reaches decision, order, fill, position, heartbeat, stop, and restart readback
- all evidence asserts is_real_order=false and is_real_capital=false; live broker/capital paths remain fail closed
