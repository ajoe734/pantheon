...
Operationally, validation requires `verify-full` TLS on the pinned PostgreSQL service, non-symlinked and non-group/world-writable projection-root and `generations/` directories, regular immutable generation files, network access to the authoritative Git remote/GitHub API, and a caught-up live projector. The future compatible publisher must hold `.projection-publish.lock` across generation creation, `current` switch, post-switch verification, and rollback, and must refuse rollback over a newer `current` inode. The current `dev` projector still emits a mutable `gNNNNNNNNNNNN-<12-hex UUID>` generation, so it cannot satisfy this gate; delivering the content-addressed publisher belongs to `LOOP-PROD-TEL-002`, and G2 stays closed until that dependency lands. The SQL identity predicate uses the projector's field-specific envelope/root/metadata precedence for all 16 stable identity fields, and the current generation must retain the exact captured target event chain plus loop-run core at the same database high-water mark. Missing credentials, plaintext/current compose defaults, unavailable authority, shallow/replaced/alternate Git object state, redirected projection parents, divergent current target content, or stale controller state fail closed; no evidence is fabricated or inferred.
...
- Current `dev` projection publication is not the required immutable content-addressed publisher. `LOOP-PROD-TEL-002` must deliver and validate that producer before real G2 admission; the test-only repackaging helper proves verifier behavior only and is not runtime-readiness evidence.
