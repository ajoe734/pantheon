...
Here, `permitted before the paper-trade proof` means the task may be admitted and materialized before G2; the ordinary dependency scheduler still applies. In particular, `LOOP-PROD-PPL-001` and `LOOP-PROD-TJ-003` retain `LOOP-PROD-VERIFY-EXEC-001`, so they are visible before G2 but cannot run until that proof task is done. They are not part of the strict-auth/security release set.
...
- The durable source attestation binds database host/port/server address/TLS protocol/cipher, projection root, source high-water mark, captured/current checkpoint and projection-source watermark, captured/current generation names, and canonical row/captured/current-projection digests into the release admission. The admission contract accepts only the canonical publisher's `gNNNNNNNNNNNN-<12-hex UUID>` generation grammar. Generation bundles may retain their umask-dependent owner/group-write bits, but world-writable paths, symlinks, or any device/inode/size/mtime/ctime change during the stable read are rejected; the manifest journey/loop digests are recomputed and the complete projection digest is bound into the attestation.
...
Operationally, validation requires `verify-full` TLS on the pinned PostgreSQL service, a non-symlinked and non-group/world-writable projection root, world-write-disabled `generations/` and generation bundle paths, network access to the authoritative Git remote/GitHub API, and a caught-up live projector. The verifier accepts the canonical publisher's exact `gNNNNNNNNNNNN-<12-hex UUID>` generation grammar and its umask-dependent owner/group-writable bundle files, but stable-reads the generation and every file by device, inode, size, mtime, and ctime; recomputes the manifest journey/loop digests; and content-binds the complete projection in the source attestation. The SQL identity predicate uses the projector's field-specific envelope/root/metadata precedence for all 16 stable identity fields, and the current generation must retain the exact captured target event chain plus loop-run core at the same database high-water mark. Missing credentials, plaintext/current compose defaults, unavailable authority, shallow/replaced/alternate Git object state, redirected projection parents, divergent current target content, world-writable paths, concurrent generation mutation, or stale controller state fail closed; no evidence is fabricated or inferred.

### Accepted closeout-truth admission

The verifier admits the target only when all of the following are true:

- The active or safely archived task is terminal `done` with a completed outcome, exact task/program/source/addendum/PR/overlay/task-contract provenance, no pending status archive/activity outbox, and one content-addressed owner `done` audit whose timestamp, message, and delivery exactly match `last_update`, `next`, and `delivery.recorded_at`. Generic owner closeout delivery identifies the merged target; the independent review binding separately identifies the artifact PR head. GitHub must report both implementation and artifact PRs merged to `dev`, their exact protected workflow unchanged, and their required checks from one successful `pull_request` workflow run. Merge parents, remote ancestry, verifier source bytes, bytes at artifact head/merge/captured target/current remote, and product manifest must all agree.
...
- The positive resolver regression consumes the canonical `AtomicProjectionBundle` output directly, including its 12-hex UUID generation and owner-writable files; no test-only repackaging is admitted. This proves verifier/producer format compatibility only. `LOOP-PROD-TEL-002` must still deliver the real hosted lifecycle evidence, and no fixture is runtime-readiness evidence.
