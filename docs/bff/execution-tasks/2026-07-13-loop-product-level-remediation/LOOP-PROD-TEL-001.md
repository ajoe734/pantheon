...
| Reviewer | Codex |
| Wave | 1 |
| Fleet lane | `telemetry-reconciliation-controller` |
| Repository | `pantheon` |
| Merge target | `dev` |
| Current maturity | api-only and opt-in fixture-oriented consumers |
| Target maturity | product-level |
| Human/Ops final sign-off | not a separate ordinary task |

## Product outcome

預設啟動 scheduler、consumer、incident listener，以真實 runtime/operator telemetry 比對 authoritative actual state，產生 DriftReport 與 dedup Incident；不可 empty-green/fixture，需 retry/DLQ/replay/restart/lag truth。

Next action: Activate and productize the real telemetry-to-drift-to-incident chain with failure truth and recovery evidence.
...
- `runtime and deployment desired state`
- `reconciliation policy and incident thresholds`

## Actual-state authorities

- `durable telemetry`
- `authoritative runtime/operator state`
- `DriftReport`
- `IncidentCase and controller records`
...
- `services/incident`
- `docker-compose.yml`
- `docs/deployment/evidence/loop-product-level/LOOP-PROD-TEL-001`
...
- default services consume real telemetry and compare non-empty authoritative actual sources
- drift persists a report and one deduplicated incident with shared identity/correlation
- empty or unavailable actual source reports degraded/failure and never green
- retry, DLQ/replay, backlog/lag, incident recovery, worker/DB/full-stack restart, and controller truth pass
