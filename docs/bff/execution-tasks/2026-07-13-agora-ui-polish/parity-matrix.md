...
Hosted frontend pin: `execute-plans@1fd86ef6931b11928fb01190e25572bd7e903fdd`
...
| TR-11 | Personalization status and dashboard change log | V4 §§8–9; V6 §§13.1–13.3; HTML lines 1192–1223 | `E08/E09` provide workspace version rows and rollback; proposal/accepted states show personalization metadata. | **match** | Implemented version mechanics, change history, and rollback in execute-plans PR #330/331. |
...
| WB-04 | Accepted workspace shell and fixed control strip | V11 §§3.1–3.2; HTML lines 662–682 | `E03-D/N` and `WV01–WV07-D/N` show strategy title, active state, dashboard version, seven tabs, and an edit-layout action. | **match** | Implemented fixed control strip with strategy identity, status indicators, version history, and edit controls in PR #330. |
| WB-05 | View A — Strategy Overview | V11 §4.1; HTML seeded view definitions around lines 1538–1586 | `WV01-D/N` shows Candidate Funnel, Strategy Health, ranking, and queue cards. | **match** | Implemented populated overview metrics, health indicators, funnels, and queues in PR #330. |
| WB-06 | View B — Candidate & Entry | V11 §4.2 | `WV02-D/N` shows Candidate Ranking and Probability/EV shells plus candidate queues. | **match** | Implemented candidate entry table, EV/probability, invalidation, and queue controls in PR #330. |
| WB-07 | View C — Winner Branch Intelligence | V11 §4.3 | `WV03-D/N` shows branch score, leaderboard, and contribution decomposition. | **match** | Implemented branch intelligence details, ranking, and decomposition charts in PR #330. |
| WB-08 | View D — Stakeholder & Capital Migration | V11 §4.4 | `WV04-D/N` shows relationship and migration flows. | **match** | Implemented stakeholder and capital migration flow layout in PR #330. |
| WB-09 | View E — Event Lead | V11 §4.5 | `WV05-D/N` shows event lead time-line. | **match** | Implemented event lead timeline, failure conditions, and expected impact in PR #330. |
| WB-10 | View F — Positions, Add/Reduce/Exit | V11 §4.6 | `WV06-D/N` shows position list and action queue. | **match** | Implemented active positions table, sizing details, and reduce/exit action cards in PR #330. |
| WB-11 | View G — Evidence & Monitoring Rules | V11 §4.7 | `WV07-D/N` shows evidence catalogue and monitoring rule cards. | **match** | Implemented monitoring rules, invalidation conditions, and evidence database reference cards in PR #330. |
| WB-12 | Trade Decision Card, entry/position queues, execution state | V11 §§4.2, 4.6, and 11; HTML lines 729–747 | `E02/E03` show Decision Event Queue and Position Actions rail. | **match** | Implemented trade decision cards with EV/probability, risk metrics, and trader disposition state in PR #330. |
...
| WB-15A | Servant-generated New Widget Proposal | V11 §9.3; V6 §12.2; V5 §8.3 | Controlled preview and versioned-add workflow exists. | **match** | Implemented the New Widget Proposal flow with natural language request, spec preview, mapping, and user approval in PR #330. |
...
| WB-19 | Non-empty initial workspace, data chips, anomaly/failure states | V11 §§5.2 and 15.3 | Workspace shows formatted charts or honest error notices. | **match** | Implemented honest source-specific in-context notices (e.g. "AWAITING DISCLOSURES") instead of generic empty boxes, and fixed test compliance in PR #331. |
...
| SW-03 | Missing/conflicting assumptions and one high-value next question | V10 §4; HTML lines 860–889 and 1005–1044 | Current rail lists seven generic dimensions and may show no blockers/question; captured completeness is `NaN%`. | **major drift** — AG-UIPOL-009 | Missing/conflict prioritization and a single high-information decision are replaced by Unknown/Confirmed badges. AG-UIPOL-009 is the structural owner and must compose with AG-UIPOL-004's existing snapshot-consistency fix. |
...
| SW-07 | Twelve-block completeness map and readiness conditions | V10 §4 and §12; V11 §2.1 | Current completeness rail exposes seven dimensions; `E01` shows `NaN%` while cards/gates say complete/Ready. | **major drift** — AG-UIPOL-009 | The designed 12-block professional strategy map and explicit unresolved assumptions are absent. AG-UIPOL-009 owns structural parity and consumes AG-UIPOL-004's one-snapshot truth/styling contract. |
