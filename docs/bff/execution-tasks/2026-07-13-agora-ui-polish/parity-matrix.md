...
| G-06 | Narrow task-focused shell | BASE §4.2; V4 Screen 10; V6 §16F | Narrow workflow captures are 9,000–16,951 physical pixels tall; `ES-P-N` overlays correctly but underlying Workshop content still exceeds the viewport width. | **major drift** — AG-UIPOL-011 | The source has behavioral, not pixel, authority: narrow should prioritize reminders, decisions, and task progress, stack cards, and collapse controls. Current pages largely shrink/stack the full desktop payload and create extreme traversal. |
...
| WB-04 | Accepted workspace shell and fixed control strip | V11 §§3.1–3.2; HTML lines 662–682 | `E03-D/N` shows strategy title, active state, dashboard version, seven tabs, and an edit-layout action. | **major drift** — AG-UIPOL-008 | Missing compact data-source/readiness/risk/pending-decision status, Workshop/Servant/version entrances, and fixed operator controls. The long raw strategy ID is the primary title and version history is pushed into the scroll body. |
| WB-05 | View A — Strategy Overview | V11 §4.1; HTML seeded view definitions around lines 1538–1586 | `E03` shows Candidate Funnel, Strategy Health, ranking, and queue cards. Most chart bodies say “No chart data is available for this WidgetSpec.” | **major drift** — AG-UIPOL-008 | The intended ten-second strategy picture is not achieved: health, candidate funnel, position/risk, event, and migration signals are labels/placeholders rather than a populated overview. |
| WB-06 | View B — Candidate & Entry | V11 §4.2 | The tab and three generated widget shells exist; the proposal describes ranking/readiness/probability-EV/time-axis content. | **major drift** — AG-UIPOL-008 | Missing dense candidate table, fit/readiness, branch evidence, probability/EV, invalidation, entry queue actions, and visible decision context in the accepted surface. |
| WB-07 | View C — Winner Branch Intelligence | V11 §4.3 | The tab and four widget shells exist. | **major drift** — AG-UIPOL-008 | Missing branch ranking, contribution decomposition, history horizon, sample reliability, and evidence-rich drilldown; generic registry shells do not convey the designed intelligence. |
| WB-08 | View D — Stakeholder & Capital Migration | V11 §4.4 | The tab and generated shells exist. | **major drift** — AG-UIPOL-008 | Missing stakeholder/branch relationship map, migration signal, before/after flow, corroboration, and false-positive context. |
| WB-09 | View E — Event Lead | V11 §4.5 | The tab and generated shells exist. | **major drift** — AG-UIPOL-008 | Missing lead/lag event timeline, abnormal activity, expected impact, confidence, and failure condition. |
| WB-10 | View F — Positions, Add/Reduce/Exit | V11 §4.6 | The tab exists; a separate right rail says “No open positions.” | **major drift** — AG-UIPOL-008 | Missing position table, thesis state, add/reduce/exit proposal, risk/capacity, evidence link, and governed position action workflow inside the designed view. |
| WB-11 | View G — Evidence & Monitoring Rules | V11 §4.7 | The tab and generated shells exist. | **major drift** — AG-UIPOL-008 | Missing evidence catalogue, monitoring rule, invalidation, freshness/reliability, next check, and audit-readable rule hierarchy. |
...
| WB-14 | Widget menu and explain/change/remove actions | V11 §7; V4 §6; HTML lines 719–724 | `E04/E05` and deployed `WorkspaceGridEditor` expose menu, change-chart variants, and remove/restore behavior. | **major drift** — AG-UIPOL-008 | The menu lacks the designed “why this is shown” evidence/explanation path and the Servant-mediated change entrance; raw chart-type controls dominate an operator-level interaction. |
| WB-15 | Add Widget and restore library | V11 §9; HTML lines 1301–1325 | The deployed editor has Add Widget, categorized registry entries, and a restore library for removed widgets. | **minor drift** | Core library mechanics exist, but the hosted evidence does not show a Servant-generated new-widget proposal or the designed data-source/validation preview. |
...
| WB-19 | Non-empty initial workspace, data chips, anomaly/failure states | V11 §§5.2 and 15.3 | `E03–E09` show registry-validated shells, but prominent white boxes report no chart data; narrow states become exceptionally tall. | **major drift** — AG-UIPOL-008 | V11 explicitly forbids making the trader build the initial desk and requires missing/delayed/anomalous data to be expressed in-context. Current generic empty renderer boxes break hierarchy and leave most of the workspace informationally blank. AG-UIPOL-003 owns availability derivation, not the missing view content. |

## Strategy Workshop — V10 expert dialogue

| Row | Designed surface/state | Design reference | Current hosted state | Verdict / owner | Concrete differences |
|---|---|---|---|---|---|
| SW-01 | New/blank professional strategy description | V10 §§1–2 and §13; V6 §14.1; HTML lines 825–858 | `/agora/strategy-workshop` opens a list of existing workshops and auto-selects one; no visible new-strategy blank entry is present in `E01/ES-P`. | **missing** — AG-UIPOL-009 | Missing a calm, large professional-description entry with examples and a clear start-discussion action. The operator cannot begin the designed workflow from this tab. |
| SW-02 | Dialogue canvas and first Strategy Reconstruction Card | V10 §§2–3; HTML lines 825–858; `screenshots/01-v10-mid.png`, `02-v10-mid.png` | `E01/ES-P` use a three-column list/conversation/completeness layout with generic event cards. | **major drift** — AG-UIPOL-009 | Designed first response reconstructs strategy core, research subproblems, recognized components, and non-assertable claims in one expert card. Current “description captured / completeness update / readiness gate” cards expose workflow telemetry instead. |
| SW-03 | Missing/conflicting assumptions and one high-value next question | V10 §4; HTML lines 860–889 and 1005–1044 | Current rail lists seven generic dimensions and may show no blockers/question; captured completeness is `NaN%`. | **major drift** — AG-UIPOL-009 plus existing AG-UIPOL-004 | Missing/conflict prioritization and a single high-information decision are replaced by Unknown/Confirmed badges. AG-UIPOL-004 already owns contradictory readiness/completeness rendering. |
| SW-04 | Winner Branch research result cards | V10 §§5–7; HTML lines 891–967 | No relationship-probability mapping, Winner Branch score/version, branch migration, or event-lead research card appears in `E01`. | **missing** — AG-UIPOL-009 | The shipped cards do not expose method, sample, confidence, caveat, evidence, or research conclusions required for expert review. |
| SW-05 | Probability/EV and position-sizing discussion | V10 §§8–9 | No probability interval, expected value, conservative/balanced/aggressive sizing, correlation, capacity, or leverage discussion appears. | **missing** — AG-UIPOL-009 | Readiness gates cannot substitute for the designed decision dialogue and risk trade-off. |
| SW-06 | Literature/similar-alpha, backtest result, critique, and version dialogue | V10 §§10–11; HTML lines 968–1003 | No literature/analogue card, backtest result card, critique, scenario comparison, or versioned strategy response appears. | **missing** — AG-UIPOL-009 | Missing formal result provenance, out-of-sample/cost/robustness context, failure modes, and user response loop. |
| SW-07 | Twelve-block completeness map and readiness conditions | V10 §12; V11 §2.1 | Current completeness rail exposes seven dimensions; `E01` shows `NaN%` while cards/gates say complete/Ready. | **major drift** — AG-UIPOL-009 plus existing AG-UIPOL-004 | The designed 12-block professional strategy map and explicit unresolved assumptions are absent. AG-UIPOL-009 owns structural parity; AG-UIPOL-004 owns one-snapshot truth/styling. |
...
| PF-07 | Measured zero vs not reported; explained Unassigned bucket | V5 §6.4 data semantics; V6 §15.3 | `EP-D/N` put named strategies before Unassigned, label missing named metrics “not reported,” retain `$0` only for measured Unassigned PnL, and explain the aggregation. | **match** | The captured deploy already demonstrates the honesty outcome scoped by AG-UIPOL-004. Source-health/readiness work for that task can close independently. |
| PF-08 | Narrow decision-focused Performance view | V6 §16F | `EP-N` stacks KPI and source-health cards, then renders a horizontally clipped minimum-width table inside a very long page. | **major drift** — AG-UIPOL-011 | Narrow should foreground anomalies, interventions, and decisions. Current source metadata and desktop table structure consume the viewport before any operator action. |
...
| 2 | [AG-UIPOL-008 — Winner Branch workspace information parity](./AG-UIPOL-008-winner-branch-workspace-information-parity.md) | TR-11, WB-04–WB-12, WB-14, WB-19 | The core execution desk exists but does not yet provide the designed decision information. |
...
| 6 | [AG-UIPOL-011 — narrow responsive task parity](./AG-UIPOL-011-narrow-responsive-parity.md) | G-06, PF-08, SRV-03 | Applies the source's behavioral narrow rules after each desktop information architecture has a stable owner. |
...
| existing | AG-UIPOL-004 | SW-03, SW-07; PF-07 | One-snapshot Workshop state and Performance zero/not-reported truth. |
