...
| Pantheon 基線 | `origin/dev@1095c55bf42acc91fac18b701cd24ad5b1874438`（含 PR #5430 DOC-DEVTOOL-FOUR-GAP-COMPLETE-SD-20260830、PR #5426 Agora baseline 500 修復、PR #5425 Agora projection receipt 綁定、PR #5424 TW bounded refresh 讀回、PR #5417 TW market session 新鮮度、PR #5411 Source frontier 範圍修復） |
| execute-plans 基線 | `origin/dev@bd03c863e3c2c1c64b9b7797f27cefaf84df17c1`（含 PR #694 evidence manifest，前端 UI 程式碼同 `5ffee3db8` Workshop tenant 修正） |
| Hosted live BFF | `e7f010dccee33185bc260d06048f09e6d2125f28`（VM `pantheon-lupin-dev` IP `35.201.204.12`，PostgreSQL checkpoint `7,649,369`，backlog 0，quarantine 0，ready/live） |
| Hosted live FE | `bd03c863e3c2c1c64b9b7797f27cefaf84df17c1`（pairId `0429052b...`，profile `read-only`，`VITE_BFF_MODE=live`，`VITE_BFF_FALLBACK=strict`，`VITE_BFF_REAL_WRITES=false`） |
...
本規劃包（`FULL-OPERATION-GAP-SA-SD-PLAN-FREEZE-20260830`）旨在將 2026-08-29 全產品運作稽核所識別之 **OP-G01 至 OP-G20** 共 20 項 Canonical GAP，以最新 `origin/dev`、`execute-plans` dev、hosted runtime 與 canonical task store 現狀為基準，進行嚴謹處置分類、目標系統分析（SA）、系統設計（SD）、並產出可平行化且無相依環（acyclic）之執行 DAG 與任務目錄（Execution Task Catalog）。
...
2. **不重工與歷史事實不可變（No Redundant Work & Immutable Task History）**：
   ...
   - 針對 PR #5410~#5426 已在 `origin/dev` 完成之 source-level 修復，明確定位為「source 已修復，待 live atomic switch 與 hosted 閉環驗證」，避免重複開發。
3. **延伸證據之根因歸併與非功能性觀察項記錄**：
   - **OP-G21 歸併至 OP-G08**：BFF 多副本測試失敗、router `import main` 與 18 個重複 operation IDs 歸併至 OP-G08 BFF 組裝驗收。
   ...
   - **OP-G25 歸併至 OP-G04 / OP-G16**：Release workflow 0-job 失敗與 fail/skip 處置歸併至部署可靠度與門禁 exit-code 強化，排除無關之 GitHub branch protection / 資安管理作業。
...
4. **熱點檔案獨占擁有（Exclusive Hot-File Ownership）**：
   - 針對跨任務共用檔案（如 `services/control-plane/bff/main.py`、`execute-plans/src/App.tsx`、`execute-plans/src/lib/bff-v1/index.ts`、`docker-compose.yml`、`scripts/deploy_nonprod_vm.sh`），在 Wave 1 僅由領域子模組獨立實作，並指定單一整合任務在 Wave 2/3 進行切換，防止多 worker 同時編輯造成衝突。
5. **共享執行資源模型化（Execution Resource Modeling）**：
...
6. **Supervisor Clone Session 平行機制與註冊身分邊界**：
   - 任務擁有者全面指派給具備 active capacity 之註冊身分 `Antigravity` 與 `Antigravity2`。
...
7. **功能優先邊界（Functional-First Boundary & Non-Goals）**：
...
| [`CURRENT_GAP_DISPOSITION_2026-08-30.md`](CURRENT_GAP_DISPOSITION_2026-08-30.md) | 落差處置矩陣 | OP-G01 至 OP-G20 逐項處置、直接證據、延伸證據歸併、延後觀察項與單一任務歸屬。 |
| [`SA_GAP_REMEDIATION_2026-08-30.md`](SA_GAP_REMEDIATION_2026-08-30.md) | 目標系統分析 (SA) | 6 大子系統之單一 owner 架構、資料流、狀態不變量與邊界規範。 |
| [`SD_GAP_REMEDIATION_2026-08-30.md`](SD_GAP_REMEDIATION_2026-08-30.md) | 系統設計規格 (SD) | 14 個設計單元（Design Units）之程式碼面、DTO 契約、狀態轉移與測試規範。 |
| [`EXECUTION_DAG_2026-08-30.md`](EXECUTION_DAG_2026-08-30.md) | 平行執行 DAG | 5 波次（Wave 0~4）相依圖、熱點檔案擁有者分配、資源鎖定與交付排程。 |
| [`EXECUTION_TASK_CATALOG_2026-08-30.json`](EXECUTION_TASK_CATALOG_2026-08-30.json) | 機器可讀任務目錄 | 1 個凍結任務 + 14 個實作/整合任務之完整 V2 TaskStore 契約 JSON。 |
...
| **OP-G08** | P1 | BFF `main.py` 巨型路由未完成拆分；多副本依賴與 operation IDs 重複 | 主動修復 (Active Remediation, 含 OP-G21 歸併) | `OPGAP-BFF-MAIN-ASSEMBLY-20260830` |
| **OP-G09** | P1 | Agora routers 跨域 import 私有 helper/store | 主動修復 (Active Remediation) | `OPGAP-BE-AGORA-RESEARCH-20260830` |
| **OP-G10** | P2 | generic legacy action adapter 殘留 dead code；中央相容命令面未退役 | 清理刪除 (Cleanup & Deletion, 含 OP-G24 歸併) | `OPGAP-BE-COMMAND-PLANE-RETIREMENT-20260830` |
...
本規劃套件之任務目錄（`EXECUTION_TASK_CATALOG_2026-08-30.json`）具備以下 100% 確定性之 jq/Python 機器檢驗保證：
...
1. **GAP 單一擁有者檢驗 (OP-G01..OP-G20 恰好各出現一次)**：
   ...
   # Result: true (Exit 0)
   ```
2. **實作任務數量檢驗 (恰好 14 個實作任務)**：
   ```bash
   jq -e '.tasks | length == 14' docs/04/pantheon_full_product_operation_audit_2026-08-29/EXECUTION_TASK_CATALOG_2026-08-30.json
   # Result: true (Exit 0)
   ```
3. **修改構件唯一性檢驗 (無跨任務衝突寫入)**：
   ```bash
   ...
   # Result: true (Exit 0)
   ```
4. **對稱正規化前綴感知構件衝突檢驗 (Symmetric Normalized Prefix Collision Check)**：
   ```bash
   python3 -c 'import json, sys; d=json.load(open("docs/04/pantheon_full_product_operation_audit_2026-08-29/EXECUTION_TASK_CATALOG_2026-08-30.json")); arts=[(t["id"], a) for t in [d["plan_freeze_task"]]+d["tasks"] for a in t["artifacts"]]; bad=[(t1,a1,t2,a2) for i,(t1,a1) in enumerate(arts) for j,(t2,a2) in enumerate(arts) if i<j and (a1.endswith("/") or a2.endswith("/") or a1==a2 or a2.startswith(a1+"/") or a1.startswith(a2+"/"))]; sys.exit(1 if bad else 0)'
   # Result: Exit 0
   ```
5. **Owner != Reviewer 檢驗 (嚴格獨立審查)**：
   ```bash
   ...
   # Result: true (Exit 0)
   ```
6. **Dev VM 執行資源限定檢驗 (僅 Wave 3/4 宣告 `pantheon-dev`)**：
   ```bash
   ...
   # Result: true (Exit 0)
   ```
7. **單一倉庫與構件路徑相容性檢驗 (無複合 target_repo，且構件路徑符合倉庫範疇)**：
   ```bash
