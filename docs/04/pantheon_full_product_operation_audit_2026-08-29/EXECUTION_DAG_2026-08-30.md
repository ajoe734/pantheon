...
1. **五波次漸進演進（5-Wave Progressive Execution）**：
   ...
   - **Wave 3（原子部署）**：單一任務排他性取得 `pantheon-dev-vm` 資源，在 dev VM 執行原子 promotion 與容器健康驗收。
...
flowchart TD
    ...
    subgraph Wave3 ["Wave 3: Deployed VM Promotion (Resource: pantheon-dev-vm)"]
        T12["OPGAP-HOSTED-DEV-PROMOTION-20260830<br/>(Atomic Dev VM Deploy & Switch)"]
    end

    subgraph Wave4 ["Wave 4: Hosted Acceptance & Closeout (Resource: pantheon-dev-vm)"]
