...
### 1.3 Reverse Import Elimination
...
- Subrouters (including `agora/identity/router.py` and `agora/personalization/router.py`) must import shared authentication, session context, and idempotency utilities from `services/control-plane/bff/ports/` and shared domain helpers, with zero imports of `main.py`.

### 1.4 Single-Receipt Source Contract
...
The 441 HTTP route decorators are partitioned across 18 cohesive domain router owners:
...
16. **Command Adapters** (`OPGAP-BE-COMMAND-ADAPTERS-20260830`): Typed execution adapters.
