...
8. **Remaining tooling gap (out of product SA/SD scope)**: `OPGAP-DEVTOOL-BRIDGE-REPO-ALLOWLIST-V3-20260830` tracks the still-open defect that the live supervisor cannot derive `PANTHEON_ASSISTANT_DEV_BRIDGE_ALLOWED_REPOS` from `coordination.repositories` on its own and depends on an operator-injected environment override. This is development-tooling debt, not one of the 20 product operation gaps (`OP-G01`-`OP-G20`) above.
