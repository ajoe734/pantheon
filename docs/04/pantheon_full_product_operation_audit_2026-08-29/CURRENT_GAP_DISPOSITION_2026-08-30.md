...
The 20 operational gaps (**OP-G01** through **OP-G20**) from the frozen full product operation audit (`FULL_OPERATION_AUDIT_2026-08-29.md`) remain preserved with their original evidence and target DAG. Governed execution exposed two additional gaps, **OP-G21** and **OP-G22**, which are appended here rather than back-written into the immutable 30-task catalog. Status statements below distinguish the frozen plan, post-freeze code delivery, read-only deployment evidence, and still-unfinished hosted product proof.
...
2. **Current Baseline State**:
   - Hosted Pair: Pair ID `b9209d6382cf109fda2504d7622fe7d9f137a084b0214988cc5588fffdeabc93`, Release Candidate ID `1497419171e98b33b42a01ebfd76c60368ab20e75ea45ac5fda61636a289e1cd`, accepted at `2026-08-30T18:28:45Z`, Backend `4f0994be548f56da627740f5b7fb193844c1faed`, Frontend `7d30e78476be61222af63a089e7ab141aa43b809`, Controller Run `33325479949`, Gate Run `33327155009`, Execute-Plans Deploy Run `33327736542`, Status `accepted`.
   - Hosted identity baseline: Served `/deployment.json` and live `/bff/version` both report backend source commit `4f0994be548f56da627740f5b7fb193844c1faed` (reconciling previous manifest/runtime drift via controller run 33325479949 / gate 33327155009 / deploy run 33327736542); this product BFF runtime is distinct from remote `origin/dev` (`3322f802de76b91ee06fbbe08590ff2ed4bdaadf`) and the promoted command runtime (`b49ebd934f34b3c4a648c1da863adb51a917a397`). `OP-G03` remains planned for target candidate promotion under `OPGAP-HOSTED-DEV-PROMOTION-20260830` to deploy the unified post-remediation release candidate and verify matching runtime SHA-256 identities.
3. **Post-Freeze Release Attempt (not product completion evidence)**:
   - Pantheon controller run `33332882810` targeted backend `cbf4e0a7303de1c0e9a51614c99ac2d8ddd96cfe` and frontend `7d30e78476be61222af63a089e7ab141aa43b809`, but concluded `failure` because `Dev OpenClaw assistant live smoke under lease` failed with the `OPENCLAW_GATEWAY_TIMEOUT` incident recorded by OP-G22.
   - The controller's release job still dispatched exact execute-plans integration gate `33334694659`, which concluded `success` and produced release candidate `9122d0fecd5cf9d5ae574c4c5e802df1d336dd2fd778a54019d2ad4995a2843d`.
   - Exact switch run `33335314834` concluded `success` for pair `b33741b326b82dea85a647d812bf75880cf7e7e97f0a012cc5375e84ea2f5f21` using the `read-only` artifact (`real writes=false`, `stub writes=false`). Its `Coordinate bounded authenticated Persona proof` and `Confirm independent same-pair read-only restore` jobs were both skipped. A successful gate/read-only switch therefore does not convert the failed controller run into Firebase mutation proof or full hosted acceptance.
...
- Split into two independent acceptance lanes:
  - Agora authentic hosted demo: Assigned to `AGORA-AGC-14-HOSTED-DEMO-AUTHENTIC-V5-20260829` (in `execute-plans`, canonical status `in_progress` generation 9 under supervisor worker `codex1_2` / owner `Codex`, parent write-proof run `33328350776`, following PR #5442 backend fix and accepted pair restoration).
...
1. **Task-Board-vs-Git Drift & Reconciled Board IDs**:
   ...
   - **`AGORA-AGC-14-HOSTED-DEMO-AUTHENTIC-V5-20260829`**: Canonical status `in_progress` (generation 9, owner `Codex` / reviewer `Antigravity`, supervisor worker `codex1_2`, parent write-proof run `33328350776` dispatched for accepted pair `b9209d6382cf109fda2504d7622fe7d9f137a084b0214988cc5588fffdeabc93` following PR #5442 backend fix; distinct from `OPGAP-HOSTED-MGMT-ACCEPTANCE-20260830`).
...
## 5. Comprehensive 22-GAP Disposition Matrix
...
| `OP-G03` | P0 | `planned` | `OPGAP-HOSTED-DEV-PROMOTION-20260830` | Current accepted pair b9209d63... aligned /deployment.json and live /bff/version on 4f0994be548f56da627740f5b7fb193844c1faed; full product operation remediation requires final unified candidate promotion and post-switch identity verification. | Deploy verified atomic pair from current dev lines post-remediation and verify byte-identical SHA-256 in /deployment.json and /bff/version. | `OPGAP-HOSTED-DEV-PROMOTION-20260830` | Deployed /deployment.json and /bff/version match exact target pair SHAs and return identical runtime identities on pantheon-dev. |
...
| `OP-G21` | P0 | `review` | `OPGAP-BE-PERSONA-RECONCILIATION-MUTATION-PORT-20260830` | Deployed Persona provisioning reconciliation attempted `update_persona` through `ReadSurfacePorts`, producing `ReadSurfacePorts object has no attribute update_persona` and violating the read-only port boundary. | Keep `ReadSurfacePorts` mutation-free; persist `provisioning` and `provisioning_failed` terminal transitions through one explicit typed Persona mutation/command port backed by the authoritative store. | `OPGAP-BE-PERSONA-RECONCILIATION-MUTATION-PORT-20260830` | Focused reproduction and regression tests prove zero read-surface mutation, durable terminal reconciliation, no startup warning, and hosted detail/workshop readback on the exact accepted pair. |
| `OP-G22` | P0 | `code_delivered_hosted_reproof_required` | `OPGAP-OPENCLAW-PROVIDER-READINESS-FALLBACK-20260830` | Run `33332882810` job `99314532003` exhausted readiness attempts while the expired primary Claude CLI OAuth session consumed the candidate budget; observed code `OPENCLAW_GATEWAY_TIMEOUT`. | Partition the bounded readiness budget across configured candidates, retain the healthy probed fallback for the subsequent single invoke, require exact readiness sentinel, and prohibit invoke-time retries of ambiguous side-effecting turns. | `OPGAP-OPENCLAW-PROVIDER-READINESS-FALLBACK-20260830` | PR #5471 exact head `74ec2510e3fc757a0d01e6d5a5cf015f07dd3f5b` merged as `4df84a187219f17f7a41036c4fdc7d6b594ebbbe`; evidence records 4/4 deployed smoke and no credential, secret, or provider-priority change. The failed aggregate controller run remains failed and future full-product promotion must re-prove the combined candidate. |
...
## 7. Plan-Execution Errata: Batch B/C Canonical Replacement Lineage (2026-08-30)

The frozen `EXECUTION_TASK_CATALOG_2026-08-30.json` (Section 5 above) is immutable planning evidence and is never edited in place. When the live supervisor actually drained the signed DevTaskPackets derived from it, the dispatch history diverged from the catalog's `depends_on` graph in a specific, fully-evidenced way, now recorded machine-readably in [EXECUTION_REPLACEMENT_LEDGER_2026-08-30.json](./EXECUTION_REPLACEMENT_LEDGER_2026-08-30.json).

1. **Root cause**: `OPGAP-DEVTOOL-TARGET-REPO-BRIDGE-20260830` (Batch A bootstrap) delivered a dev-bridge dispatcher whose allowlist of writable repositories was not derived from the live supervisor's `coordination.repositories` configuration. Batch B materialized while only `pantheon` was configured. When the Batch C mixed `pantheon`/`execute-plans` packet drained, the dispatcher rejected it (`allowedRepos contains unconfigured repositories: execute-plans`).
2. **Canonical task definitions cannot be amended in place**: `ai_status.py` has no update-in-place path for a task's id, artifacts, owner/reviewer pairing, or dependency binding. The only exits from a stuck `todo`/`in_progress` row are `done`, `blocked`, or `supersede`. Because all 14 Batch B rows were created before the allowlist fix existed, none could be repaired by editing `depends_on` — each was superseded by a new row (`-20260830` -> `-V2-20260830`) that binds the corrected dependency (`OPGAP-DEVTOOL-TARGET-REPO-BRIDGE-V2-20260830`).
3. **Fail-closed evidence** (full detail in the ledger's `fail_closed_evidence` array): the initial Batch C unconfigured-repository rejection, two authoritative command-runtime identity check failures (`PANTHEON_TASK_STATE_STORE_MODE=authoritative is required`, then `PANTHEON_COMMAND_RUNTIME_SHA is required with PANTHEON_COMMAND_ROOT`), and the successful cross-repository packet readback that finally admitted all 9 Batch C tasks.
4. **Safe replacement-before-supersede sequence**: the Batch B V2 replacement packet was admitted at `2026-08-30T20:14:00Z`, and the 14 Batch B V1 rows were archived as `superseded` only afterward, at `2026-08-30T20:16:09Z`. No dependency window ever pointed at a terminal id with no live successor.
5. **V2 cutoff: Batch B (14) superseded, Batch C (9) not yet superseded**: because the Batch C packet never reached a prior successful admission, the 9 Batch C task ids materialized directly on their first successful drain. At the V2 ledger cutoff they carried no supersede record and were correctly recorded as identity mappings; Section 7.9 records their later governed V2 replacements.
6. **Superseded terminal facts do not satisfy downstream dependencies**: a `superseded` task row is archived with `terminal_status: "done"` for bookkeeping, but the supervisor's dependency-satisfaction check binds to the exact id named in `depends_on`. All Batch B/C rows in canonical state now depend on `OPGAP-DEVTOOL-TARGET-REPO-BRIDGE-V2-20260830`, never on the terminal-superseded `OPGAP-DEVTOOL-TARGET-REPO-BRIDGE-20260830`.
7. **Batch D dependency transformation still pending**: the frozen catalog's `OPGAP-BFF-MAIN-ASSEMBLY-20260830` (Batch D) names all 14 original Batch B ids plus the 5 Batch C support-router ids in `depends_on`. Batch D has not yet materialized (still gated on Batch B/C completion); when it does, its `depends_on` must be rebound to the 14 V2 replacement ids while the 5 unchanged Batch C support ids pass through unmodified — recorded in the ledger's `batch_d_dependency_transformation` table so the rebinding is not missed at materialization time.
8. **V2 cutoff did not close the documentation gap**: PR #5462 exact head `58da56ea59f8d0dcfc2cdf83b64542cc6b71736a` merged as `4cb0007386a6ad0c48b68cc2d71663f3655e5424` and remains valid evidence for the ledger's original `2026-08-30T20:45:00Z` snapshot. It did not include later Batch C V2 replacements, Training V3, allowlist V4, Batch D materialization, OP-G21/OP-G22, or the exact deployment/write-proof outcomes. V3 supersedes only that incomplete documentation result; it does not rewrite V2's reviewed history.
9. **Batch C was later superseded after first admission**: the nine original Batch C ids did materialize directly on their first successful admission, exactly as V2 recorded. After that cutoff, every original Batch C row was governed-superseded by its one-to-one `-V2-20260830` replacement. The original direct-admission array and the later replacement array coexist in the machine ledger because they describe different canonical times.
10. **Evidence-contract replacements**: `OPGAP-BE-TRAINING-ROUTER-V2-20260830` was superseded by completed V3 (PR #5474, merge `fb55131864957a5ede398164e6ee060da1e0dead`), and `OPGAP-DEVTOOL-BRIDGE-REPO-ALLOWLIST-V3-20260830` was superseded by completed V4 (PR #5473, merge `5b0d02196acfc9c3ef956ae4c47865601bc43da6`). Both replacements preserve functional scope and add the immutable evidence-manifest artifact contract required for exact-head review.
11. **Allowlist runtime gate remains fail-closed**: V4 delivered automatic derivation from `coordination.repositories`, and the current governed command runtime contains its merge. That satisfies the present materialization transport, but any new or replayed mixed-repository packet is still gated on a promoted runtime containing V4 and a live registry containing `execute-plans`; no operator-injected environment override is accepted as durable product truth.

---

## 8. Product-Proof Completion Boundary

The post-freeze execution state does not support a complete-product claim:

1. Functional Batch D is not complete. `OPGAP-BFF-MAIN-ASSEMBLY-20260830`, `OPGAP-BE-COMMAND-CALLER-CUTOVER-20260830`, and `OPGAP-BE-COMMAND-PLANE-RETIREMENT-20260830` are materialized but `todo`; Main Assembly's immutable dependency list still names terminal-superseded Training V2 instead of completed Training V3 and therefore requires governed replacement/reissue; several other Batch B/C V2 predecessors are not `done`; the three frozen hosted promotion/acceptance task ids are not yet materialized.
2. `AGORA-AGC-14-HOSTED-DEMO-AUTHENTIC-V5-20260829` remains `in_progress`. Its acceptance requires a parent-bound Firebase/BFF write-proof with `proof_window_ack`, an armed watchdog, real GCP email/password login, exact object/receipt capture, negative controls, and restoration of the same pair to accepted read-only.
3. Run `33335314834` proves only the read-only switch. Because its bounded Persona proof and independent restore jobs were skipped, it cannot satisfy the mutation/watchdog prerequisite. Hosted proof remains open until successful immutable child and watchdog runs plus independent live-manifest refetch are bound to the exact evidence head.
