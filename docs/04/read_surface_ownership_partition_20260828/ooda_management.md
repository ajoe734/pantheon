...
1. **Pairwise Disjointness**:
   $$orall i, j \in \{	ext{train}, 	ext{cap}, 	ext{ooda}, 	ext{ops}, 	ext{res}, 	ext{ltg}\}, \; i 
eq j \implies D_i \cap D_j = \emptyset$$

2. **Complete Coverage**:
   $$igcup_{k \in \{	ext{train}, 	ext{cap}, 	ext{ooda}, 	ext{ops}, 	ext{res}, 	ext{ltg}\}} D_k = \mathcal{M}_{	ext{main.py}} \quad (|\mathcal{M}_{	ext{main.py}}| = 203)$$
...
2. **Direct Method Replacement Rules:**
   - Replace `read_store.list_ooda_packets(...)` $ightarrow$ `ports.ooda.list_ooda_packets(...)`
   - Replace `read_store.get_ooda_packet(id)` $ightarrow$ `ports.ooda.get_ooda_packet(id)`
   - Replace `read_store.list_ooda_packets_for_strategy(id)` $ightarrow$ `ports.ooda.list_ooda_packets_for_strategy(id)`
   - Replace `read_store.list_ooda_packets_for_runtime(id)` $ightarrow$ `ports.ooda.list_ooda_packets_for_runtime(id)`
   - Replace `read_store.list_ooda_packets_for_evolution_program(id)` $ightarrow$ `ports.ooda.list_ooda_packets_for_evolution_program(id)`
   - Replace `read_store.list_synthesis_conflict_logs(...)` $ightarrow$ `ports.conflict_logs.list_synthesis_conflict_logs(...)`
   - Replace `read_store.get_synthesis_conflict_log(id)` $ightarrow$ `ports.conflict_logs.get_synthesis_conflict_log(id)`
   - Replace `read_store.list_governance_review_queue_items(...)` $ightarrow$ `ports.review_queue.list_governance_review_queue_items(...)`
   - Replace `read_store.list_approval_queue_items(...)` $ightarrow$ `ports.review_queue.list_approval_queue_items(...)`
   - Replace `read_store.get_deployment_diff(id)` $ightarrow$ `ports.review_queue.get_deployment_diff(id)`
   - Replace `read_store.get_approval_decision(id)` $ightarrow$ `ports.review_queue.get_approval_decision(id)`
   - Replace `read_store.list_approval_decisions(...)` $ightarrow$ `ports.review_queue.list_approval_decisions(...)`
   - Replace `read_store.get_review_summary(id)` $ightarrow$ `ports.review_queue.get_review_summary(id)`
   - Replace `read_store.get_latest_run(id)` $ightarrow$ `ports.review_queue.get_latest_run(id)`
   - Replace `read_store.list_registry_entries()` $ightarrow$ `ports.review_queue.list_registry_entries()`
