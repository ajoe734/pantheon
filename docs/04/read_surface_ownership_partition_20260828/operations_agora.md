...
| **OODA & Management** | `ACG-RS-OODA-OWNERSHIP-MAP-20260828` | `ooda_management.py` | `4b9f0b5a28af08bde017ce85cc17226b94fcbe98` (PR #5357, merged on dev `a880bf5b2`) | **15** | **49** | 16 / 50 | OODA loop packets, synthesis conflict logs, governance review queue, approval decisions (51 total AST calls with 2 dynamic `getattr`; 16/50 in lexical space including L6953 docstring) |
| **Research & Knowledge** | `ACG-RS-RESEARCH-OWNERSHIP-MAP-20260828` | `research_knowledge_source.py` | `3507b39c8ced923351c04cdfe87fa3584c5a5d38` (PR #5359, merged on dev `78eea2641`) | **44** | **119** | 42 / 116 | Research tickets, experiments, analyses, artifacts, strategy specs, search index, dataset sources (including 13 `dataset_source` calls; 39 typed port APIs; 42/116 in PR #5359) |
| **Persona Training** | `ACG-RS-TRAINING-OWNERSHIP-MAP-20260828` | `persona_training.py` | `7853a6e64a5b0bf7c5815452dda3b9f02d8720af` (PR #5355, merged on dev `70378ff90`) | **17** | **31** | 17 / 31 | Interactive trainer sessions, trainer controls, preview evaluation, trainer replay commit/discard, rapid evaluation |
...
| **Lifecycle, Telemetry & Governance** | `ACG-RS-LIFECYCLE-OWNERSHIP-MAP-20260828` | `lifecycle_telemetry_governance.py` | `aa7d1558f945b6c81ad75583c51fd7172b440785` (PR #5360, merged on dev `1813a0344`) | **33** | **110** | 33 / 110 | Incidents, postmortems, kill switch, sentinel findings, loop runs, lineage, telemetry drift, evolution decisions (111 AST calls with L7736 dynamic `getattr`; includes 13 evolution calls across 3 methods) |
...
| 14 | `open_committee_session` | **WRITE** | 1 | `StrategyWorkshopStore.complete_command()` / `StrategyWorkshopStore.create_event()` | `services/control-plane/bff/agora/strategy_workshop/store.py` | Concrete durable store in strategy_workshop |
| 15 | `close_committee_session` | **WRITE** | 1 | `StrategyWorkshopStore.complete_command()` | `services/control-plane/bff/agora/strategy_workshop/store.py` | Concrete durable store in strategy_workshop |
...
| 18 | `submit_committee_session_memo` | **WRITE** | 1 | `ConsultationStore.put_memo()` / `StrategyWorkshopStore.ensure_current_version_link()` | `services/consultation/store.py` & `services/control-plane/bff/agora/strategy_workshop/store.py` | Concrete durable store in consultation & strategy_workshop |
| 19 | `publish_committee_session_memo` | **WRITE** | 1 | `ConsultationStore.put_memo()` / `ConsultationServiceClient.intake_policy_learning_candidate()` | `services/consultation/store.py` / `ConsultationServiceClient` | Concrete durable store in consultation |
| 20 | `create_agora_handoff` | **WRITE** | 2 | `TradingRoomStore.upsert_handoff()` / `ConsultationStore.put_handoff()` | `services/control-plane/bff/agora/trading_room/store.py` & `services/consultation/store.py` | Concrete durable store in trading_room & consultation |
| 21 | `create_agora_committee_evidence_pack` | **WRITE** | 1 | `StrategyWorkshopStore.create_event()` / `ConsultationStore.put_evidence_attachment()` | `services/control-plane/bff/agora/strategy_workshop/store.py` & `services/consultation/store.py` | Concrete durable store in strategy_workshop & consultation |
| 22 | `get_agora_committee_evidence_pack` | **READ** | 1 | `StrategyWorkshopStore.list_events()` / `ConsultationStore.list_evidence_for_request()` | `services/control-plane/bff/agora/strategy_workshop/store.py` & `services/consultation/store.py` | Concrete durable store in strategy_workshop & consultation |
| 23 | `append_agora_committee_evidence_files` | **WRITE** | 1 | `StrategyWorkshopStore.create_event()` / `ConsultationStore.put_evidence_attachment()` | `services/control-plane/bff/agora/strategy_workshop/store.py` & `services/consultation/store.py` | Concrete durable store in strategy_workshop & consultation |
...
| 25 | `create_agora_note` | **WRITE** | 1 | `DefaultResearchKnowledgeSourcePort.create_research_note()` / `ResearchWriteOwner.create_research_note()` | `services/control-plane/bff/domain_ports/research_knowledge_source.py` / `services/research/` | Concrete port/owner in research_knowledge_source |
...
| 33 | `create_agora_training_example` | **WRITE** | 1 | `InteractionLifecycleStore.create_request()` / `PersonaTrainingDomainPort.create_training_example()` | `services/control-plane/bff/agora/interaction/store.py` & `domain_ports/persona_training.py` | Concrete store/port in interaction & training |
...
| 35 | `record_agora_audit_event` | **WRITE** | 9 | `ProposalStore.once()` / `InteractionLifecycleStore.create_request()` / `ConsultationStore.append_audit()` | `services/control-plane/bff/agora/governance/store.py`, `interaction/store.py`, `consultation/store.py` | Concrete durable store in governance, interaction & consultation |
| 36 | `create_decision_journal_entry` | **WRITE** | 1 | `CandidateDecisionStore.create_candidate()` / `TradingRoomStore.upsert_decision_event()` | `services/control-plane/bff/agora/candidate_decisions/store.py` & `trading_room/store.py` | Concrete durable store in candidate_decisions & trading_room |
...
| 41 | `record_sponsor_decision` | **WRITE** | 1 | `ConsultationServiceClient.record_sponsor_decision()` / `ConsultationStore.append_audit()` | `services/consultation/client.py` & `services/consultation/store.py` | Concrete durable client/store in consultation |
...
| 43 | `create_consult_request` | **WRITE** | 1 | `ConsultationStore.put_request()` / `ConsultationServiceClient.create_request()` | `services/consultation/store.py` & `services/consultation/client.py` | Concrete durable store/client in consultation |
| 44 | `cancel_consult_request` | **WRITE** | 1 | `ConsultationServiceClient.cancel_request()` / `ConsultationStore.put_request()` | `services/consultation/client.py` & `services/consultation/store.py` | Concrete durable store/client in consultation |
