...
- 4 個循環有直接功能／契約阻斷：Source Ingestion、Promotion/Deployment、Capital Pool
  Execution、BFF Health Monitoring。
- 8 個循環的主要 domain flow 已存在，但缺 current deployed E2E 或被上游阻斷：Strategy
  Distillation、Alpha Replication、Persona Teaching、Agora Interaction Evidence、Human
...
- authenticated `/bff/v5/loop-health` 回傳 12 canonical + 1 composite overlay，但 12 個
  canonical rows 全部 `is_live=false`，controller runtime record count 為 0，surface 為
  `degraded/registry_metadata`。
...
| 2 | Strategy Distillation | default-on controller、durable queue、Registry draft write、terminal identity、loop writer | 未發現新的獨立功能缺口；上游 Source primary trigger 未完成 | 沒有最新 dev 的 SourceRecord→StrategySpec→Alpha admission deployed proof | 部分閉環 |
...
| 12 | BFF Health Monitoring | 單一 `DownstreamHealthMonitor`、typed API probes、durable telemetry/incident outbox、error-rate ledger、v5 read models | (a) 只前三循環寫 `LoopControllerRecord`，其餘九個 owner observation 未開發；(b) target registry只探 API與 paper fleet，漏掉真正執行工作的 schedulers/consumers與 `paper-signal-producer`；(c) `record_downstream_outcome` 沒有 production caller，event-driven error spike只做了 method沒有接 adapter；(d) live BFF目前讀不到任何 controller record | Management 全部顯示 non-live，且看不到目前 producer unhealthy 的正確 loop原因 | **直接阻斷** |
...
### GAP-F02 — RuntimeBinding 必須是一個可執行契約
...
### GAP-F03 — Capital continuous loop 的 market input owner
...
### GAP-F04 — 九個 owner 的 runtime observation
...
目前 live readback連這三筆都是 0。再由其餘既有 owner在真實 trigger/terminal/readback時寫
同一 contract：
...
### GAP-F05 — BFF worker health 與 event-error 真正接線
...
### GAP-F06 — Current deployed E2E 與 cross-loop stimulus
...
2. UI只能誠實呈現 BFF輸入；目前 backend static catalog仍把 Loops 4–12 controller標成
   `not_implemented`，所有 runtime health又是 unobserved，因此畫面雖沒有造假，仍不能提供
...
- focused unit/contract suite：`157 passed`；
...
- RuntimeBinding executable projection contract（GAP-F02）。
- 九個 owner observation adapters，可按互斥 service scope平行（GAP-F04）。
- BFF worker target/error-outcome wiring（GAP-F05）。
...
```text
GAP-F02 RuntimeBinding contract
  -> GAP-F03 continuous market input / canonical binding migration
  -> Runtime 8–12 deployed E2E

GAP-F01
  -> Research 1–4 deployed E2E

各 owner observation + GAP-F05
...
| Management truth | `services/control-plane/bff/downstream_health_monitor.py`；`loop_inventory.py`；`services/loop-control/`；`docs/deployment/loop-catalog.registry.json` |
...
下一階段若要製作 SD與 execution tasks，必須只從 GAP-F01～F06切片，先與 canonical active/
