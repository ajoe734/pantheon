...
> 2026-08-20 operator posture override：開發與驗收繼續，但 dev 不得常駐對外拉資料。
> `source-ingest-scheduler` 預設不啟動且 external egress 維持 deny；只有明確選取
> `source-ingest-scheduler` profile 的測試，才可在 exact-host allowlist、單一 tick、
> `restart: no` 的限制下手動拉取一次。Loop 1 的 continuous scheduled closure 因此是
> operator-directed hold，不得以恢復常駐外拉方式修復。
