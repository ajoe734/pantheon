...
這份 working source 不應再把前一波已 archive-done 或 loop-complete 的 task id 清單直接當成 active implementation lane。

截至目前，真正仍未收斂的主線只剩：

- `EP5-002` human-gated canary / live proof
- runtime verification 覆蓋仍為 `32/46`
- `TRL` / `Qlib` 仍在 smoke-tested -> activation-ready 收尾
- `FinRL` / `RLlib` / `Ray Tune` / `W&B` 仍維持 deferred / criteria-defined，不屬於本輪未完成主線
- cross-repo docs / status / readiness wording rebaseline

補充 truth：

- `front-ai-trading-system` default branch 已掛上先前那批 live routes，不應再把 `EW-04/05`、`CW-01/03/04`、`PKT-001/003` 視為 blocked-shell 主線缺口
- `Settings` surface 已切到 Pantheon BFF-backed `/api/v1/settings*` 路由，後續若有工作應視為 config-domain refinement，而不是「尚未接 Pantheon」
