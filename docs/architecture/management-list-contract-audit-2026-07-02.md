...
| Result | 169 existing contract smells after the first seven remediation slices: 33 P0, 136 P1 |
...
| `camel-snake-duplicate` | 128 | P1 | DTOs return both casing variants for the same fields |
...
| Persona League Family | Remediated in `MGMT-LIST-CONTRACT-007`: league, rankings, movers, tiers, heatmap, quarterly ranking, recommendations, and legacy `/bff/persona-league` now use one list envelope without top-level aliases | Continue removing camel/snake duplicates and move detail-grade row hydration behind detail routes |
...
4. Done in `MGMT-LIST-CONTRACT-006`: PM12 analytics tables now use one
   ...
   aliases.
5. Done in `MGMT-LIST-CONTRACT-007`: Persona League and Quarterly Ranking list
   routes now use one `data.items` envelope, retired top-level
   `items`/`rows`/`rankings`/`recommendations`/`movers`/`tiers` aliases, and
   removed the shadowed legacy `/bff/management/persona-league` decorator.
6. Normalize Cost Attribution and the remaining NL/AI management list families.
