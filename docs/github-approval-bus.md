...
- Before creating anything, ReviewBus queries all GitHub PRs for that exact task branch. When the local/task head SHA is available, candidates must match it.
...
- If exact task evidence targets a legacy or otherwise unexpected base, ReviewBus records `skipped_base_mismatch` plus candidate details and refuses to create a synthetic broad PR. Multiple commits without an exact head SHA, multiple matching PRs, closed-unmerged evidence, and missing head SHA also fail closed with actionable activity-log diagnostics.
