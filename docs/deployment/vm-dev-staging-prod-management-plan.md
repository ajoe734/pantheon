...
- `core`、`workers`、`research`、`management-ai`、`execution` service inventory。
- Dev 現況 CPU、memory、disk、startup time 與 deployment duration 基線。
- FE/BFF、DB migration、worker ownership、backup/restore 現有能力差距。
- Production RPO/RTO、real-capital scope 與 on-call owner 決策。

完成條件：能以量測結果選 instance size，不以完整 service 數量推測。
