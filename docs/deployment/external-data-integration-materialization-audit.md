...
| P2-SOURCE-SEARCH-LIVE-CONNECTOR-SMOKE-001 | in_progress | Claude | Bounded live/test source connector smoke through SourceRecord/EvidenceBundle, durable search index, and BFF/SearchGateway readback. |
...
| P2-SOURCE-SEARCH-LIVE-CONNECTOR-SMOKE-001 | In progress: bounded live connector smoke with end-to-end integration test. | test_source_search_end_to_end_durable_readback: bounded external feed connector → SourceRecord/EvidenceBundle with PIT/entitlement/governance → shared durable evidence store → search index refresh → SearchGateway durable query with citation refs. 138 source+search tests pass. Governance enforcement (no Lean/broker/execution routes) confirmed in test_live_connector_smoke_rejects_forbidden_execution_routes. | dependency_missing evidence written (no live feed URL configured); bounded in-process proof via the integration test. | Pending Codex2 review approval. |
