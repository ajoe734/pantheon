...
| **DB Migration Execution** | Per-service `db_migrate.sh` / alembic SQL scripts executed on container startup. | Forward migration rehearsal on sanitized snapshot data before service startup. | Expand/contract migration policy; zero-downtime backward-compatible schema changes. | **High** (Phase 1/2 Expand/contract guard + snapshot restore pipeline) |
...
| **Execution Boundary Isolation** | Dev VM co-locates `runtime-manager` & `broker` but disables live trading (`PANTHEON_LIVE_BROKER_ENABLED=false`). | Standard releases use single VM; Execution releases provision temporary dual-VM staging. | Dedicated Prod Execution VM with strict firewall, internal-only VPC, and isolated broker secrets. | **High** (Phase 4 Real-capital execution boundary) |
