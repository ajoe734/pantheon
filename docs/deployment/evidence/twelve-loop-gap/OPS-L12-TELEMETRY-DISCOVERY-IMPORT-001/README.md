...
**Base:** `dev` @ `71aea154b8a1ab6e652e02018f47c57f26513de0`
...
The first attempt (anchor `80ecb47e8`, evidence commit `bcd850aef`, PR #4219
from base `6445eacd6`) failed the required `Commit trailers` check: the anchor
subject `OPS-L12-TELEMETRY-DISCOVERY-IMPORT-001: anchor telemetry test import
repair` was 75 characters against a 72-character limit. The branch had also
fallen behind `dev`. Force-push recovery is not authorized for background
workers, so PR #4219 was closed with an explanatory comment, the remote task
branch was deleted, and the identical three-file delivery was re-cut from `dev`
`71aea154b` with a 60-character anchor subject.

`services/telemetry` is byte-identical between `6445eacd6` and `71aea154b`
(the newer tip only added `OPS-CI-PR-TRAILER-RANGE-001` evidence docs), and both
the baseline and post-change discovery runs were re-executed on the new base.
No delivered file content differs between the two cuts.
