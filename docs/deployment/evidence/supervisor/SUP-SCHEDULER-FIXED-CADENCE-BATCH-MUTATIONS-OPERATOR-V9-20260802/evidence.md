...
Human/Ops rejected PR #4520 heads `ab0d79e33bb1dfff452c93c32a96721168222ad9`, `fb923abe2182acb4b5c10a0e557040b968598008`, `ba3b176500f557944eb6f3e08709361281609989`, and `c9299f51a4ccb6d76bd3964f060cfde87673201b`. This repaired head addresses all six cumulative blocking findings:
...
5. Marker proof no longer rounds the fractional launch intent down to the start of its wall-clock second. Same-second running and terminal markers are rejected as ambiguous, a live marker without an exact process candidate cannot claim the lease even when it starts in a later second, and a strictly post-intent terminal marker remains recoverable. The forked hard-crash test now launches a wrapper-shaped process with the exact task, agent, provider, and run environment, proving positive adoption through `/proc` rather than the removed marker-only shortcut.
6. Marker proof now also rejects exact equality. This closes the legacy/reconstructed intent case where both `prepared_at` and the runner's `started_at` have one-second resolution: equality cannot establish post-intent order, so neither a running nor terminal marker is eligible. The strictly-next-second terminal and exact-process hard-crash positive paths remain green.
...
- Supervisor: 579 tests and 158 subtests passed.
