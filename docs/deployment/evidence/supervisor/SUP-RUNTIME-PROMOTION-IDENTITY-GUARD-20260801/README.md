...
1. `validate_candidate_root`: Validates that candidate root resolves under `/home/lupin/pantheon-ci-deploy/command-runtimes/<40-hex-commit>` with exact basename-to-HEAD SHA matching, rejecting `/tmp`, worker worktrees, symlink escapes, deleted roots, and invalid naming.
2. `capture_config_bytes_identity`: Captures live config file bytes, length, and SHA256 before handoff.
...
## Verification Output
1. Unit tests pass (28/28 passed):
   ```bash
