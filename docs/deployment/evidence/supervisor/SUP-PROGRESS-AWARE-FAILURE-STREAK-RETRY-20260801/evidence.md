...
2. **Supersession Thesis (Acceptance Item 10)**:
   ...
   - The generic progress-aware failure streak recovery feature was implemented on `dev` via `SUP-FAILURE-STREAK-RECOVERY-DECISION-V2-20260801` (commit `e84fabefe`, PR #4489) and `SUP-FAILURE-STREAK-DISPATCH-CONSUMPTION-V2-20260801` (commit `25c30277c`, PR #4491), introducing `decide_failure_streak_recovery`, `failure_streak_recovery_progress_generation`, and `failure_streak_recovery_decision_for_task` in `.orchestrator/supervisor.py`.
   - Thus, PR #4385 is fully superseded by the V2 generic infrastructure on `origin/dev`.

3. **Test Collection Status (Acceptance Item 9)**:
   - Command: `PYTHONPATH=.orchestrator .venv-pantheon/bin/python3 -m pytest .orchestrator/test_supervisor.py --collect-only`
   - Outcome: Collection is currently blocked on `origin/dev` tip due to an upstream ImportError: `cannot import name 'provider_auth_probe_due' from 'provider_permissions'` at line 88 of `.orchestrator/supervisor.py`.
   - This test collection blocker originates from dev-tip import dependency mismatch in `provider_permissions.py` and is documented here for empirical transparency.
...
| Item 9 | Zero-net code deletion & dev tip alignment | `evidence.json`, Git branch HEAD | Cleanly merged with `origin/dev` tip (`ab5caf7d4`), preserving dev infrastructure without code deletion. Test collection error on dev tip documented in Section 1. |
| Item 10 | PR #4385 supersession evidence | `evidence.md` Section 1 | PR #4385 superseded by generic progress-aware failure streak recovery V2 (PR #4489 / #4491, commits e84fabefe / 25c30277c). |
