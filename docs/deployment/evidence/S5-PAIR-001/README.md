...
This document records the exact evidence reconciliation for Release I (`ae41705b4637110e665d2eed735afbd8307e28e6` on `pantheon` / `dbe737e0676640f1b9b2395b54fb3c0416099f8a` on `execute-plans`) under the 2026-09-13 operator handoff. This reconciliation explicitly distinguishes accepted Release-I reconciliation from literal original criteria: in the observed execution sequence, BFF was publicly switched before the FE gate ran, whole-pair prequalification was not performed concurrently offline, and run `34750913608` compensation steps were skipped because deployment succeeded. Literal failure compensation was not exercised in this run; full bidirectional rollback roundtrip remains governed by `S5-ROLLBACK-001`.
...
| 3 | **Run gate-before-switch against the final FE public-origin bytes and exact staged BFF; strict auth, CORS, safe-write and paper-only defaults are mandatory. No public switch until all required gates and served readbacks pass.** | **RECONCILED (ACCEPTED UNDER OPERATOR HANDOFF; LITERAL WHOLE-PAIR PREQUALIFICATION NOT SATISFIED)** | Gate controller `cross_repo_release_controller.py` and integration gate run `34751164876` executed successfully. In literal execution, BFF was publicly switched (`10:09:55Z`) before the FE gate (`10:10:27Z`) and FE switch (`10:34:41Z` / `10:49:48Z`), so whole-pair prequalification was not performed concurrently offline. Accepted under the 2026-09-13 operator handoff based on verified gate run `34751164876`, strict auth (`auth_mode=strict`, `auth_stub=false`), canonical CORS (`https://app.dev.mvl-cap.tw`), served readbacks, and safe-write defaults (read-only profile has `VITE_BFF_REAL_WRITES=false`; served operator-live profile has `VITE_BFF_REAL_WRITES=true` with paper-only backend boundaries). **Execution sequence boundary**: see Section 4 for detailed disclosure. |
| 4 | **On any failure, compensate to the exact prior pair and publish terminal outcome plus durable readback; unknown digest or mixed served identity is failure.** | **NOT EXERCISED IN RUN 34750913608 (HISTORICAL COMPENSATION CAPABILITY REFERENCED)** | In run `34750913608`, deployment succeeded, so failure compensation steps were skipped (not exercised). Workflow `nonprod-deploy.yml` defines compensation restoring `previous_backend_sha` (`cdc02e2c65136275e00828950c18e53672fa5a40`) and `previous_frontend_sha` (`ba0b47f445517ded95d910ee9a62e779a6a8d9d1`), and compensation was historically observed in Release B, C, and F runs. However, failure cases were unexercised in run `34750913608` and are not marked passed. Full bidirectional roundtrip drill (`accepted -> prior -> accepted`) remains governed under `S5-ROLLBACK-001`. |
...
| **FE Dist Artifact (Read-Only)** | `eab971709869b3f443222a13f769862072a4b2ad63557363c613ca0f1c982386` | Canonical asset-manifest SHA256 (computed by execute-plans `scripts/release-candidate.mjs` lines 554-566 over canonical asset bytes excluding `deployment.json`) for read-only profile |
| **FE Dist Artifact (Operator-Live)** | `01fbeb26d1d7523b44bf263e5c914eb0452ee6e620d9cacf40e963338432289c` | Canonical asset-manifest SHA256 (computed by execute-plans `scripts/release-candidate.mjs` lines 554-566 over canonical asset bytes excluding `deployment.json`) for operator-live profile |
| **GitHub Actions FE Artifact Digest** | `sha256:9b4628a482aa963469c40a263962e5b99ecfd9599330be07aabaff230db152f4` | GitHub Actions uploaded artifact `10315906574` ZIP archive digest |
...
**Boundary Truth**:
The actual system architecture updates the BFF first, gates the FE candidate against the staged BFF, and then switches the FE with compensation back to the exact prior pair if any failure occurs. It is **not** a simultaneous zero-downtime offline whole-pair qualification switch. In run `34750913608`, because deployment succeeded, failure compensation steps were skipped (not exercised); literal failure compensation was not triggered in this run. Full bidirectional roundtrip drill remains governed under `S5-ROLLBACK-001`.

**Safe Write Defaults & Paper/Live Boundary**:
The served frontend bundle on `pantheon-dev-fe` uses the `operator-live` profile with `VITE_BFF_MODE=live`, `VITE_BFF_FALLBACK=strict`, `VITE_BFF_REAL_WRITES=true`, `VITE_BFF_ALLOW_DEV_STUB_WRITES=false`, and `VITE_BFF_EMBEDDED_BEARER_TOKEN=false`. The staging profile was read-only with `VITE_BFF_REAL_WRITES=false`. Enabling `VITE_BFF_REAL_WRITES=true` in the dev operator-live profile allows operator UI actions against dev BFF paper endpoints, but backend execution on dev is strictly paper-only (fail-closed for live capital). Serving `VITE_BFF_REAL_WRITES=true` is not proof of live-capital activation or paper-only validation.
