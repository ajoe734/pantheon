...
3. **Bounded Readiness Probe**: The 20s readiness probe budget is partitioned across candidates, bounding the primary candidate to <= 5s when fallbacks exist and allocating up to 10s to live Codex fallback (`openai/gpt-5.6-sol`). If primary model times out or returns non-zero exit, probe captures sanitized `primary_unavailable` evidence and converges quickly via fallback without incurring excessive delay.
...
## Verification
- Unit & live test suites: 447 passed, 4 skipped across `services/openclaw-gateway-adapter` and `scripts/test_openclaw_assistant_openclaw_live_smoke.py`.
