...
2. **Strict Guardrail & Regression Suite (`scripts/test_deploy_nonprod_vm.py`)**:
   ...
   - `test_postgres_db_behavior_vacuum_succeeds_without_enospc`: executes live `VACUUM;`, `VACUUM ANALYZE;`, and `VACUUM FULL;` confirming 0 errors.
...
- `.venv-pantheon/bin/python3 -m pytest -v scripts/test_deploy_nonprod_vm.py` (15 passed)
...
- Live database execution `VACUUM FULL;` + `VACUUM ANALYZE;` completed with code 0 and 0 errors
