...
4. **Cross-Process Serialization & Replay Deduplication**:
   Invocations serialize across processes using an exclusive file lock on `<state_path>.lock`. When an `operation_key` is supplied, repeated/replayed executions are deduplicated against recorded terminal operations in `ControllerStateStore`, returning terminal readback without re-triggering provider egress. Neither failure nor completion leaves recurring processes, background daemons, or polling loops.
