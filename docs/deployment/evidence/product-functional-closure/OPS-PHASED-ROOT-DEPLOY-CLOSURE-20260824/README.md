...
4. **Release Admission Failure Isolation and Automatic Exact-BFF Rollback**:
   ...
   - Any failure in post-up verification gates triggers `rollback_dev_bff_on_failure` on the VM, automatically restoring `operator-bff` and `loop-run-projector-scheduler` to the captured rollback baseline SHA.
    ...
    - **Bounded Compensation Budget and Contention Modeling**: `deploy-dev` job `timeout-minutes: 60` (3600s) provides an explicit bounded compensation budget that accommodates the initial deploy deadline (`DEV_DEPLOY_DEADLINE_SECONDS=1200`), the quarantined original lease TTL wait (`--ttl-seconds 300`), and the rollback compensation deploy deadline (`DEV_DEPLOY_DEADLINE_SECONDS=1200`) = 2700s total, with 900s of headroom for setup, candidate build, paper bootstrap, and post-rollback verification. Active-lease contention against the quarantined predecessor lease is modeled and proven: rollback acquisition polls and takes over the expired lease, restoratively executes baseline deploy under `TARGET_ENV=dev`, and releases the lease.
...
- `pytest -v scripts/test_dev_environment_lease_deploy_contract.py` (201 passed, including positive and negative executable contract checks for mutation-aware deploy_compensation with dedicated rollback lease under TARGET_ENV=dev, real heartbeat lifecycle and preceding failure-state handling, quarantined lease active contention and TTL takeover, compensation budget hierarchy asserting initial deadline + lease TTL wait + rollback deadline fits within job timeout, nested baseline compensation binding, post-up failure handling, target GIT_SHA prebuild export, built image identity verification, full environment export, and inner rollback variable preservation)
- `pytest -v scripts/test_dev_environment_lease_guard.py scripts/test_dev_environment_lease.py scripts/test_cross_repo_release_controller.py` (42 passed)
- `pytest -v scripts/test_*deploy*.py` (314 passed across all deploy contract test suites)
