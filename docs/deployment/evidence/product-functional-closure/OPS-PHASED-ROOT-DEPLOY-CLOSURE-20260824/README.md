...
- `pytest -v scripts/test_dev_environment_lease_deploy_contract.py` (198 passed, including positive and negative executable contract checks for mutation-aware deploy_compensation, nested baseline compensation binding, post-up failure handling, target GIT_SHA prebuild export, built image identity verification, full environment export, and inner rollback variable preservation)
- `pytest -v scripts/test_*deploy*.py` (311 passed across all deploy contract test suites)
